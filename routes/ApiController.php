    ...
    public function randevular(Request $request, $isletme_id, $returnres)
{
    ...
    if ($returnres == 2) {
        $query = Randevular::where("salon_id", $isletme_id);
    ...
    $randevular = Randevular::where("salon_id", $isletme_id);
    
    // Add date range filtering for main query if provided
    if ($startDate && $endDate) {
        $randevular->whereBetween('tarih', [$startDate, $endDate]);
    }
    
    $randevular = $randevular->pluck("id");
    ...
    // Add date range filtering for appointments query if provided
    if ($startDate && $endDate) {
        $randevu_hizmetler->whereBetween('randevular.tarih', [$startDate, $endDate]);
    }
    
    $randevu_hizmetler = $randevu_hizmetler->get();
    ...
    return [
        "randevular_liste" => Randevular::where("salon_id", $isletme_id)
...

{
    ...
    public function yeniMusteriKaydi($adsoyad,$telefon)
    {
        $user = new User();
        $user->name = $adsoyad;
    ...
    public function aktarimMusteriKontrol(Request $request)
    {
        ...
        $user->dogum_tarihi = $request->dogumTarihi != "" ? date('Y-m-d',strtotime($request->dogumTarihi)) : null;
        $user->meslek = $request->meslek;
