...
// GECICI TESHIS: secili personelin belirli gundeki calisma/mola verisi + saatgetir ara degerleri
// Kullanim: /dev-saat-teshis?personel=Yaprak&tarih=2026-09-07
Route::get('/dev-saat-teshis', function() {
    ...
    $day = (int) date('N', strtotime($tarih));
    $ad = request('personel', '');
    $pers = \App\Personeller::where('salon_id', $salon->id)
        ->when($ad, function($q) use($ad){ $q->where('personel_adi','like','%'.$ad.'%'); })->get();

    $out = "<pre style='font:13px monospace;padding:20px;line-height:1.7'>";
    $out .= "SALON #{$salon->id} {$salon->salon_adi}  domain={$salon->domain}\n";
    $out .= "TARIH={$tarih}  haftanin_gunu(N)={$day}  randevu_saat_araligi=".var_export($salon->randevu_saat_araligi,true)."\n";
    $sc = \App\SalonCalismaSaatleri::where('salon_id',$salon->id)->where('haftanin_gunu',$day)->first();
    $out .= "SalonCalisma[gun={$day}]: ".($sc ? ("calisiyor={$sc->calisiyor} {$sc->baslangic_saati}-{$sc->bitis_saati}") : 'YOK')."\n";
    $out .= "OnlineKisitlamaAktif=".var_export($salon->online_saat_kisitlama_aktif,true)."\n";
    $out .= "izinliAraliklar=".json_encode(\App\SalonOnlineRandevuSaatleri::izinliAraliklar($salon->id,$tarih))."\n";
    $out .= str_repeat('-',80)."\n";
    foreach ($pers as $p) {
        $out .= "PERSONEL #{$p->id} '".e($p->personel_adi)."' aktif=".var_export($p->aktif,true)." takvimde_gorunsun=".var_export($p->takvimde_gorunsun,true)."\n";
        $out .= "  PersonelCalisma (tum gunler):\n";
        foreach (\App\PersonelCalismaSaatleri::where('personel_id',$p->id)->orderBy('haftanin_gunu')->get() as $c) {
            $mark = ($c->haftanin_gunu==$day) ? '  <-- BUGUN' : '';
            $out .= "     gun={$c->haftanin_gunu} calisiyor={$c->calisiyor} {$c->baslangic_saati}-{$c->bitis_saati}{$mark}\n";
        }
        $out .= "  PersonelMola (tum gunler):\n";
        foreach (\App\PersonelMolaSaatleri::where('personel_id',$p->id)->orderBy('haftanin_gunu')->get() as $c) {
            $mark = ($c->haftanin_gunu==$day) ? '  <-- BUGUN' : '';
            $out .= "     gun={$c->haftanin_gunu} mola_var={$c->mola_var} {$c->baslangic_saati}-{$c->bitis_saati}{$mark}\n";
        }
    }
    $out .= str_repeat('-',80)."\n";
    $out .= "TARIHTEKI RANDEVULAR ({$tarih}) — user_id=2012 => KAPALI SAAT bloklari\n";
    $persIds = $pers->pluck('id')->toArray();
    $rlar = \App\Randevular::where('tarih',$tarih)
        ->with(['hizmetler'=>function($q){ $q->select('id','randevu_id','personel_id','hizmet_id','saat','saat_bitis','sure_dk'); }])
        ->get();
    foreach ($rlar as $r) {
        $kh = ($r->user_id == 2012) ? ' *** KAPALI SAAT ***' : '';
        $ilgili = false;
        $satir = "  RND #{$r->id} user_id={$r->user_id} durum=".var_export($r->durum,true)." tarih={$r->tarih}{$kh}\n";
        foreach ($r->hizmetler as $h) {
            if (in_array($h->personel_id, $persIds) || $h->personel_id==0) $ilgili = true;
            $satir .= "       rh#{$h->id} personel_id={$h->personel_id} hizmet_id={$h->hizmet_id} saat=".var_export($h->saat,true)." bitis=".var_export($h->saat_bitis,true)." sure={$h->sure_dk}\n";
        }
...
Route::get('/save-excel', function () {
   ...
   Route::post('/salonlar','HomeController@salonara')->name('salonara');
	...
	// Form POST -> /api/v1/siteden-yeni-kullanici-kaydi (2 adimli SMS dogrulamali).
	$ucretsizDeneme = function () {
		return response()->file(resource_path('views/static/ucretsiz-denemenizi-baslatin.html'));
	};
...
Route::prefix('sistemyonetim/v2')->namespace('SistemYonetim')->group(function() {
    ...
    Route::post('/salon/{id}/lisans-kapat', 'PanelController@salonLisansKapat');
    ...
    // Manuel odeme linki (serbest hizmet + adet + tutar -> PayTR linki)
    Route::get('/manuel-odeme-linki', 'PanelController@manuelOdemeLinki');
    Route::post('/manuel-odeme-linki', 'PanelController@manuelOdemeLinkiOlustur');
...
Route::prefix('isletmeyonetim')->group(function() {
	...
	Route::post('/dogum-gunu-atla','StoreAdminController@dogumGunuAtla')->name('isletmeadmin.dogumgunuatla');
   ...
   Route::get('/saglikbilgilerigir','StoreAdminController@saglikbilgilerigir');
	...
	Route::get('/musterihizmetkopyala/{kaynak}/{hedef}','StoreAdminController@musteri_hizmet_kopyala_salonlar_arasi');
	Route::get('/pakethizmetlerinisalonakopyala/{kaynak}/{hedef}','StoreAdminController@paket_hizmetlerini_salona_kopyala');
	Route::get('/salonhizmetlerinikopyala/{kaynak}/{hedef}','StoreAdminController@salon_sunulan_hizmetleri_kopyala');
	...
	Route::post('/harici-tahsilat-ekle','StoreAdminController@hariciTahsilatEkle');
   ...
   Route::post('/adisyonhizmetfiyatguncelle','StoreAdminController@adisyonhizmetfiyatguncelle');
	...
	Route::get('/gunsonuraporu','StoreAdminController@gun_sonu_raporu');
