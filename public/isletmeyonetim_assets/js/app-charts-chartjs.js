    ...
    var randomScalingFactor = function() {
      ...
      //Get the canvas element
		...
		function donutChart(){
			...
			var color1 ="";
		    var color3 = "";
			var yuzde = parseInt($('#gonderilensmssayisi').val())/(parseInt($('#gonderilensmssayisi').val())+parseInt($('#kalansmskullanimsayisi').val()));
			var yuzde2 = parseInt($('#gonderilenmailsayisi').val())/(parseInt($('#gonderilenmailsayisi').val())+parseInt($('#kalanmailkullanimsayisi').val()));
			if(parseFloat(yuzde)>0.5 && parseFloat(yuzde)<=0.75)
 				color1 = App.color.warning;
 			else if(parseFloat(yuzde)>0.75)
 				color1 = App.color.danger;
 			else
 				color1 = App.color.success;
			if(parseFloat(yuzde2)>0.5 && parseFloat(yuzde2)<=0.75)
 				color3 = App.color.warning;
 			else if(parseFloat(yuzde2)>0.75)
 				color3 = App.color.danger;
 			else
 				color3 = App.color.success;
			var color2 =  App.color.grey;
			var color4 = App.color.grey;
			/*var color3 = tinycolor( App.color.warning ).lighten( 22 );*/

      //Get the canvas element
			var ctx = document.getElementById("donut-chart");
			var ctx2 = document.getElementById("donut-chart2");
			var data = {
			  labels: [
			    "Gönderilen",
			    "Kalan Kullanım",
			  ...
			  datasets: [
			    {
			      data: [$('#gonderilensmssayisi').val(), $('#kalansmskullanimsayisi').val()],
			      backgroundColor: [
			        color1,
			        color2
			...
			var data2 ={
				...
				datasets:[
					{
			      data: [$('#gonderilenmailsayisi').val(), $('#kalanmailkullanimsayisi').val()],
			      backgroundColor: [
			        color3,
			        color4
