...
$('#randevusistemi').on('click','#randevuonayla',function(e){
    ...
    if($('#sifre').val()===''){
        ...
        var formData =  $('#randevuozeti').serialize()+'&ceptelefon='+$('#cep_telefon').val()+'&sifre='+$('#sifre').val();
...
$("button[name='randevuiptalet']").click(function(e){
    if(confirm('Randevuyu iptal etmek istediğinize emin misiniz? Bu işlem geri alınamaz')){
        var url= '/randevuiptalet';
    var dataval = $(this).attr('data-value');
     $.ajax({
        type: "GET",
        url: url,
        data: { randevuno : $(this).attr('data-value')},
        dataType: "text",
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        },
        success: function(result)  {
             var status =  $("span[name='guncelrandevudurum']").attr('data-value',dataval);
             $("span[name='randevudurum']").attr('style','display:none');
...
function readGorseller(input){
    ...
    if(document.getElementById('isletmeid')){
       baslangicindex = $.ajax({
            type :'GET',
             url: '/sistemyonetim/kayitlisalongorselisayisi',
            data: {isletmeid:$('#isletmeid').val()},
            ...
            if(result.teksube==1){
               ...
               document.getElementById("personelsecimbolumu").innerHTML = result.personelbolumu;
                $.ajax({
                    ...
                    data: $('#personellisteparametreler').serialize()+'&'+$('#randevuozeti').serialize()+'&sube='+result.sube+'&personeller[]='+result.personel,
                    dataType: "json",
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
                   ...
                   success: function(result)  {   
                        ...
                        $('#saatsecimtablosu').empty();
                        $('#saatsecimtablosu').append(result.tarihsaatbolumu);
...
$('#randevusistemi').on('submit','#personellisteparametreler' ,function(e){
        e.preventDefault();
        if($('select[name="sube"]').val()==0){
            alert('Devam etmek için lütfen şube seçiniz!');
...
$('#hizmetekle_personel').click(function(e){
     ...
     $.ajax({
        type: "GET",
        url: '/isletmeyonetim/personelhizmetekle/'+$('#personelid').val(),
...
$('#mevcutisletmeduzenleme_isletmeadmin').on('click','a[name="gorsel"]',function(e){
    e.preventDefault();
    if(confirm('Görseli silmek istediğinize emin misiniz? Bu işlem geri alınamaz')){
         $.ajax({
        type: "GET",
...
$('#randevutablo').on('click','.randevuiptalet',function(e){
      if(confirm('Randevuyu iptal etmek istediğinize emin misiniz? Bu işlem geri alınamaz')){
...
$('#randevutablo').on('click','.randevuonayla',function(e){
    var id = $(this).attr('data-value');
     $.ajax({
        type: "GET",
...
$('#yetkilibilgiguncelle').click(function(e){
    e.preventDefault();
     $.ajax({
        type: "GET",
...
$('a[name="randevuiptalblock"]').click(function(e){
 ...
 $('#randevutarihiyeni').change(function(e){
      e.preventDefault();
      
            // $('#saatbitis').empty();
      $.ajax({
        type: "GET",
        url: '/isletmeyonetim/calismasaatigetir',
        
        dataType: "json",
        data : {randevutarihi:$('#randevutarihiyeni').val()},
       ...
       success: function(result)  {   
           ...
           console.log(result);
           $('#saatbaslangicbolumu').empty();
           $('#saatbitisbolumu').empty();
            $('#saatbaslangicbolumu').append(result.saatbaslangic);
             $('#saatbitisbolumu').append(result.saatbitis);
           //  $('#saatbitis').append(result);
            $("#preloader").hide();
        },
        error: function (request, status, error) {
             document.getElementById('hata').innerHTML = request.responseText;
 ...
 $('input[name="adsoyad"]').change(function(e){
      e.preventDefault();

     
     var val = $("#adsoyad").val(); // get the value from #account-list
     if(val != ''){
         var cn;
  var x = document.getElementById("adsoyad2").options.length; //get option total = 4
  for (var i = 0; i < x; i++) {
    var v = document.getElementById("adsoyad2").options[i].value; //get values by index
    if (val == v) { // compare option value from #account-list value
      cn = document.getElementById("adsoyad2").options[i].className; // get the cname
    ...
    if(!cn){
       ...
       success: function(result)  {   
            ...
            $('input[name="eposta"]').val(result.eposta);
             $('input[name="ceptelefon"]').val(result.telefon);
            $("#preloader").hide();
        },
        error: function (request, status, error) {
             document.getElementById('hata').innerHTML = request.responseText;
...
$('#yenirandevuekleform').on('submit',function(e){
    e.preventDefault();
     if($('select[name="yenirandevupersonelleri[]"]') == null){
         alert('Lütfen personel seç butonuna tıklayıp randevu için personel seçiniz!');
     }
     else{
         $.ajax({
        type: "GET",
        ...
        dataType: "json",
        data : $('#yenirandevuekleform').serialize(),
...
$("#hepsinisec").change(function(){
 ...
 $('input:checkbox').not('#hepsinisec').change(function(){ 
  ...
  $('#smsbolumubody').on('click','a[name="smstaslaklari"]',function(e){
  ...
  $('#toplusmsgonder').click(function(e){
       if(telefonnumarasıvar()){
            alert('Lütfen mesaj içeriğinizde telefon numarası kullanmayınız!');
       }
       else if($('#smsmesaj').val()!='' && $('input:checkbox[name="musteriler[]"]:checked').length>0){
        ...
        alert('Lütfen listeden seçim yapın ve mesajınızı yazın!');
...
hasphonenumber = true;
  ...
  $('#smstaslakolarakkaydet').click(function(e){
        ...
        dataType: "json",
        data : {taslak_icerik:$('#smsmesaj').val()},
       ...
       success: function(result)  {  
            ...
            if(result.liste!=''){
               ...
               $('#taslaklarbolumu2').append(result.liste);
                $('html, body').animate({ scrollTop: $("#taslaklarbolumu2").offset().top }, 'slow');
            }
  ...
  $('#yenipersonelbilgiekle').on('submit',function(e){
    ...
    var profilresim=  $('#profilresmi_yeni').get(0).files[0];
     
    var formData = new FormData();
    formData.append('profilresmi_yeni',profilresim);
     var other_data = $('#yenipersonelbilgiekle').serializeArray();
    ...
    $.each(other_data,function(key,input){
     ...
     var eklenebilir = 0;
     var uyari = "";
     if($('#eposta_yeni').val()!= '' || $('#ceptelefon_yeni').val()!=''){
         if($('#sifre_yeni').val()=='' && $('#sifre_yeni_tekrar').val()==''){
             uyari = "Personelinizden sistem kullanıcısı oluşturmak için lütfen şifreleri eksiksiz giriniz";

         }
         else if($('#sifre_yeni').val()!=   $('#sifre_yeni_tekrar').val()){
            uyari ="Personelinizden sistem kullanıcısı oluşturmak için girdiğiniz şifreler uyuşmamaktadır";
         }
         else
            eklenebilir =1;
     }
         ...
         if($('#sifre_yeni').val()!='' || $('#sifre_yeni_tekrar').val()!=''){
             uyari = "Personelinizden sistem kullanıcısı oluşturmak için lütfen eposta veya cep telefonu ve şifreleri eksiksiz giriniz";
             eklenebilir = 0;
         }
         else{
     ...
     if(eklenebilir==1){
             $.ajax({
        type: "POST",
        url: '/isletmeyonetim/yenipersonelbilgiekle',
  ...
  $('#yenimusterilistesiekle').on('submit',function(e){
        
    e.preventDefault();
    var liste=  $('#listedosyasi_yeni_musteri').get(0).files[0];
...
$('#bir_hizmet_daha_ekle').click(function (e) {
    e.preventDefault();
    $('.hizmetler_bolumu').find(".custom-select2").each(function(index){
    ...
    $('.hizmetler_bolumu').append('<div class="row" data-value="'+index +'">'+$('.hizmetler_bolumu .row').last().html()+'</div>');
...
$('.hizmetler_bolumu').on('click','button[name="hizmet_formdan_sil"]',function(e){
    e.preventDefault();
...
$('#urun_formu').on('submit',function(e){
    e.preventDefault();
    $.ajax({
        type: "POST",
...
$('#urun_liste').on('click','a[name="urun_duzenle"]',function(e){
    ...
    $('#urun_adi').val(tds[0].innerHTML);
    $('#stok_adedi').val( tds[1].innerHTML); 
    $('#fiyat').val(tds[2].innerHTML);
    $('#barkod').val(tds[3].innerHTML);
    $('#urun_id').val($(this).attr('data-value'));
    $('#urun_modal_baslik').empty();
    $('#urun_modal_baslik').append("Ürün Güncelleme");
});

$('.hizmetler_bolumu').on('change','select[name="randevuhizmetleriyeni"]',function(e){
    e.preventDefault();
    var hizmetid = $(this).val();
    var fiyat_text = $(this).closest('div .row').find('input[name="hizmetfiyatiyeni"]');

    var sure_text = $(this).closest('div .row').find('input[name="hizmetsuresiyeni"]');
    $.ajax({
        type: "GET",
