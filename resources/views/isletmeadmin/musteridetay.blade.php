...
@if(!empty($isletme->studyo_modu))
{{-- ===================== Otomatik Ders Plani (studyo modu) ===================== --}}
<style>
...
async function dersPlaniModalAc(){
   ...
   const res = await dpFetch('/isletmeyonetim/ders-tekrarli-kaynaklar', { method:'POST', body:{ musteri_id:DP_MUSTERI } });
   if(!(res.ok && res.data && res.data.durum==='ok')){
      $('#dp_kaynak').html('<option>Yüklenemedi</option>'); return;
   }
   ...
   $('#dp_kaynak').html(dpKaynaklar.map(function(k,i){
      return '<option value="'+i+'">'+k.etiket+' • '+k.kalan_seans+' seans</option>';
   }).join(''));
   dpRenderSlots();
}
...
function dpUygunSablon(){
   var k = dpKaynak(); if(!k) return [];
   return dpSablonlar.filter(function(s){ return String(s.hizmet_id) === String(k.hizmet_id); });
}

function dpRenderSlots(){
   dpSecili = {}; // kaynak degisince secim sifirla
   ...
   // Egitmen filtresi
   var egit = {}; slot.forEach(function(s){ if(s.personel_id) egit[s.personel_id]=s.personel; });
   var egitArr = Object.keys(egit);
   var eh = '';
   if(egitArr.length>1){
...
function dpFiltreEgit(pid){ dpFiltreEgitmen = pid; dpAktifGun = null; dpRenderGunVeSlot(); dpRenderSlots2Egit(); }
function dpRenderSlots2Egit(){ // egitmen chip on/off yenile
...
function dpFiltreli(){
   return dpUygunSablon().filter(function(s){ return dpFiltreEgitmen===null || String(s.personel_id)===String(dpFiltreEgitmen); });
}
...
function dpRenderGunVeSlot(){
   ...
   $('#dp_gun_sekme').html(gunler.map(function(g){
      return '<span class="dp-chip '+(dpAktifGun===g?'on':'')+'" onclick="dpGunSec('+g+')">'+DP_GUNAD[g]+'</span>';
   }).join(''));
   ...
   $('#dp_slot_liste').html(gsl.map(function(s){
      ...
      return '<div class="dp-slot '+(on?'on':'')+'" onclick="dpSlotTikla('+s.id+')">'+
             '<div class="dp-rad"></div>'+
...
function dpOzetGuncelle(){
   var n = Object.keys(dpSecili).length;
   $('#dp_secim_ozet').text(n? (n+' slot seçildi') : '');
}
...
async function dersPlaniKaydet(){
   ...
   var res = await dpFetch('/isletmeyonetim/ders-tekrarli-kaydet', { method:'POST', body:{
      musteri_id: DP_MUSTERI,
      hizmet_id: k.hizmet_id,
      toplam_seans: k.kalan_seans,
      sablonlar: slotlar,
      adisyon_paket_id: k.adisyon_paket_id,
      adisyon_hizmet_id: k.adisyon_hizmet_id,
   ...
   if(res.ok && res.data && res.data.durum==='ok'){
      ...
      swal('Planlandı', (s.olusan||0)+' ders otomatik oluşturuldu'+((s.yerlesmeyen>0)?(' • '+s.yerlesmeyen+' seans yerleştirilemedi'):'')+'.', 'success');
   } else {
      swal('Hata',(res.data && res.data.mesaj)||'Kaydedilemedi.','error');
   }
}
