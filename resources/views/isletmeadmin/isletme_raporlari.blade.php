...
:root{
  ...
  --rep-accent:#6e4bff; --rep-green:#1fbf6f; --rep-orange:#ff8a3d;
  --rep-pink:#ff5c8a; --rep-blue:#1e9fff; --rep-violet:#a16bff; --rep-cyan:#22c4cb; --rep-yellow:#f7c948;
...
.rep-pchip-date{background:#fff3e3;border:1px solid #ffd9ad;border-radius:14px;padding:12px 16px;display:flex;align-items:center;gap:10px;color:#ff8a3d;font-weight:600;font-size:13px;}
.rep-pchip-date input{border:0;background:transparent;color:#ff8a3d;font-weight:600;font-size:13px;outline:none;width:106px;}
.rep-pchip-date .sep{opacity:.55;}
.rep-toplam-gelir{background:#defaeb;border:1px solid #bff0d6;border-radius:14px;padding:14px 18px;display:flex;align-items:center;gap:10px;font-weight:700;color:#1fbf6f;min-width:200px;justify-content:flex-end;}
...
.rep-stat.bg-1{background:#eaf4ff;border-color:#cfe7ff;}.rep-stat.bg-1 .ico{background:#cfe7ff;color:#1e9fff;}
.rep-stat.bg-2{background:#defaeb;border-color:#bff0d6;}.rep-stat.bg-2 .ico{background:#bff0d6;color:#1fbf6f;}
.rep-stat.bg-3{background:#ffe3eb;border-color:#ffc9d7;}.rep-stat.bg-3 .ico{background:#ffc9d7;color:#ff5c8a;}
.rep-stat.bg-4{background:#fff3e3;border-color:#ffd9ad;}.rep-stat.bg-4 .ico{background:#ffd9ad;color:#ff8a3d;}
.rep-stat.bg-5{background:#efe9ff;border-color:#dccfff;}.rep-stat.bg-5 .ico{background:#dccfff;color:#6e4bff;}
.rep-stat.bg-6{background:#deffff;border-color:#bff4f6;}.rep-stat.bg-6 .ico{background:#bff4f6;color:#22c4cb;}
...
.rep-card::before{content:'';position:absolute;left:0;top:0;width:100%;height:4px;background:linear-gradient(90deg,var(--c-from,#6e4bff),var(--c-to,#a16bff));}
.rep-card.tone-blue{--c-from:#1e9fff;--c-to:#22c4cb;}
.rep-card.tone-green{--c-from:#1fbf6f;--c-to:#46d68b;}
.rep-card.tone-pink{--c-from:#ff5c8a;--c-to:#ff8aae;}
.rep-card.tone-orange{--c-from:#ff8a3d;--c-to:#ff5c8a;}
.rep-card.tone-violet{--c-from:#6e4bff;--c-to:#a16bff;}
...
.rep-card-title .ico{width:32px;height:32px;border-radius:10px;display:flex;align-items:center;justify-content:center;background:linear-gradient(135deg,var(--c-from,#6e4bff),var(--c-to,#a16bff));color:#fff;font-size:15px;}
...
.rep-detay-btn{display:flex;align-items:center;gap:6px;margin:18px auto 0 auto;background:linear-gradient(135deg,#efe9ff,#dccfff);border:0;border-radius:24px;padding:10px 22px;color:#6e4bff;font-weight:700;font-size:13px;cursor:pointer;transition:transform .18s ease;text-decoration:none;}
...
.cinsiyet-row.kadin{background:#ffe3eb;color:#ff5c8a;}
.cinsiyet-row.erkek{background:#e3f2ff;color:#1e9fff;}
...
.kaynak-chip.k1{background:#e3f2ff;color:#1e9fff;}
.kaynak-chip.k2{background:#defaeb;color:#1fbf6f;}
.kaynak-chip.k3{background:#efe9ff;color:#6e4bff;}
</style>
      ...
      charts.gelir = new Chart(ctx, {
        ...
        data:{labels:['Nakit','Kredi/Banka Kartı','Havale/EFT','Diğer'],
              datasets:[{data:[d.nakit||0,d.kart||0,d.havale||0,d.diger||0],
                         backgroundColor:['#1e9fff','#1fbf6f','#ff5c8a','#9097ad'],borderWidth:0,hoverOffset:6}]},
      ...
      charts.urun = new Chart(ctx, {
        ...
        data:{labels:top10.map(function(u){return u.urun_adi;}),
              datasets:[{label:'Adet',data:top10.map(function(u){return u.adet;}),
                         backgroundColor:'rgba(255,92,138,.85)',borderRadius:8,maxBarThickness:24}]},
        options:{indexAxis:'y',responsive:true,maintainAspectRatio:false,
          plugins:{legend:{display:false},tooltip:{callbacks:{
            label:function(c){return c.parsed.x+' adet';},
            afterLabel:function(c){return 'Ciro: '+trMoney(top10[c.dataIndex].ciro)+' ₺';}
  ...
  function renderPersonel(){
    ...
    api('/personel', currentPeriodQS()).then(function(d){
      ...
      charts.personel = new Chart(ctx, {
        type:'bar',
        data:{labels:labels,datasets:[{label:'Ciro (₺)',data:ciros,backgroundColor:'rgba(110,75,255,.85)',borderRadius:8,maxBarThickness:36}]},
      ...
      charts.saat = new Chart(ctx1, {
        ...
        data:{labels:saatFiltered.map(function(s){return s.saat+':00';}),
              datasets:[{data:saatFiltered.map(function(s){return s.adet;}),
                         borderColor:'#6e4bff',backgroundColor:'rgba(110,75,255,.15)',
                         fill:true,tension:.35,pointRadius:3,pointBackgroundColor:'#6e4bff'}]},
        options:{responsive:true,maintainAspectRatio:false,
      ...
      charts.gun = new Chart(ctx2, {
        ...
        data:{labels:gunData.map(function(g){return g.gun;}),
              datasets:[{data:gunData.map(function(g){return g.adet;}),
                         backgroundColor:['#1e9fff','#1fbf6f','#a16bff','#ff8a3d','#ff5c8a','#22c4cb','#f7c948'],
