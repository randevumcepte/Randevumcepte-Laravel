...

{
...

    {
        ...
        $personel_idler = Personeller::where("salon_id", $isletme_id)->pluck(
        ...
        $randevu_hizmetler = DB::table("randevu_hizmetler")
            ...
            ->select(
                ...
                DB::raw(
                    ...
                    'CONCAT(COALESCE(randevular.durum,"na"),"-",COALESCE(randevular.randevuya_geldi,"na")) as durum'
                ...
                DB::raw(
                    ...
                    'CASE WHEN randevular.user_id=2012 then "#000000" WHEN randevular.randevuya_geldi=0 then "0xFFFF0000" WHEN randevular.randevuya_geldi=1 THEN "0xFF008000" ELSE REPLACE(renk_duzenleri.renk,"#","0xFF") END as bgcolor'
        ...
        $resources = DB::table("salon_personelleri")
            ...
            ->select(
                ...
                "salon_personelleri.personel_adi as name",

                DB::raw('REPLACE(renk_duzenleri.renk,"#","0xFF") as bgcolor'),

                "isletmeyetkilileri.profil_resim as avatar"
        ...
        return [
            ...
            "randevular_liste" => Randevular::where(
   ...
   DB::raw('CASE WHEN on_gorusmeler.paket_id IS NOT NULL THEN paketler.paket_adi ELSE urunler.urun_adi END as ilgili '),
...

    {
        ...
        // If customer ID is provided (not empty), filter by customer

        if ($request->musteri_id != "") {

            $adisyonlar = Adisyonlar::where("salon_id", $salonid)
                ...
                ->whereIn(
                    ...
                    User::where("name", "like", "%" . $request->arama . "%")
        ...
        $paketler = AdisyonPaketler::whereIn(
        ...
        foreach ($paketler->items() as $paket) {
            ...
            if ($adisyon) {
                ...
                array_push($result, [
                    ...
                    "seanslar" => AdisyonPaketSeanslar::where(
            ...
            if ($cakisma_varmi == "" || $request->cakisanrandevuekle == "1") {
                ...
                foreach ($randevu_tarihleri as $tarihler) {
                    ...
                    foreach ($request->hizmetler as $key2 => $value) {
                        ...
                        $yenirandevuhizmetpersonel->randevu_id =

                            $yenirandevu->id;

                        $yenirandevuhizmetpersonel->hizmet_id =
                        ...
                        if ($key2 == 0) {
                            ...
                            $yenirandevuhizmetpersonel->saat =

                                $request->randevu_saati;

                            $yenirandevuhizmetpersonel->saat_bitis = date(
                        ...
                        foreach (
                            ...
                            $request->yardimcipersoneller

                            as $_yardimcipersonel

                        ) {

                            if (

                                $yardimcipersonel["randevuhizmetid"] =
                                ...
                                $yardimci_personel->randevu_id =

                                    $yenirandevu->id;

                                $yardimci_personel->hizmet_id =

                                    $yenirandevuhizmetpersonel->hizmet_id;

                                $yardimci_personel->cihaz_id =

                                    $yenirandevuhizmetpersonel->cihaz_id;

                                $yardimci_personel->personel_id =

                                    $_yardimcipersonel["yardimcipersonel"][

                                        "id"

                                    ];

                                $yardimci_personel->oda_id =

                                    $yenirandevuhizmetpersonel->oda_id;

                                $yardimci_personel->sure_dk =

                                    $yenirandevuhizmetpersonel->sure_dk;

                                $yardimci_personel->fiyat =

                                    $yenirandevuhizmetpersonel->fiyat;

                                $yardimci_personel->saat =

                                    $yenirandevuhizmetpersonel->saat;

                                $yardimci_personel->saat_bitis =

                                    $yenirandevuhizmetpersonel->saat_bitis;
                    ...
                    $isletme = Salonlar::where(

                        "id",

                        $yenirandevu->salon_id

                    )->first();

                    $musteribilgi = User::where(
                    ...
                    if (
                        ...
                        SalonSMSAyarlari::where("ayar_id", 12)
                            ...
                            ->where("salon_id", $yenirandevu->salon_id)
                        ...
                        array_push($mesajlar, [
                            ...
                            "message" =>
                                ...
                                ". Randevunuza 15 dk önce gelmenizi rica ederiz. Detaylı bilgi için bize ulaşın. 0" .
                    ...
                    foreach ($yenirandevu->hizmetler as $hizmet) {
                        ...
                        if (
                            ...
                            $mesaj =
                                ...
                                $yenirandevu->users->name .

                                " isimli müşterinin " .

                                date("d.m.Y", strtotime($yenirandevu->tarih)) .
...

    {
        ...
        if ($request->on_gorusme_id != "") {
            ...
            $ongorusme = OnGorusmeler::where(

                "id",

                $request->on_gorusme_id

            )->first();

            $eskirandevu = Randevular::where(
        ...
        if ($request->musteri_id != 0) {
            ...
            if (
                ...
                MusteriPortfoy::where("user_id", $user->id)
        ...
        if ($request->cakisanrandevuekle == "") {
            ...
            $cakisma_varmi = self::cakisan_randevu_kontrol($request, [
...

    {
        ...
        if (
            ...
            Adisyonlar::where("user_id", $request->musteri_id)
        ...
        if (isset($request->adisyon_paket_id)) {
            ...
            foreach ($request->adisyon_paket_id as $key3 => $paket_id) {
                ...
                $paket_tahsilat_tutar =
                    ...
                    $request->adison_paketleri[$key3]["fiyat"];
...

    {
        ...
        if($hizmet && $request->hizmet != ""){
            ...
            return self::enyakinuygunrandevubul('',$hizmet,30);
    ...
    function enyakinuygunrandevubul($randevuid, $salonHizmet, $maxDaysToCheck = 30)  

    {
        ...
        Log::info("Başlangıç tarih saat : ".$startDate);
        ...
        for ($day = 0; $day < $maxDaysToCheck; $day++) {
            ...
            foreach (range($salonStart->timestamp, $salonEnd->timestamp, 15 * 60) as $timestamp) {
                if(Salonlar::where('id',$salon_id)->value('randevu_takvim_turu')!=2)
                {
                    foreach ($personeller as $personel) {
                        /*if ($salonHizmet != "")  
                            if (!PersonelHizmetler::where('personel_id', $personel->id)->where('hizmet_id', $salonHizmet->hizmet_id)->exists() ) {
                                continue;
                        }*/
                        ...
                        if ($this->hasAppointmentConflict($personel->id, $startSlot, $endSlot, $salon_id)) {
                         ...
                         return response()->json([
                            ...
                            'metin' => $randevuid !="" ?  base64_encode(self::convertToBugunYarin($randevu->tarih)." saat ".$randevu->saat ." randevunuzu " . self::convertToBugunYarin($startSlot->format('Y-m-d')) . " saat " . $startSlot->format('H:i') . " olarak güncelleyebiliriz. Randevunuzu güncellemek için biri, operatöre bağlanmak için ikiyi tuşlayınız")  : base64_encode($salonHizmet->hizmetler->hizmet_adi. " için " . self::convertToBugunYarin($startSlot->format('Y-m-d')) . " ". $startSlot->format('H:i') . " saatinde uygun randevu bulunmaktadır. Randevunuzu  oluşturmak için biri, operatöre bağlanmak için ikiyi tuşlayınız"),
...

{
     ...
     public function drKlinikSatisHizmetEkle(Request $request)
    {
            ...
            $paket_var = Paketler::where('paket_adi','LIKE','%'.$request->hizmetAdi.'%')->where('salon_id',$reques->salonId)->first();
            if($paket_var)
            {
                $adisyon_paket = new AdisyonPaketler();
    ...
    public function drKlinikTahsilatEkle(Request $request){
        ...
        $tahsilat->odeme_yontemi_id = OdemeYontemleri::where('odeme_yontemi',$request->odemeYontemi)->value('id');
        ...
        foreach($request->adisyon_urun_id as $key2=>$urun_id)
         {  
                    ...
                    $odeme->adisyon_urun_id = $urun_id;
                    $odeme->tahsilat_id = $tahsilat->id;
                    $urun_tahsilat_tutar = AdisyonHizmetler::where('id',$urun_id)->where('adisyon_id',$request->adisyonId)->value('fiyat');
                    $toplamAdisyonTutari = AdisyonHizmetler::where('adisyon_id',$request->adisyonId)->sum('fiyat')+AdisyonUrunler::where('adisyon_id',$request->adisyonId)->sum('fiyat')+AdisyonPaketler::where('adisyon_id',$request->adisyonId)->sum('fiyat');
    ...
    public function salonAppyMusteriKontrol(Request $request)
    {
        $user = User::where('name',$request->musteriAdi)->where('cep_telefon',self::telefon_no_format_duzenle($request->telefon))->first();
        if(!$user)
        {
            $user = new User();
        }
        ...
        $user->dogum_tarihi = date('Y-m-d',strtotime($request->dogumTarihi));
