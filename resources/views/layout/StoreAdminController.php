...
<input type="hidden" name="adisyon_urun_senet_id[]" value="'.$urun->senet_id.'">
    ...
    public function yeniisletmeekle(Request $request)
    {
        ...
        if(isset($request->yonetici_ekle))
        {
            ...
            $personel->yetkili_id = $yetkili->id;
            $personel->save();
            self::sms_gonder($request,array(array("to"=>$yetkili->gsm1,"message"=>"RandevumCepte hesabınız oluşturulmuş olup ".$olusturulansifre." şifrenizle giriş yapabilirsiniz.")),false,1,false);
        }
        ...
        DB::insert('insert into model_has_roles (role_id, model_type,model_id,salon_id) values (1, "App\\\IsletmeYetkilileri",'.Auth::user()->id.','.$isletme->id.')');
        $_personel = new Personeller();
        $_personel->personel_adi = Auth::user()->name;
        $_personel->cep_telefon = Auth::user()->gsm1;
        ...
        $_personel->role_id = 1;
        $_personel->yetkili_id = Auth::user()->id;
        $_personel->save();
        return array(
    ...
    public function ongorusmesatisyapildi(Request $request)
    {
        ...
        if($ongorusme->user_id == null)
        {
            if(User::where('cep_telefon',$ongorusme->cep_telefon)->count() == 0){
                ...
                $user->save();
                self::sms_gonder($request,array(array("to"=>$user->cep_telefon,"message"=>"RandevumCepte hesabınız oluşturulmuş olup ".$olusturulansifre." şifrenizle giriş yapabilirsiniz.")),false,1,false);
            }
            else
