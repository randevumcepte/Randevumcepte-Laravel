...
$(document).on('click','a[name="tahsil_et"]',function(e){
 ...
 function randevuyaGeldiIsaretle(randevu_id, hizmetid, dogrulamaKodu, kvkkKodu, isKvkkProcess = false, seansDusuldu = false, seansFormData = null) {
        ...
        success: function(result) {
            ...
            if (result.geldiIsaretlendi) {
                ...
                if (result.onayGerekli) {
                    kvkkGerekli(randevu_id, hizmetid, result, dogrulamaKodu);
                } else if (result.dogrulamaGerekli) {
                    dogrulama_islemleri(randevu_id, hizmetid, dogrulamaKodu);
                } else if(result.seansDusumuBildir) {
                    seansDusumu(randevu_id, hizmetid, result, dogrulamaKodu, kvkkKodu);   
                }
            }
        },
        error: function(request, status, error) {
...
function dogrulamaGonder(randevuid)
{
    $.ajax({
                                type: "POST",
                               ...
                               success: function(result)  {
                                 $("#preloader").hide();
                                 dogrulama_islemleri(randevu_id,hizmetid);
                                },
                                error: function (request, status, error) {
                                    $("#preloader").hide();
                                    document.getElementById('hata').innerHTML = request.responseText;
...
$(document).on('click','a[name="geldi_isaretle"]',function(e){
    ...
    swal({
        ...
        if(result.value){
            ...
            {
                dogrulamaGonder(randevu_id);
            }
            else
            {
                randevuyaGeldiIsaretle(randevu_id,hizmetid,'','');
                
            }
        }
    });
});
function dogrulama_islemleri(randevu_id,hizmetid)
{
                        ...
                        confirmButtonClass: 'btn btn-success',
                            ...
                            if(result.value){
                                randevuyaGeldiIsaretle(randevu_id,hizmetid,result.value,'');
                               
                            }
                        })
}
function kvkkGerekli(randevu_id, hizmetid, result1, dogrulamaKodu) {
    swal({
        type: result1.status,
        title: result1.title,
        ...
        if (result.value) {
            ...
            randevuyaGeldiIsaretle(randevu_id, hizmetid, dogrulamaKodu, result.value, true);
        }
    });
}
function seansDusumu(randevu_id, hizmetid, result1, dogrulamaKodu,kvkkKodu) {
    swal({
        ...
        cancelButtonClass: 'btn btn-danger',
            ...
            if(result.value)
            {
                ...
                randevuyaGeldiIsaretle(randevu_id, hizmetid, dogrulamaKodu, kvkkKodu, true, true,  formData);
