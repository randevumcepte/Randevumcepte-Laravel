              ...
              <div class="page-header">
                  ...
                  <div class="row">
                     ...
                     <div class="col-md-6  col-sm-6 col-xs-6 col-6 text-right">
                      ...
                      <select class="custom-select2"  name="paket_satis_musteri_id" >
                  <option value="0">@if($isletme->salon_turu_id==15) Danışan @else Müşteri @endif Seçiniz...</option>
                              @foreach(\App\MusteriPortfoy::where('salon_id',$isletme->id)->where(

                              function($q){$q->where('kara_liste','!=',true);$q->orWhere('kara_liste',null);})->get() as $musteri)
                                <option value="{{$musteri->user_id}}">{{$musteri->users->name}} ({{$musteri->users->cep_telefon}})
                                </option>
                              @endforeach
                           </select>
                        ...
                        <button data-toggle="modal" data-target="#paket-modal" type="button" class="btn btn-success yenieklebuton501">
