...
class StoreAdminController extends Controller
{
    ...
    public function mevcutsube($request)
    {
        ...
        { 
            $subeler = Auth::user()->yetkili_olunan_isletmeler->pluck('salon_id')->toArray();
            $sube = $subeler[0];
        }
        return $sube;
    }
    public function sube_yetki_kontrol_et($request)
    {
        ...
        if(YetkiliOlunanSubeler::where('yetkili_id',Auth::user()->id)->where('salon_id',$mevcutsube)->first()===null){
            return false;
            exit;
        }
        return true;
    ...
    {
        $isletmeler = Auth::user()->yetkili_olunan_isletmeler->pluck('salon_id')->toArray();  
    ...
    public function index(Request $request)
    {
         ...
         /*if(!self::sube_yetki_kontrol_et($request))
         {
            return view('isletmeadmin.yetkisizerisim');
            exit;
         }*/
        ...
        $randevusayisi = Randevular::where('salon_id',self::mevcutsube($request))->where('user_id','!=',2012)->where('tarih',date('Y-m-d'))->count();
        ...
        $gunluk_urun_satislari = self::adisyon_yukle($request,3,0,date('Y-m-d 00:00:00'),date('Y-m-d 23:59:59'),'','');
        $gunluk_paket_satislari = self::adisyon_yukle($request,2,0,date('Y-m-d 00:00:00'),date('Y-m-d 23:59:59'),'','');
        
           $yaklasan_dogumgunleri = DB::table('musteri_portfoy')->join('users','musteri_portfoy.user_id','=','users.id')->select('users.name as ad_soyad','users.cep_telefon as telefon', 'users.dogum_tarihi as dogum_tarihi')->whereDay('dogum_tarihi', '>=',date('d'))->whereDay('dogum_tarihi','<=',date('d',strtotime('+5 days',strtotime(date('Y-m-d')))))->whereMonth('dogum_tarihi',date('m'))->where('musteri_portfoy.salon_id',self::mevcutsube($request))->get();
           $alacak_hatirlatmalari = DB::table('alacaklar')->join('users','alacaklar.user_id','=','users.id')->select('users.name as musteri','alacaklar.planlanan_odeme_tarihi as planlanan_odeme_tarihi',DB::raw('FORMAT(alacaklar.tutar,2,"tr_TR") as tutar'))->where('alacaklar.salon_id',self::mevcutsube($request))->where('planlanan_odeme_tarihi','=',date('Y-m-d'))->get();
           $etkinlikler = DB::table('etkinlikler')->join('etkinlik_katilimcilari','etkinlikler.id','=','etkinlik_katilimcilari.etkinlik_id')->select('etkinlikler.etkinlik_adi as etkinlik_adi',
          ...
          $on_gorusmeler = self::ongorusmegetir($request,true);
          ...
          return view('isletmeadmin.dashboard',[
            ...
            'yaklasan_dogumgunleri'=>$yaklasan_dogumgunleri,
    ...
    public function randevular(Request $request){ 
        ...
        return view('isletmeadmin.randevular',['bildirimler'=>self::bildirimgetir($request), 'paketler'=>$paketler,'sayfa_baslik'=>'Randevu Takvimi','pageindex' => 2,'randevular'=>$randevular,'isletme'=>$isletme,'kalan_uyelik_suresi'=>$kalan_uyelik_suresi]);
    }
    ...
    public function randevugetir(Request $request){
        ...
        if($randevu->durum===1)
            ...
            $buttons = '<div class="row">
                                <div class="col-3 col-xs-3 col-sm-3"><a data-toggle="modal" data-target="#randevu-duzenle-modal" name="randevu_duzenle" href="#" class="btn btn-primary btn-block btn-lg" data-value="'.$randevu->id.'"> Düzenle</a></div>
    ...
    public function avantajraporlar(){
        ...
        return view ('isletmeadmin.kampanyaraporlar',['title' => 'Avantaj Raporları |Avantajbu.com','pageindex' => 1052,'isletme' => $isletme,'avantajlar' => $avantajlar]);
    }
    ...
    public function smslistesi(){
        ...
        $listeler = SMSListeleri::where('user_id',Auth::user()->id)->get();
         $isletme = Salonlar::where('id',Auth::user()->salon_id)->first();
        return view('isletmeadmin.smslistesi',['title' => 'Toplu SMS Listelerim | Avantajbu.com','pageindex' => 107,'isletme'=>$isletme,'listeler'=> $listeler, 'kalan_uyelik_suresi' => self::lisans_sure_kontrol($request)]);
    }
    public function toplumailgonder(){
        ...
        $isletme = Salonlar::where('id',Auth::user()->salon_id)->first();
        return view('isletmeadmin.toplumailgonder',['title' => 'Toplu Mail Gönder | Avantajbu.com','pageindex' => 110,'isletme'=>$isletme, 'kalan_uyelik_suresi' => self::lisans_sure_kontrol($request)]);
    }
    public function ayarlar(Request $request){
        ...
        $isletme = Salonlar::where('id',self::mevcutsube($request))->first();
        $subeler = Subeler::where('salon_id',self::mevcutsube($request))->get();
    ...
    public function randevuyukle(Request $request,$takvim_turu){
        $randevu_hizmetler = "";
        $resources = ""; 
        $personel_idler = Auth::user()->yetkili_olunan_isletmeler->where('salon_id',self::mevcutsube($request))->pluck('id');
        if($takvim_turu == 1 || Auth::user()->hasRole('Personel')){
            ...
            ->select('randevular.id as randevu_id','randevular.user_id as userid','randevu_hizmetler.id as id',DB::raw('CASE when randevular.user_id = 2012 THEN "Kapalı Saat" ELSE CONCAT(users.name,"\n",hizmetler.hizmet_adi,"\n","Cihaz : ",COALESCE(cihazlar.cihaz_adi,""),"\n","Oda : ",COALESCE(odalar.oda_adi,"")) END as title'),DB::raw('CONCAT("#ffffff") as borderColor'),
                   DB::raw('TRIM(CONCAT("<table style=\"width:100%;margin:0 0 10px 0\"><tr><td>Telefon</td><td>:</td><td>",COALESCE(users.cep_telefon,""),"</td></tr>
                    <tr><td>Hizmet</td><td>:</td><td>",COALESCE(hizmetler.hizmet_adi,""),"</td></tr>
                    <tr><td>Personel</td><td>:</td><td>",COALESCE(salon_personelleri.personel_adi,""),"</td></tr>
                   ...
                   DB::raw(' CASE WHEN randevular.durum=1 AND randevular.randevuya_geldi IS NOT TRUE THEN 
                    ...
                    TRIM(CONCAT("<div class=\"row\">
                                <div class=\"col-3 col-xs-3 col-sm-3\"><a data-toggle=\"modal\" data-target=\"#randevu-duzenle-modal\" name=\"randevu_duzenle\" href=\"#\" class=\"btn btn-primary btn-block btn-lg\" data-value=\"",randevular.id,"\"> Düzenle</a></div>
                                ...
                                <div class=\"col-3 col-xs-3 col-sm-3\"><a name=\"geldi_isaretle\" href=\"#\" class=\"btn btn-success btn-block btn-lg\" data-value=\"",randevular.id,"\"> Geldi & Tahsilat</a></div>
                ...
                DB::raw('CASE WHEN randevular.user_id=2012 then "#000000" WHEN randevular.durum=0 then "#ff0000" ELSE renk_duzenleri.renk END as color'),'randevu_hizmetler.personel_id as resourceId')->where('randevular.salon_id',self::mevcutsube($request))->where('randevular.durum','<',2)->where(function($q) use($personel_idler){ if(Auth::user()->hasRole('Personel')) $q->whereIn('randevu_hizmetler.personel_id',$personel_idler); })->get();
        ...
        if($takvim_turu == 0 && !Auth::user()->hasRole('Personel')){ 
            $randevu_hizmetler = DB::table('randevu_hizmetler')->
            join('randevular','randevu_hizmetler.randevu_id','=','randevular.id')->
            join('users','randevular.user_id','=','users.id')->
            join('hizmetler','randevu_hizmetler.hizmet_id','=','hizmetler.id')->
            ...
            ->leftjoin('isletmeyetkilileri','randevular.olusturan_personel_id','=','isletmeyetkilileri.id')
             ->leftjoin('cihazlar','randevu_hizmetler.cihaz_id','=','cihazlar.id')
            ->leftjoin('odalar','randevu_hizmetler.oda_id','=','odalar.id')
            ->select('randevular.id as randevu_id','randevular.user_id as userid','randevu_hizmetler.id as id',DB::raw('CASE when randevular.user_id = 0 THEN "Kapalı Saat" ELSE CONCAT(users.name,"\n",hizmetler.hizmet_adi,"\n","Personel : ",COALESCE(salon_personelleri.personel_adi,""),"\n","Cihaz : ",COALESCE(cihazlar.cihaz_adi,""),"\n","Oda : ",COALESCE(odalar.oda_adi,"")) END as title'),DB::raw('CONCAT("#ffffff") as borderColor'),
                   DB::raw('TRIM(CONCAT("<table><tr><td>Telefon</td><td>:</td><td>",COALESCE(users.cep_telefon,""),"</td></tr>
                    <tr><td>Hizmet</td><td>:</td><td>",COALESCE(hizmetler.hizmet_adi,""),"</td></tr>
                    <tr><td>Personel</td><td>:</td><td>",COALESCE(salon_personelleri.personel_adi,""),"</td></tr>
                    <tr><td>Cihaz</td><td>:</td><td>",COALESCE(cihazlar.cihaz_adi,""),"</td></tr>
                    <tr><td>Oda</td><td>:</td><td>",COALESCE(odalar.oda_adi,""),"</td></tr>
        ...
        if($takvim_turu == 2 && !Auth::user()->hasRole('Personel')){
            $randevu_hizmetler = DB::table('randevu_hizmetler')->
            join('randevular','randevu_hizmetler.randevu_id','=','randevular.id')->
            join('users','randevular.user_id','=','users.id')->
            join('hizmetler','randevu_hizmetler.hizmet_id','=','hizmetler.id')->
            ...
            ->select(
                ...
                'randevu_hizmetler.id as id',
                 DB::raw('CASE when randevular.user_id = 0 THEN "Kapalı Saat" ELSE CONCAT(users.name,"\n",hizmetler.hizmet_adi,"\n","Personel : ",COALESCE(salon_personelleri.personel_adi,""),"\n","Oda : ",COALESCE(odalar.oda_adi,"")) END as title'),DB::raw('CONCAT("#ffffff") as borderColor'),
                 DB::raw('TRIM(CONCAT("<table><tr><td>Telefon</td><td>:</td><td>",COALESCE(users.cep_telefon,""),"</td></tr>
                    <tr><td>Hizmet</td><td>:</td><td>",COALESCE(hizmetler.hizmet_adi,""),"</td></tr>
                    <tr><td>Personel</td><td>:</td><td>",COALESCE(salon_personelleri.personel_adi,""),"</td></tr>
                    <tr><td>Cihaz</td><td>:</td><td>",COALESCE(cihazlar.cihaz_adi,""),"</td></tr>
                ...
                DB::raw('CONCAT(randevular.tarih," ", CASE WHEN randevu_hizmetler.saat is NULL THEN randevular.saat ELSE randevu_hizmetler.saat END) AS start'),
                DB::raw('CONCAT(randevular.tarih," ", CASE WHEN randevu_hizmetler.saat_bitis is NULL THEN randevular.saat_bitis ELSE  randevu_hizmetler.saat_bitis END) AS end'),
                DB::raw('CASE WHEN randevular.user_id=2012 then "#000000" WHEN randevular.durum=0 then "#ff0000" ELSE renk_duzenleri.renk END as color'),
                'cihazlar.id as resourceId'
            )->where('randevular.salon_id',self::mevcutsube($request))->where('randevular.durum','<',2)->get();
        ...
        if($takvim_turu == 3 && !Auth::user()->hasRole('Personel')){
            $randevu_hizmetler = DB::table('randevu_hizmetler')->
            join('randevular','randevu_hizmetler.randevu_id','=','randevular.id')->
            join('users','randevular.user_id','=','users.id')->
            join('hizmetler','randevu_hizmetler.hizmet_id','=','hizmetler.id')->
            ...
            ->select(
                ...
                'randevular.user_id as userid',
                'randevu_hizmetler.id as id',
                 DB::raw('CASE when randevular.user_id = 0 THEN "Kapalı Saat" ELSE CONCAT(users.name,"\n",hizmetler.hizmet_adi,"\n","Personel : ",COALESCE(salon_personelleri.personel_adi,""),"\n","Cihaz : ",COALESCE(cihazlar.cihaz_adi,"")) END as title'),DB::raw('CONCAT("#ffffff") as borderColor'),
                 DB::raw('TRIM(CONCAT("<table><tr><td>Telefon</td><td>:</td><td>",COALESCE(users.cep_telefon,""),"</td></tr>
                    <tr><td>Hizmet</td><td>:</td><td>",COALESCE(hizmetler.hizmet_adi,""),"</td></tr>
                    <tr><td>Personel</td><td>:</td><td>",COALESCE(salon_personelleri.personel_adi,""),"</td></tr>
                    <tr><td>Cihaz</td><td>:</td><td>",COALESCE(cihazlar.cihaz_adi,""),"</td></tr>
            ...
            )->where('randevular.salon_id',self::mevcutsube($request))->where('randevular.durum','<',2)->where(function($q){ if(Auth::user()->hasRole('Personel')) $q->where('randevu_hizmetler.personel_id',Auth::user()->personel_id); })->get();
        ...
        if(min($baslangic_saatleri) < $calismasaatibaslangic)
            $takvim_baslangic_saat = min($baslangic_saatleri);
        else
            $takvim_baslangic_saat = $calismasaatibaslangic;
        if(max($bitis_saatleri) > $calismasaatibitis)
            $takvim_baslangic_saat = min($baslangic_saatleri);
        else
            $takvim_baslangic_saat = $calismasaatibitis;
        ...
        return array(
                ...
                'minbaslangic' => min($baslangic_saatleri),
                'maxbitis' => max($bitis_saatleri)         
    ...
    public function kasadefteri(Request $request){
        ...
        return view('isletmeadmin.kasadefteri',['sayfa_baslik'=>'Kasa Raporu', 'title'=> 'Kasa Defteri | '.$isletme->salon_adi.' İşletme Yönetim Paneli',   'pageindex' => 103, 'bildirimler'=>self::bildirimgetir($request),'isletme'=>$isletme,'kasa'=>$kasa, 'kalan_uyelik_suresi' => self::lisans_sure_kontrol($request)]);
        
    }
    public function kasa_raporu_getir(Request $request)
    {
        ...
        foreach($tahsilatlar as $tahsilat){
            $tahsilat_liste .= '<tr>
                            ...
                            <td>';
            if($tahsilat->adisyon_id != null)
            {

                $tahsilat_liste .= $tahsilat->adisyon->musteri->name;
            }
            $tahsilat_liste .='</td>
    ...
    public function personel_liste_getir(Request $request)
    {
        ...
        ->select(
            ...
            DB::raw('CONCAT("<div class=\"dropdown\">
                ...
                <div class=\"dropdown-menu dropdown-menu-right dropdown-menu-icon-list\">
                  <a class=\"dropdown-item\" href=\"/isletmeyonetim/personeldetay/",salon_personelleri.id,"\"><i class=\"fa fa-eye\"></i> Detaylı Bilgi</a>
        ...
        )->where('salon_personelleri.salon_id',self::mevcutsube($request))->where('model_has_roles.salon_id',self::mevcutsube($request))->get();
        return $personeller;
    }
    ...
    public function denemesql()
    {
        ...
        ->select('hizmetler.hizmet_adi as hizmet_adi',DB::raw('GROUP_CONCAT(salon_personelleri.personel_adi) as personel'))->where('salon_sunulan_hizmetler.salon_id',Auth::user()->salon_id)->groupBy('hizmetler.id')->get();
    ...
    public function profilresimyukle(Request $request){
        ...
        $folderPath = '/home/gvxrande/public_html/public/profil_resimleri/';
     ...
     public function isletmekapakresimyukle(Request $request){
        ...
        $folderPath = '/home/gvxrande/public_html/public/salon_gorselleri/';
    ...
    public function randevuguncelle(Request $request){
        ...
        if($cakisma_varmi != '' && !isset($request->cakisanrandevuekle))
        {
            ...
            if($cakisma_varmi == '' || $request->cakisanrandevuekle==1)
            {
                ...
                $randevu->save();
                $totalsure = 0;
                $eskihizmetler = RandevuHizmetler::where('randevu_id',$randevu->id)->delete();
                ...
                {
                    $mesajlar = array(
                    array("to"=>$randevu->users->cep_telefon,"message"=>$randevu->salonlar->salon_adi . " tarafından ".$eskitarihsaat." randevunuz ".date('d.m.Y',strtotime($randevu->tarih)) .'-'.$randevu->saat .' olarak güncellenmiştir. Detaylı bilgi için bize ulaşın. 0'.$randevu->salonlar->telefon_1),
                ...
                if(SalonSMSAyarlari::where('ayar_id',14)->where('salon_id',$randevu->salon_id)->value('personel')==1)
                {
                    foreach($randevu->hizmetler as $hizmet)
                    {
                        $mesaj = $randevu->users->name." isimli müşterinin ".$hizmet->hizmetler->hizmet_adi." randevusu ". date('d.m.Y',strtotime($randevu->tarih)) ." - ". date('H:i',strtotime($hizmet->saat)) ." olarak ".Auth::user()->name." tarafından güncellenmiştir.";
                        ...
                        self::sms_gonder($request,$mesajlar,false,1,false);
                        self::bildirimekle($request,$randevu->salon_id,$mesaj,"#",$hizmet->personel_id,null, Auth::user()->profil_resim,$randevu->id);
                ...
                if(!isset($request->takvim_sayfasi)){
                    ...
                    $butonlar .= "<a class='btn btn-primary btn-lg btn-block' href='/isletmeyonetim/randevular/'>"
                                          ."Takvime Git</a>".
                ...
                return json_encode($randevuguncellendi);
                exit();
    ...
    public function randevu_resize_drop(Request $request)
    {
        ...
        $randevu->olusturan_personel_id = Auth::user()->id;
        $randevu->save();
    ...
    public function randevuiptalet(Request $request){
        ...
        if($red)
        {
            if(SalonSMSAyarlari::where('salon_id',$randevu->salon_id)->where('ayar_id',3)->value('musteri')==1){
                $mesajlar = array(
                    array("to"=>$randevu->users->cep_telefon,"message"=>$isletme->salon_adi." için oluşturduğunuz ".date('d.m.Y',strtotime($randevu->tarih)) ." ". date('H:i',strtotime($randevu->saat)) ." tarihli randevu talebiniz reddedilmiştir. Detaylı bilgi için bize ulaşın. 0".$isletme->telefon_1)
                );
            ...
            if(SalonSMSAyarlari::where('salon_id',$randevu->salon_id)->where('ayar_id',3)->value('personel')==1)
            {
                ...
                foreach($randevu->hizmetler as $hizmet)
                {
                    $mesaj = $randevu->users->name .' isimli müşterinin yarın '.date('H:i',strtotime($hizmet->saat)).' saatli '.$hizmet->hizmetler->hizmet_adi.' randevusu '.Auth::user()->name.' tarafından reddedilmiştir.';
                   $yetkiliid = Personeller::where('id',$hizmet->personel_id)->value('yetkili_id');
                   self::sms_gonder($request,array(array("to"=>IsletmeYetkilileri::where('id',$yetkiliid)->value('gsm1'),"message"=>$mesaj)),false,1,false); 
                   self::bildirimekle($request,$randevu->salon_id,$mesaj,"#",$hizmet->personel_id,null, Auth::user()->profil_resim,$randevu->id);
                    $bildirimkimlikleri = BildirimKimlikleri::where('isletme_yetkili_id',$hizmet->personel_id)->pluck('bildirim_id')->toArray(); 
        ...
        else
        {
            if(SalonSMSAyarlari::where('salon_id',$randevu->salon_id)->where('ayar_id',3)->value('musteri')==1){
                 $mesajlar = array(
                    array("to"=>$randevu->users->cep_telefon,"message"=>$isletme->salon_adi." için oluşturulan ".date('d.m.Y',strtotime($randevu->tarih)) ." ". date('H:i',strtotime($randevu->saat)) ." tarihli randevunuz iptal edilmiştir. Detaylı bilgi için bize ulaşın. 0".$isletme->telefon_1)
                );
                self::sms_gonder($request,$mesajlar,false,1,false);
            }
            if(SalonSMSAyarlari::where('salon_id',$randevu->salon_id)->where('ayar_id',3)->value('personel')==1)
            {
                ...
                {
                    $yetkiliid = Personeller::where('id',$hizmet->personel_id)->value('yetkili_id');
                    $mesaj = $randevu->users->name .' isimli müşterinin yarın '.date('H:i',strtotime($hizmet->saat)).' saatli '.$hizmet->hizmetler->hizmet_adi.' randevusu '.Auth::user()->name.' tarafından iptal edilmiştir.';
                   self::sms_gonder($request,array(array("to"=>IsletmeYetkilileri::where('id',$yetkiliid)->value('gsm1'),"message"=>$mesaj)),false,1,false); 
                   self::bildirimekle($request,$randevu->salon_id,$mesaj,"#",$hizmet->personel_id,null, Auth::user()->profil_resim,$randevu->id);
                    $bildirimkimlikleri = BildirimKimlikleri::where('isletme_yetkili_id',$hizmet->personel_id)->pluck('bildirim_id')->toArray(); 
     ...
     public function randevuonayla(Request $request){
        ...
        {
            $mesajlar = array(
            array("to"=>$randevu->users->cep_telefon,"message"=>$isletme->salon_adi." için oluşturduğunuz ".date('d.m.Y',strtotime($randevu->tarih)) ." ". date('H:i',strtotime($randevu->saat)) ." tarihli randevu talebiniz onaylanmıştır. Randevunuza 15 dk önce gelmenizi rica ederiz. Detaylı bilgi için bize ulaşın. 0".$randevu->salonlar->telefon_1),
            );
            self::sms_gonder($request,$mesajlar,false,1,false);
        }
        if(SalonSMSAyarlari::where('ayar_id',2)->where('salon_id',$randevu->salon_id)->value('personel') == 1)
        {
                ...
                {
                    $yetkiliid = Personeller::where('id',$hizmet->personel_id)->value('yetkili_id');
                    $mesaj = $randevu->users->name .' isimli müşterinin yarın '.date('H:i',strtotime($hizmet->saat)).' saatli '.$hizmet->hizmetler->hizmet_adi.' randevusu '.Auth::user()->name.' tarafından onaylanmıştır.';
    ...
    public function yetkilibilgiguncelle(Request $request){
        $user = IsletmeYetkilileri::where('id',Auth::user()->id)->first();
    ...
    public function subeaktifet(Request $request){
     ...
     public function gorselsil(Request $request){
        ...
        unlink('/home/gvxrande/public_html'.$gorsel->salon_gorseli);
        $gorsel->delete();
        $gorseller_html = self::salon_gorselleri($request);
        echo $gorseller_html;
    ...
    public function yenirandevu(){
        ...
        $mevcutmusteriler = Randevular::join('users','randevular.user_id','=','users.id')->where('randevular.salon_id',Auth::user()->salon_id)->groupBy('randevular.user_id')->get();
        return view('isletmeadmin.yenirandevu',[ 'title' =>  'Yeni Randevu Ekle | Avantajbu.com','pageindex' => 102,'isletme'=>$isletme,'personeller'=> $personeller, 'sunulanhizmetler'=> $sunulanhizmetler,'mevcutmusteriler'=>$mevcutmusteriler, 'kalan_uyelik_suresi' => self::lisans_sure_kontrol($request)] );
    }
        ...
        else
        {
            if($cakisma_varmi == '' || $request->cakisanrandevuekle==1)
            {
                foreach($randevu_tarihleri as $tarihler){
                    ...
                    $yenirandevu->olusturan_personel_id = Auth::user()->id;
                ...
                {
                    $mesajlar = array(
                    array("to"=>$gsm,"message"=>$isletme->salon_adi . " tarafından ".date('d.m.Y',strtotime($request->tarih)) .'-'.$request->saat .' olarak randevunuz oluşturulmuştur. Randevunuza 15 dk önce gelmenizi rica ederiz. Detaylı bilgi için bize ulaşın. 0'.$isletme->telefon_1),
    ...
    public function kampanyalar(){
        ...
        return view('isletmeadmin.kampanyalar',['kampanyalar' => $kampanyalar, 'pageindex'=> 105,'title'=>'Avantajlarım | '.$isletme->salon_adi.' Salon Yönetim Paneli','isletme'=>$isletme,'kampanyahtml' => $kampanyahtml, 'kalan_uyelik_suresi' => self::lisans_sure_kontrol($request)]);
    }
    ...
    public function kampanyadetaylari($id){
        ...
        return view('isletmeadmin.kampanyadetay',['pageindex'=> 105,'title'=>'Kampanya Detayları | '.$isletme->salon_adi.' İşletme Yönetim Paneli','isletme'=>$isletme,'kampanyadetay' => $kampanyadetay, 'kalan_uyelik_suresi' => self::lisans_sure_kontrol($request)]);
    }
    public function yenikampanyaekle(){
        ...
        $isletme = Salonlar::where('id',Auth::user()->salon_id)->first();
        return view('isletmeadmin.yenikampanya',['pageindex'=> 105,'title'=>'Yeni Kampanya Ekle | '.$isletme->salon_adi.' İşletme  Yönetim Paneli','isletme'=>$isletme, 'kalan_uyelik_suresi' => self::lisans_sure_kontrol($request)]);
    ...
    public function avantajyapilanodemeler(){
        ...
        return view('isletmeadmin.kampanyayapilanodemeler',['pageindex'=>1051,'title'=>'Yapılan Ödemeler | '.$isletme->salon_adi.' İşletme Yönetim Paneli','avantajodemeler'=> $avantajodemeler,'isletme'=>$isletme, 'kalan_uyelik_suresi' => self::lisans_sure_kontrol($request)]);
    }
    ...
    public function smslistedetay($listeid){
        ...
        $isletme = Salonlar::where('id',Auth::user()->salon_id)->first();
        return view('isletmeadmin.smslistedetay',['title' => $listeadi.' SMS Liste Detayı | Avantajbu.com','pageindex' => 107,'isletme'=>$isletme,'smslistedetaylari'=> $smslistedetaylari, 'kalan_uyelik_suresi' => self::lisans_sure_kontrol($request)]);
    }
    ...
    public function smsraporlar(Request $request){
        ...
        return view('isletmeadmin.smsraporlar',['pageindex'=> 109,'title'=>'SMS Raporlarım | '.$isletme->salon_adi.' İşletme Yönetim Paneli','isletme'=>$isletme,'rapor'=>$raporhtml, 'kalan_uyelik_suresi' => self::lisans_sure_kontrol($request)]);
    }
...
<input type="hidden" id="smstaslak'.$taslak->id.'" value="'.$taslak->taslak_icerik.'">
    ...
    public function musteriliste(Request $request){
        ...
        $paketler = self::paket_liste_getir('',true,$request);
        return view('isletmeadmin.musteriler',[ 'bildirimler'=>self::bildirimgetir($request),'paketler'=>$paketler,'sayfa_baslik'=>'Müşteriler','pageindex' => 4, 'musteriler'=> $musteriler,'isletme'=> $isletme,'aktif_musteriler'=>$aktif_musteriler,'sadik_musteriler'=>$sadik_musteriler,'pasif_musteriler'=> $pasif_musteriler , 'kalan_uyelik_suresi' => self::lisans_sure_kontrol($request)]);
    }
    ...
    public function musteri_liste_getir(Request $request,$durum)
    {
        ...
        if($durum == 3)
        {
            ...
            ->groupBy('users.id')->orderBy('users.id','desc')->get(); 
        }
        if($durum == 0){
            $musteriler = DB::table('musteri_portfoy')->join('users','musteri_portfoy.user_id','=','users.id')->
            leftjoin('randevular','randevular.user_id','=','users.id')->
        ...
        if($durum == 1){
            ...
            ->orHaving(DB::raw('(SELECT  DATE_ADD(adisyonlar.created_at , INTERVAL 3 MONTH) FROM adisyonlar where adisyonlar.user_id = users.id order by adisyonlar.id desc limit 1)'),'>=',date('Y-m-d H:i:s', strtotime('+90 days', strtotime(date('Y-m-d H:i:s')))))->groupBy('users.id')->orderBy('users.id','desc')->get();
        ...
        if($durum == 2){
            ...
            ->having(DB::raw('(SELECT  DATE_ADD(adisyonlar.created_at , INTERVAL 3 MONTH) FROM adisyonlar where adisyonlar.user_id = users.id order by adisyonlar.id desc limit 1)'),'<',date('Y-m-d H:i:s', strtotime('+90 days', strtotime(date('Y-m-d H:i:s')))))->groupBy('users.id')->orderBy('users.id','desc')->get();
    ...
    public function musteridetay(Request $request,$id){
        ...
        $paketler = self::paket_liste_getir('',true,$request);
        return view('isletmeadmin.musteridetay',['bildirimler'=>self::bildirimgetir($request),'paketler'=>$paketler,'pageindex'=>41, 'sayfa_baslik'=> $portfoy->users->name,'isletme'=>$isletme,'portfoy'=>$portfoy, 'musteri_bilgi'=>$portfoy->users,'randevular'=>$randevular,'adisyonlar'=>$adisyonlar, 'kalan_uyelik_suresi' => self::lisans_sure_kontrol($request)]);
    }
    ...
    public function toplusmsbasvuru(){
        ...
        $smspaketler = SMSPaketleri::all();
         return view('isletmeadmin.toplusmsbasvuru',['title' => 'Toplu SMS Paket Satın Al - Başvur | Avantajbu.com','pageindex' => 114,'isletme'=>$isletme,'smspaketler' => $smspaketler, 'kalan_uyelik_suresi' => self::lisans_sure_kontrol($request)]);
    }
    ...
    public function randevu_sil(Request $request){
        ...
        if(Auth::user()->is_admin)
            ...
            $randevular = Randevular::where('salon_id',self::mevcutsube($request))->where('tarih','like','%'.$request->tarih.'%')->where('sube_id',Auth::user()->salon_personelleri->sube_id)->orderBy('id','desc')->get();
    ...
    public function adisyondetay(Request $request,$id)
    {
        ...
        return view('isletmeadmin.adisyondetay',['isletme'=>$isletme,'paketler'=>$paketler,'bildirimler'=>self::bildirimgetir($request), 'sayfa_baslik'=>$adisyon->musteri->name .' '.date('d.m.Y', strtotime($adisyon->created_at)).' tarihli adisyon detayı','pageindex' => 111,'adisyon'=>$adisyon,'request'=>$request, 'kalan_uyelik_suresi' => self::lisans_sure_kontrol($request), 'musteri'=>$adisyon->musteri,'tahsilatlar'=>$tahsilatlar,'adisyon_id'=>$id]);
    }
   ...
   public function urunler(Request $request){
        ...
        $isletme = Salonlar::where('id',self::mevcutsube($request))->first();
        $paketler = self::paket_liste_getir('',true,$request);
        return view('isletmeadmin.urunler',['bildirimler'=>self::bildirimgetir($request),'paketler'=>$paketler,'pageindex'=>30, 'sayfa_baslik'=>'Ürünler','urunler'=>$urunler,'isletme'=>$isletme, 'kalan_uyelik_suresi' => self::lisans_sure_kontrol($request)]);
    }
 ...
 public function urun_liste_getir(Request $request,$returntext){
    ...
    public function urunadisyondansil(Request $request)
    {
        ...
        else
        {
            ...
            return array(
                    ...
                    'tum_tahsilatlar' => self::musteri_tahsilatlari($request,$musteriid,$request->adisyon_id)
    ...
    {       
        ...
        else
        {
            ...
            /*if(AdisyonHizmetler::where('adisyon_id',$adisyon_id)->count()+AdisyonUrunler::where('adisyon_id',$adisyon_id)->count()+AdisyonPaketler::where('adisyon_id',$adisyon_id)->count()==0)
                Adisyonlar::where('id',$adisyon_id)->delete()*/
            return self::adisyon_detay($request,Adisyonlar::where('id',$adisyon_id)->first(),false,$musteriid);
            exit;
        }
    ...
    public function tahsilatekle(Request $request){
        ...
        return self::musteri_tahsilatlari($request,$request->ad_soyad,$request->adisyon_id);
    ...
    public function adisyonlar(Request $request){
        ...
        'taksitler_kapali'=>self::taksitleri_getir($request,1,''),
            ...
            ,'adisyonlar_paket'=>$adisyonlar_paket ]);
    }
   
    public function testcases(Request $request)
    {  
       return self::adisyon_yukle($request,'','','1970-01-01',date('Y-m-d'),'','');
    }
    ...
    public function seanstakip(Request $request){
       ...
       return view('isletmeadmin.seanstakip',['bildirimler'=>self::bildirimgetir($request),'adisyonlar'=>$adisyonlar,'pageindex'=>14, 'sayfa_baslik'=>'Seans Takibi','isletme'=>$isletme,'seanstakip'=>$adisyonlar, 'kalan_uyelik_suresi' => self::lisans_sure_kontrol($request)]);
    }
    ...
    public function satis_filtre(Request $request)
    {
        ...
        if($request->tarih1!=''&&$request->tarih2!='')
            $adisyonlar =self::adisyon_yukle($request,'','',date($request->tarih1.' 00:00:00'),date($requst->tarih2.' 23:59:59'),'',$request->personel_id);
        else
                    ...
                    ((SELECT COALESCE(SUM(tahsilat_urunler.tutar),0) from tahsilat_urunler where tahsilat_urunler.adisyon_urun_id = adisyon_urunler.id)*(COALESCE(p2.urun_prim_yuzde,0)/100)) + 
                     ((SELECT COALESCE(SUM(tahsilat_paketler.tutar),0) FROM tahsilat_paketler where tahsilat_paketler.adisyon_paket_id = adisyon_paketler.id)*(COALESCE(p3.paket_prim_yuzde,0)/100)) as hakedis_numeric'),
                       ...
                       'users.name as musteri',
                        ...
                        DB::raw('CONCAT("<a style=\"line-height:5px;padding:5px\" title=\"Tahsil Et\" href=\"/isletmeyonetim/tahsilat/",adisyonlar.user_id,"\" type=\"button\"  class=\"btn btn-success\"><i class=\"fa fa-money\"></i> </a> 
...
<input type="hidden" name="adisyon_urun_senet_id[]" value="'.$urun->senet_id.'">
    ...
    public function tahsilatkaldir(Request $request){
        ...
        $tahsilat->delete();
        return self::musteri_tahsilatlari($request,$musteri_id,$request->adisyon_id); 
    }
    public function ongorusmeler(Request $request)
    {
        ...
        return view('isletmeadmin.ongorusmeler',['paketler'=>$paketler,'bildirimler'=>self::bildirimgetir($request),'sayfa_baslik'=>'Ön Görüşmeler','on_gorusmeler'=>$ongorusmeler,'pageindex' => 12,'isletme'=>$isletme, 'kalan_uyelik_suresi' => self::lisans_sure_kontrol($request)]);  
    }
    ...
    { 
         $headers = array(
                     'Authorization: Key DgeXZa33Nmg7vj10CXj7GY77FcdUWPlPxmvzKzIVQyG4',
                     'Content-Type: application/json',
                     'Accept: application/json'
         );
    ...
    public function ongorusmegetir(Request $request,$gunluk){
        ...
        ->where('salonlar.id',self::mevcutsube($request))
        ->where(function ($q){if(Auth::user()->hasRole('Personel')) $q->where('salon_personelleri.id',Auth::user()->personel_id);})
    ...
    public function paketsatislari(Request $request)
    {
        ...
        $personeller = Personeller::where('salon_id',Auth::user()->salon_id)->get();
        return view('isletmeadmin.paketsatislari',['paketler'=>$paketler, 'bildirimler'=>self::bildirimgetir($request),'sayfa_baslik' => 'Paketler','pageindex' => 13,'isletme' => $isletme,'personeller'=>$personeller, 'kalan_uyelik_suresi' => self::lisans_sure_kontrol($request)]);
    }
    ...
    public function randevuliste(Request $request)
    {
        ...
        $randevular = self::randevu_liste_getir($request,'','','','','','',self::mevcutsube($request));
        $isletme = Salonlar::where('id',self::mevcutsube($request))->first();
         return view('isletmeadmin.randevular_liste',['isletme'=>$isletme,'randevular_liste'=>$randevular,'paketler'=>$paketler,'bildirimler'=>self::bildirimgetir($request),'pageindex'=>3,'sayfa_baslik'=>'Randevular', 'kalan_uyelik_suresi' => self::lisans_sure_kontrol($request)]);
    }
    ...
    {
        $personel = Personeller::where('salon_id',self::mevcutsube($request) )->where('yetkili_id',Auth::user()->id)->value('id');
        $bildirimler = Bildirimler::where('personel_id',$personel)->orderBy('id','desc')->get();
        return $bildirimler;
    }
    public function paketsatisekle(Request $request){
        ...
        foreach($request->paketadi as $key=>$paket){
            ...
            if($request->paket_id[$key] != "")
                ...
                $paket = new Paketler();
            $paket->paket_adi = $request->paketadi[$key];
            ...
            $paketsatis->paket_id =  $request->paketadi[$key];
            ...
            $paketsatis->salon_id = $request->sube;
        ...
        return array(
           ...
           'tahsil_edilen'=>number_format($tahsil_edilen_tutar,2,',','.'),
           'kalan_tutar' =>number_format($kalan_tutar,2,',','.'),
           'tum_tahsilatlar'=>self::musteri_tahsilatlari($request,$request->musteri_id,$request->adisyon_id)
    ...
    public function adisyon_paket_satis_getir($adisyon_id,$visibility,$paket_id)
    {
        ...
        foreach($satislar as $paket){
            $tahsilat_paket_eklenecek .= ' <div class="row" style="background:#cce5ff;margin:5px 0 5px 0; padding:5px;font-size:14px" data-value="0">
                     ...
                     ';
                     if($visibility && $paket_id == $paket->id && !Auth::user()->hasRole('Personel'))
    ...
    public function alacaklar(Request $request)
    {
        ...
        $isletme = Salonlar::where('id',self::mevcutsube($request))->first();
        return view('isletmeadmin.alacaklar',['pageindex'=>16,'sayfa_baslik'=>'Alacak Takibi','bildirimler'=>self::bildirimgetir($request),'paketler'=> self::paket_liste_getir('',true,$request),'alacaklar'=>self::alacakgetir($request),'isletme'=>$isletme, 'kalan_uyelik_suresi' => self::lisans_sure_kontrol($request)]);
    ...
    public function masraflar(Request $request)
    {
        ...
        $isletme = Salonlar::where('id',self::mevcutsube($request))->first();
        return view('isletmeadmin.masraflar',['isletme'=>$isletme,'bildirimler'=>self::bildirimgetir($request),'paketler'=> self::paket_liste_getir('',true,$request),'pageindex'=>15,'sayfa_baslik'=>'Masraflar','bildirimler'=>self::bildirimgetir($request),'paketler'=> self::paket_liste_getir('',true,$request),'masraflar'=>self::masrafgetir($request), 'kalan_uyelik_suresi' => self::lisans_sure_kontrol($request)]);
    }
    ...
    public function masrafekleduzenle(Request $request)
    {
        ...
        if(!isset($request->masraf_sayfasi))
            $butontext = "<p><a href='/isletmeyonetim/masraflar' class='btn btn-primary btn-lg btn-block'>Masraf Listeme Git</a></p>";
        if($guncelleme)
    ...
    public function randevu_liste_getir(Request $request,$tarih1,$tarih2,$salon,$web,$uygulama,$durum,$salon_id)
    {
        ...
        select(
                ...
                <div class=\"dropdown-menu dropdown-menu-right dropdown-menu-icon-list\">
                    <a class=\"dropdown-item\" data-toggle=\"modal\" data-value=\"",randevular.id,"\" data-target=\"#randevu-duzenle-modal\" name=\"randevu_duzenle\" href=\"#\"> <i class=\"fa fa-edit\"></i> Düzenle</a>
            ...
            where(function($q) use ($durum){if($durum!='') $q->where('randevular.durum',$durum);})->where(function($q){ if(Auth::user()->hasRole('Personel')) $q->where('randevu_hizmetler.personel_id',Auth::user()->personel_id); })->
    ...
    public function senetler(Request $request){
        ...
        $senetler_odenmemis = self::senetleri_getir($request,2,'');
         return view('isletmeadmin.senetler',['pageindex'=>17,'sayfa_baslik'=>'Senet Takibi','bildirimler'=>self::bildirimgetir($request),'paketler'=> self::paket_liste_getir('',true,$request),'isletme'=>$isletme,'senetler'=>$senetler,'senetler_acik'=>$senetler_acik,'senetler_kapali'=>$senetler_kapali,'senetler_odenmemis'=>$senetler_odenmemis, 'kalan_uyelik_suresi' => self::lisans_sure_kontrol($request)]);
    }
    ...
    public function senetleri_getir(Request $request,$sorgu,$musteriid)
    {
        if($sorgu===''){
           ...
           DB::raw('CASE WHEN (SELECT senet_vadeleri.vade_tarih FROM senet_vadeleri WHERE senet_vadeleri.odendi=false AND senet_vadeleri.senet_id = senetler.id
                ...
                <a title=\"Tahsil Et\" href=\"/isletmeyonetim/tahsilat/",senetler.user_id, "\"    data-value=\"",senetler.id,"\" class=\"btn btn-success\"><i class=\"fa fa-money\"></i></a>") as islemler')
    ...
    public function senetvadegetir_tahsilat(Request $request)
    {
        ...
        $vadeler = SenetVadeleri::join('senetler','senet_vadeleri.senet_id','=','senetler.id')->leftJoin('odeme_yontemleri','senet_vadeleri.odeme_yontemi_id','=','odeme_yontemleri.id')->where('senetler.user_id',$request->musteriid)->select('senetler.id as senet_id','senet_vadeleri.id as id','senet_vadeleri.vade_tarih as vade_tarih','senet_vadeleri.odendi as odendi','senet_vadeleri.notlar as notlar','odeme_yontemleri.odeme_yontemi as odeme_yontemi','senet_vadeleri.tutar as tutar')->where('senet_vadeleri.odendi',false)->get();
        $html = '';
        foreach($vadeler as $key => $vade)
        {
    ...
    public function profilbilgileri(Request $request){
        ...
        $isletme = Salonlar::where('id',self::mevcutsube($request))->first();
        return view('isletmeadmin.profilbilgileri',['pageindex'=>19,'sayfa_baslik'=>'Profil Bilgileri','bildirimler'=>self::bildirimgetir($request),'paketler'=> self::paket_liste_getir('',true,$request),'isletme'=>$isletme, 'kalan_uyelik_suresi' => self::lisans_sure_kontrol($request)]);
    }
    ...
    public function yeniisletmeekle(Request $request)
    {
        ...
        $yetkili->password = Hash::make($request->password);
        $yetkili->salon_id = $isletme->id;
        $yetkili->syncRoles('Şube Yöneticisi');
        $yetkili->save();
        $yetkili_olunan_sube = YetkiliOlunanSubeler::create(
            ['salon_id'=> $isletme->id,'yetkili_id'=>$yetkili->id],
            ['salon_id'=> $isletme->id, 'yetkili_id'=> Auth::user()->id]
        );
        $yetkili_olunan_sube->salon_id = $isletme->id;
        $yetkili_olunan_sube->yetkili_id = $yetkili->id;
        $yetkili_olunan_sube->save();
        return array(
    ...
    public function odeme_sayfasi(Request $request)
    {
        ...
        $isletme = Salonlar::where('id',$request->id)->first();
        $odeme = "5000";
        $urun = "Yeni Şube Ekleme";
        return view('isletmeadmin.odeme',['sayfa_baslik'=>'Ödeme','urunler'=>$urunler,'hizmetler'=>$hizmetler,'paketler'=>$paketler,'pageindex'=>9,'musteri_bilgileri'=>$musteri_bilgileri,'odeme'=>$odeme,'urun'=>$urun,'isletme'=>$isletme,'bildirimler'=>$bildirimler, 'kalan_uyelik_suresi' => self::lisans_sure_kontrol($request)]);
   ...
   public function etkinlikekleduzenle(Request $request)
       ...
       foreach ($etkinlikbeklenen->katilimcilar as  $katilimci) {
        if($katilimci->durum===null)
        {
            ...
            if(SalonSMSAyarlari::where('ayar_id',10)->where('salon_id',$etkinlikbeklenen->salon_id)->value('musteri')==1)
                $katilim_link = ' Katılım için : https://'.$_SERVER['HTTP_HOST'].'/kampanyakatilim/'.$etkinlikbeklenen->id.'/'.$katilimci->user_id;
    ...
    {
        return self::randevuyukle($request,$request->ayar);
    }
    ...
    public function etkinlikler(Request $request){
       ...
       $isletme = Salonlar::where('id',self::mevcutsube($request))->first();
       $katilimci= self::etkinlikyukle($request);
        return view('isletmeadmin.etkinlikler',['paketler'=>$paketler,'pageindex'=>20,'sayfa_baslik'=>'Etkinlikler','isletme'=>$isletme,'bildirimler'=>self::bildirimgetir($request),'katilimci'=>$katilimci, 'kalan_uyelik_suresi' => self::lisans_sure_kontrol($request)]);
    ...
    public function randevu_geldi_isaretle_adisyon_olustur(Request $request)
    {
        $randevu = Randevular::where('id',$request->randevuid)->first();
        $adisyonvar = false;
        $adisyon = '';
    ...
    public function senetekleguncelle(Request $request)
    {
        ...
        $senet->olusturan_id = Auth::user()->id;
...
$odeme->tutar = round((str_replace(['.',','],['','.'],$request->urun_fiyat_senet[$key])/($on_odeme_tutari+str_replace(['.',','],['','.'],$request->senet_tutar)))*$on_odeme_tutari ,2 );
        ...
        if(isset($request->paket_id_senet))
        {
            foreach($request->paket_id_senet as $key=>$paket){
                $adisyon_paket_id = self::adisyona_paket_ekle($adisyon_id,$paket,$request->paket_fiyat_senet[$key],$request->paket_baslangic_tarihi_senet[$key],$request->sens_aralik_gun_senet[$key],$request->paket_satan_senet[$key],$senet->id,null);
    ...
    public function taksitekleguncelle(Request $request)
    {
        ...
        $taksitlitahsilat->olusturan_id = Auth::user()->id;
        ...
        return self::musteri_tahsilatlari($request,$musteri->id,$request->adisyon_id);
        exit;
 ...
 $paketler = self::paket_liste_getir('',true,$request);
     ...
     return view('isletmeadmin.kampanya_yonetimi',['paketler'=>$paketler,'pageindex'=>22,'sayfa_baslik'=>'Reklam Yönetimi','isletme'=>$isletme,'bildirimler'=>self::bildirimgetir($request),'kampanya_yonetimi'=>self::paket_kampanyalar($request), 'kalan_uyelik_suresi' => self::lisans_sure_kontrol($request)]);
    ...
    public function seanstanrandevuolustur(Request $request)
    {
        ...
        if(SalonSMSAyarlari::where('ayar_id',12)->where('salon_id',$randevu->salon_id)->value('personel')){
            ...
            $mesaj = $randevu->users->name .' isimli müşterinin '. $hizmet . " için ".date('d.m.Y',strtotime($randevu->tarih)) .'-'.date('H:i',
                        strtotime($randevu->saat)) .' tarihli randevusu oluşturulmuştur.';
            self::bildirimekle($request,$randevu->salon_id,$mesaj,"#",$request->personel_id,null, Auth::user()->profil_resim,$randevu->id);
    ...
    public function randevudetayigetir(Request $request)
    {
        ...
        {
            $hizmetler_html .= ' <div class="row" data-value="'.$key.'">
                                    <div class="col-md-2">
                                        <div class="form-group">
                                            <label>Personel </label>
                                            <select name="randevupersonelleriyeni[]" class="form-control opsiyonelSelect" style="width: 100%;"><option></option>';
            foreach(Personeller::where('salon_id',$randevu->salon_id)->where('aktif',true)->get() as $personel)
            {
                if($hizmet->personel_id == $personel->id)
                    $hizmetler_html .= '<option selected value="'.$personel->id.'">'.$personel->personel_adi.'</option>';
                else
                    $hizmetler_html .= '<option value="'.$personel->id.'">'.$personel->personel_adi.'</option>';
            ...
            $hizmetler_html .= ' </select>
                     ...
                     <div class="col-md-1">
                        <div class="form-group">';
            if($key==0)
                            $hizmetler_html .= '<label style="visibility:visible;font-size:12px;width:100%" class="usttekiylebirlestiryazi"  >Üsttekiyle Birleştir</label>
        ...
        return json_encode(array(
            ...
            'randevu_hizmetler' => $hizmetler_html,
        ));
    }
    ...
    public function seansdetaylari(Request $request){
        ...
        foreach($paket_seanslar as $key => $seans)
        {
            ...
            $html .= '  </div>
                ...
                $html .= ' <select name="paketseanspersonelcihaz" data-value="'.$seans->id.'" class="form-control custom-select2 opsiyonelSelect" style="width: 100%;"> <option></option>';
                foreach(\App\Personeller::where('salon_id',$request->sube)->where('aktif',true)->get() as $personel)
                {
                    if($seans->personel_id == $personel->id)
                        $html .= '<option selected value="'.$personel->id.'">'.$personel->personel_adi.'</option>';
                    else
                        $html .= '<option value="'.$personel->id.'">'.$personel->personel_adi.'</option>';

                }
                foreach(\App\Cihazlar::where('salon_id',$request->sube)->where('durum',true)->where('aktifmi',true)->get() as $cihaz)
                {
                    if($seans->cihaz_id == $cihaz->id)
                        $html .= '<option selected value="cihaz-'.$cihaz->id.'">'.$cihaz->cihaz_adi.'</option>';
                    else
            ...
            $html .= '</div>
                ...
                $html .= '<select name="paketseansoda" data-value="'.$seans->id.'"  class="form-control opsiyonelSelect" style="width:100%"><option></option>';
                foreach(\App\Odalar::where('salon_id',$request->sube)->where('durum',true)->where('aktifmi',true)->get() as $oda)
                {
                    if($seans->oda_id == $oda->id)
                        $html .= '<option selected value="'.$oda->id.'">'.$oda->oda_adi.'</option>';
                    else
        ...
        if(($dogrulama_kodu_ayari && $vade->dogrulama_kodu == $request->dogrulama_kodu) || !$dogrulama_kodu_ayari)
        {
            ...
            if(isset($request->tahsilatekrani))
            {
                return array(
                    'vadeler'=>'', 
                    'status'=>'Başarılı',
                    'tahsilatlar' => self::musteri_tahsilatlari($request,$request->musteri_id,$request->adisyon_id)
                );
                exit;
            }
            else
    ...
    public function smsraportest()
    {
        return self::sms_rapor_getir(892524,Salonlar::where('id',107)->first());
    }
    ...
    {
        $personel = Personeller::where('id',$request->personelid)->value('yetkili_id');
        $yetkili = IsletmeYetkilileri::where('id',$personel)->first();
        $random = str_shuffle('ABCDEFGHJKLMNOPQRSTUVWXYZ1234567890');
        $kod = substr($random, 0, 6);
        $yetkili->password = Hash::make($kod);
        $yetkili->save();
        $mesajlar = array(array("to"=>$yetkili->gsm1,"message"=> "Sayın ".$yetkili->name.". Randevumcepte yeni şifreniz : ".$kod. ' olarak '.Auth::user()->name.' tarafından belirlenmiştir.'));
        self::sms_gonder($request,$mesajlar,false,1,false);
    ...
    public function bildirimgonder($bildirimkimlikleri,$mesaj,$baslik,$salonid){
        ...
        $headers_push_notification = array(
                                        'Accept: application/json',
                                        'Authorization: Basic ZWNhZDZiNTktNGUyZS00NzNjLTk3MGMtYjg0ZDdhZDEwMzAw',
                                        'Content-Type: application/json',
        );
        ...
        $post_data_push_notification = 
            json_encode( 
                ...
                array( 
                    ...
                    "android_channel_id" => 'fe251a6f-ddc6-455c-95a5-e7438f09c967',
    ...
    public function cakisan_randevu_kontrol(Request $request,$randevu_tarihleri)
    {
        ...
        foreach($randevu_tarihleri as $tarihler){
            ...
            foreach ($request->randevuhizmetleriyeni as $key => $value) {
                ...
                $onaylirandevular = Randevular::join('randevu_hizmetler','randevu_hizmetler.randevu_id','=','randevular.id')
                ->leftjoin('salon_personelleri','randevu_hizmetler.personel_id','=','salon_personelleri.id')
                ->join('hizmetler','randevu_hizmetler.hizmet_id','=','hizmetler.id')
                ->leftjoin('cihazlar','randevu_hizmetler.cihaz_id','=','cihazlar.id')
                ->leftjoin('odalar','randevu_hizmetler.oda_id','=','odalar.id')
    ...
    {
        $salon = Salonlar::where('domain',$_SERVER['HTTP_HOST'])->first();
        Excel::load('/public/datalar/fulya_randevular.xlsx',function($reader) use($salon) {
            foreach ($reader->toArray() as $key => $row) {
                $musteri_var = DB::table('musteri_portfoy')->join('users','users.id','=','musteri_portfoy.user_id')->select('users.id as id')->where('users.name',$row['musteri'])->where('musteri_portfoy.salon_id',$salon->id)->first();
                 if($musteri_var)
                    echo $row['musteri'].' mevcut<br>';
                ...
                $randevu->tarih = date('Y-m-d',strtotime(self::convertMonthToTurkishCharacter($row['tarih'])));
                $randevu->saat = $row['saat'];
                $randevu->durum = 1;
                $randevu->salon = true;
                $randevu->notlar = "SalonAppy den aktarım";
                
                $randevu->olusturan_personel_id = Personeller::where('salon_id',$salon->id)->where('personel_adi',$row['olusturan'])->value('id');
                $randevu->save(); 
                $yenisaatbaslangic =  $row['saat'];
                $hizmet_sureleri_okunan = array();
                foreach(self::extract_outside_text($row['hizmetler']) as $key => $hizmet){ 
                   
                    $hizmet1 = str_replace(", ", "", $hizmet);
                    ...
                    array_push($hizmet_sureleri_okunan,$hizmetvar->sure);
                    if($hizmetvar)
                        echo $hizmet1.' mevcut. ID : '.$hizmetvar->hizmet_id.' Süre dk : '.$hizmetvar->sure.' Fiyat : '.$hizmetvar->fiyat;
                    else
                        echo $hizmet1.' bulunamadı'; 

                    echo '<br>';
                    $cihaz = self::extract_inside_text($row['hizmetler']);
                    echo 'Cihaz '.$cihaz[$key]. ' id : '.Cihazlar::where('cihaz_adi',$cihaz[$key])->where('salon_id',$salon->id)->value('id');
                    echo '<br>';
  ...
  public function adisyon_sil(Request $request)
   {
    ...
    public function taksitleri_getir(Request $request,$sorgu,$musteriid)
    {
        if($sorgu===''){
           ...
           DB::raw('CASE WHEN (SELECT taksit_vadeleri.vade_tarih FROM taksit_vadeleri WHERE taksit_vadeleri.odendi=false AND taksit_vadeleri.taksitli_tahsilat_id = taksitli_tahsilatlar.id
            ...
            DB::raw('CONCAT("<a href=\"#\" name=\"taksit_vadeleri\"  data-value=\"",taksitli_tahsilatlar.id,"\" class=\"btn btn-primary\">Detaylar</a> <a title=\"Tahsil Et\" href=\"/isletmeyonetim/tahsilat/",taksitli_tahsilatlar.user_id,"\"  data-value=\"",taksitli_tahsilatlar.id,"\" class=\"btn btn-success\"><i class=\"fa fa-money\" </a>") as islemler')
     ...
     public function pakettahsilatagit(Request $request){
        ...
        foreach($bilgi as $key => $paket)
        {
            ...
            $adisyon_paket_id = self::adisyona_paket_ekle($adisyon_id,$paket->id,$toplamtutar,$request->paket_satis_seans_baslangic[$key],$request->paket_satis_seans_araligi[$key],Personeller::where('salon_id',$request->sube)->where('yetkili_id',Auth::user()->id)->value('id'),null,null);
            $seanstarih = $request->paket_satis_seans_baslangic[$key];
            $paket_mevcut = Paketler::where('id',$paket->id)->first();
            $toplam_seans_sayilari = $paket_mevcut->hizmetler->sum('seans');
        ...
        return $request->paket_satis_musteri_id;
    ...
    public function uruntahsilatagit(Request $request){
        ...
        foreach($bilgi as $key => $urun)
        {
           ...
           $adisyon_urun = new AdisyonUrunler();
                ...
                $adisyon_urun->personel_id = Personeller::where('salon_id',$request->sube)->where('yetkili_id',Auth::user()->id)->value('id');
                $adisyon_urun->save();


           
        }
        return $request->urun_satis_musteri_id;
    }
    ...
    public function ajanda(Request $request){ 
        ...
        return view('isletmeadmin.ajanda',['bildirimler'=>self::bildirimgetir($request),'ajanda'=>$ajanda_liste, 'paketler'=>$paketler,'sayfa_baslik'=>'Ajanda','pageindex' => 40,'isletme'=>$isletme,'kalan_uyelik_suresi'=>$kalan_uyelik_suresi]);
    }
            ...
            DB::raw('CASE WHEN ajanda.ajanda_durum=1 THEN CONCAT("<div class=\"dropdown\">
               ...
               WHEN  ajanda.ajanda_durum=0 THEN CONCAT("<div class=\"dropdown\">
                ...
                <div class=\"dropdown-menu dropdown-menu-right dropdown-menu-icon-list\">
                   ...
                   END AS islemler')
        )->where('ajanda.aktif',true)->orderBy('ajanda.id','desc')->get();
       return array(
                'status' => $returntext,
                'ajanda' => $ajanda,
    ...
    public function ajandaya_yeni_not_ekle(Request $request){
        ...
        $yeninot->ajanda_olusturan=Personeller::where('salon_id',$request->sube)->where('yetkili_id',Auth::user()->id)->value('id');
        $yeninot->aktif=true;
        $yeninot->save();
    ...
    public function ajanda_guncelle(Request $request){
      ...
      $ajandanot = Ajanda::where('id',$request->ajanda_id_duzenle)->first();
        ...
        $ajandanot->ajanda_olusturan=Personeller::where('salon_id',$request->sube)->where('yetkili_id',Auth::user()->id)->value('id');
