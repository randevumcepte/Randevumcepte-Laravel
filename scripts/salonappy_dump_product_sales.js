// === Salonappy Urun Satislari Dump (sadece product_sales) ===
// Cikti: salonappy_product_sales_<ts>.json
// Schema: { productSales: [...], clients: [...], payments: [...], generated_at }
// Kullanim: php artisan salonappy:import --dump-file=... --salon=X --only-product-sales
//           ardindan --only-product-payments ile tahsilatlar.
(async () => {
  ...
  // 2) Urun satislari pagination
  console.log('🔹 Urun satislari cekiliyor (/product_sale/list_v2)...');
  ...
  while (true) {
    ...
    offset += arr.length;
    if (offset % 500 === 0) console.log(`  kumule: ${productSales.length}`);
    await sleep(RATE_DELAY_MS);
    if (arr.length < limit) break;
  }
  console.log(`✓ Toplam urun satisi satir: ${productSales.length}`);

  // 3) Payments (source_text="Urun satisi" olanlar tahsilat eslestirmesi icin)
  ...
  // 4) Dump indir
  const dump = { generated_at: new Date().toISOString(), clients, productSales, payments };
  const txt = JSON.stringify(dump);
  console.log(`Boyut: ${(txt.length/1024/1024).toFixed(2)} MB`);
  const blob = new Blob([txt], { type: 'application/json' });
  const a = document.createElement('a');
  a.href = URL.createObjectURL(blob);
  a.download = 'salonappy_product_sales_' + Date.now() + '.json';
