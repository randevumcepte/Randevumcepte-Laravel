...
// REGRESYON: personelin gün-bazlı çalışma-saati KAYDI YOKKEN salon saatleri baz alınmalı
// (aksi halde tüm slotlar yanlışlıkla bloklanır — canlıda yaşanan hata)
sifirla();
DB::table('personel_calisma_saatleri')->truncate(); // hiçbir personelde çalışma kaydı yok
$bos = musaitSaatler([10], [100]);
ok(count($bos) > 0, 'Personel çalışma-saati kaydı YOKKEN slotlar gelir (salon saatleri baz)');
ok(in_array('09:00', $bos), 'Kayıt yokken 09:00 açık (salon 09:00 açılış)');
sifirla();
DB::table('personel_calisma_saatleri')->truncate();
$bos = musaitSaatler([10, 20], [101, 100]); // çok personelli de kayıtsız çalışmalı
ok(count($bos) > 0, 'Çok personelli + çalışma kaydı yok: yine slotlar gelir (salon saatleri baz)');
