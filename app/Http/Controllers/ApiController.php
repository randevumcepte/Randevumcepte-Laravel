...
private function formatAdisyonFast($adisyon, $isletmeId, &$odenenToplamTutar, &$kalanToplamTutar, $toplamSatisSayisi, $alacak = null, $sonTahsilatTarihi = null) {
    ...
    private function _buildHourlyAnalysis($salonId, $curStart, $curEnd, $period, array $hourlyRaw)
    {
        ...
        // Tum bloklari hesaplayip dolu/bos ayrimi yapmadan donelim;
        // kullanici da hangi blogun ne durumda oldugunu gorsun (her zaman 3 kart).
        foreach ($blocks as $blk) {
            $from = max($blk['from'], $workStart);
            $to   = min($blk['to'],   $workEnd);
            if ($to - $from < 1) { continue; } // calisma saati disi blok
            ...
            elseif ($avg < 0.75)  { $disc = 0;  $sev = 'Normal'; }
            else                  { $disc = 0;  $sev = 'Yoğun'; }
            ...
            $msg = $activeCampaign
                ...
                : ($disc > 0
                    ...
                    : sprintf(
                        '%s saatleri (%02d:00-%02d:00) ortalama %%%d dolulukta. Kampanya önerisine ihtiyaç yok.',
                        $blk['label'], $from, $to, (int) round($avg * 100)
                    )
                );
                    ...
                    if($yenirandevu->user_id){
                        try {
                            NotificationService::toCustomer((int)$yenirandevu->user_id, (int)$yenirandevu->salon_id)
                                ...
                                ->send();
                        } catch (\Throwable $e) { \Log::warning('api musteri yeni randevu push: '.$e->getMessage()); }
                    }
...
public function cakisan_randevu_kontrol(Request $request, $randevu_tarihleri)
{
    ...
    'condition_result' =>
        ...
        if ($kvkkAyari && $request->kvkkKodu != $olusturulanOnayKodu && (!$portfoy->kvkk_onay_alindi || $portfoy->kvkk_onay_alindi == null)) {
            ...
            if (
                ...
                // SeansBildirimService'de merkezi; StoreAdminController da aynisini
                // cagiriyor (web admin'in "Geldi" butonu) — boylece iki taraf senkron.
                try {
                    app(\App\Services\SeansBildirimService::class)->bilgilendir($randevu);
                } catch (\Throwable $e) {
                    \Log::warning('[SEANS-KULLANIM] push fail (api)', [
    ...
    public function randevuiptalet(Request $request)
    {
                ...
                if($hizmet->personel_id){
                    try {
                        NotificationService::toStaff((int)$hizmet->personel_id, (int)$randevu->salon_id)
                            ...
                            ->extra(['islem' => 'red'])
                            ->send();
                    } catch (\Throwable $e) { \Log::warning('api personel red push: '.$e->getMessage()); }
                }
            }
            ...
            foreach ($randevu->hizmetler as $hizmet) {
                ...
                if($hizmet->personel_id){
                    try {
                        NotificationService::toStaff((int)$hizmet->personel_id, (int)$randevu->salon_id)
                            ->type(NotificationTypes::APPOINTMENT_CANCELLED)
                            ->title($randevu->durum == 3 ? 'Müşteri Randevuyu İptal Etti' : 'Randevu İptal Edildi')
                            ->body($mesaj)
                            ->randevu((int)$randevu->id)
                            ->deepLink('appointment_detail', ['randevu_id' => $randevu->id])
                            ->extra(['iptal_eden' => $randevu->durum == 3 ? 'musteri' : 'salon'])
                            ->send();
                    } catch (\Throwable $e) { \Log::warning('api personel iptal push: '.$e->getMessage()); }
                }
            }
        ...
        // Bildirim metni SMS metniyle birebir ($musteriRedMesaji / $musteriIptalMesaji yukarida olusturuldu)
        if ($request->durum != 3) {
            try {
                $musteriBildirimMesaji = $red ? ($musteriRedMesaji ?? '') : ($musteriIptalMesaji ?? '');
                if($musteriBildirimMesaji && $randevu->user_id){
                    NotificationService::toCustomer((int)$randevu->user_id, (int)$randevu->salon_id)
                        ->type(NotificationTypes::APPOINTMENT_CANCELLED)
                        ->title($red ? 'Randevu Talebiniz Reddedildi' : 'Randevunuz İptal Edildi')
                        ->body($musteriBildirimMesaji)
                        ->randevu((int)$randevu->id)
                        ->deepLink('appointment_detail', ['randevu_id' => $randevu->id])
     ...
     public function bildirimkimligiekleguncelle(Request $request)
{
...

    {
        ...
        if (SalonSMSAyarlari::where("ayar_id", 2)->where("salon_id", $randevu->salon_id)->value("musteri") == 1) {
            array_push($mesajlar, ["to" => $randevu->users->cep_telefon, "message" => $musteriOnayMesaji]);
        }
            ...
            if($hizmet->personel_id){
                try {
                    NotificationService::toStaff((int)$hizmet->personel_id, (int)$randevu->salon_id)
                        ...
                        ->deepLink('appointment_detail', ['randevu_id' => $randevu->id])
                        ->send();
                } catch (\Throwable $e) { \Log::warning('api personel onay push: '.$e->getMessage()); }
            }
                    ...
                    if($hizmet->personel_id){
                        try {
                            NotificationService::toStaff((int)$hizmet->personel_id, (int)$randevu->salon_id)
                                ...
                                ->deepLink('appointment_detail', ['randevu_id' => $randevu->id])
                                ->send();
                        } catch (\Throwable $e) { \Log::warning('api asistan guncelleme push: '.$e->getMessage()); }
                    }
                    ...
                    if($hizmet->personel_id){
                        try {
                            NotificationService::toStaff((int)$hizmet->personel_id, (int)$randevu->salon_id)
                                ...
                                ->deepLink('appointment_detail', ['randevu_id' => $randevu->id])
                                ->send();
                        } catch (\Throwable $e) { \Log::warning('api randevu talebi push: '.$e->getMessage()); }
                    }
                    ...
                    if($hizmet->personel_id){
                        try {
                            NotificationService::toStaff((int)$hizmet->personel_id, (int)$randevu->salon_id)
                                ...
                                ->randevu((int)$randevu->id)
                                ->deepLink('appointment_detail', ['randevu_id' => $randevu->id])
                                ->extra(['iptal_eden' => 'musteri'])
                                ->send();
                        } catch (\Throwable $e) { \Log::warning('api musteri iptal push: '.$e->getMessage()); }
                    }
    ...
    public function adisyonSil(Request $request)
   {
    ...
    public function carkHatirlatmaKaydetApi(Request $request, $salonId)
    {
        ...
        if (\Schema::hasColumn('cark_hatirlatma_ayarlari', 'aktif_1')) {
            ...
            // Eski 4. slot artik kullanilmiyor — defensive 0
            $a->aktif_son = 0;
        }
        // Salon-ozelinde basliklar (organizasyon icin admin UI'da gorunur)
        if (\Schema::hasColumn('cark_hatirlatma_ayarlari', 'baslik_1')) {
            $a->baslik_1  = trim((string) $request->input('baslik_1', '')) ?: null;
            $a->baslik_2  = trim((string) $request->input('baslik_2', '')) ?: null;
            $a->baslik_3  = trim((string) $request->input('baslik_3', '')) ?: null;
            $a->altyazi_1 = trim((string) $request->input('altyazi_1', '')) ?: null;
            $a->altyazi_2 = trim((string) $request->input('altyazi_2', '')) ?: null;
            $a->altyazi_3 = trim((string) $request->input('altyazi_3', '')) ?: null;
        }
