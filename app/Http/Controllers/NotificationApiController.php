...
class NotificationApiController extends Controller
{
     ...
     * POST /api/v1/bildirim/test-cihaz
     *
     * Bildirim akisini uctan uca test eden endpoint. Uygulama icinde "bildirimler
     * calismiyor" tarzi supheli durumda: verilen user_id + app_bundle icin
     * bildirim_kimlikleri kayitlarini listeler, ilgili brand'in ilk salon_id'sini
     * cozer ve NotificationService::forTokens ile push atar. Yanit: kayit
     * listesi + send() sonucu (sent/failed/total).
     *
     * Body:
     *  - user_id     : hedef musteri user_id (zorunlu)
     *  - app_bundle  : brand build bundle (zorunlu, orn: com.randevumcepte.salooncadde)
     *  - baslik      : opsiyonel, default 'Test Bildirimi'
     *  - mesaj       : opsiyonel, default 'Bu bir test bildirimidir.'
     *
     * Musteri disi test (personel/yetkili) icin ayni endpoint kullanici_tipi
     * parametresi ile genisletilebilir; simdilik en yaygin dert olan musteri
     * push testi hedeflendi.
     */
    public function testCihaz(Request $request)
    {
        $userId = (int) $request->input('user_id');
        $appBundle = trim((string) $request->input('app_bundle'));
        $baslik = $request->input('baslik', 'Test Bildirimi');
        $mesaj  = $request->input('mesaj', 'Bu bir test bildirimidir.');

        if ($userId <= 0 || $appBundle === '') {
            return response()->json([
                'success' => false,
                'message' => 'user_id ve app_bundle zorunludur.',
            ], 400);
        }

        // 1) Cihaz kayitlari (bilgi amacli listelenir, tanida yardimci)
        $kayitlar = BildirimKimlikleri::where('user_id', $userId)
            ->where('app_bundle', $appBundle)
            ->where('aktif', true)
            ->whereNotNull('bildirim_id')
            ->where('bildirim_id', '!=', '')
            ->get(['id', 'kullanici_tipi', 'salon_id', 'app_bundle', 'platform', 'token_tipi', 'cihaz', 'gonderim_hatalari', 'created_at', 'updated_at', 'bildirim_id']);

        $cihazlar = $kayitlar->map(function ($r) {
            return [
                'id'                => (int) $r->id,
                'kullanici_tipi'    => $r->kullanici_tipi,
                'salon_id'          => $r->salon_id,
                'app_bundle'        => $r->app_bundle,
                'platform'          => $r->platform,
                'token_tipi'        => $r->token_tipi,
                'cihaz'             => $r->cihaz,
                'gonderim_hatalari' => $r->gonderim_hatalari,
                'token_kisa'        => substr((string) $r->bildirim_id, 0, 25),
                'created_at'        => (string) $r->created_at,
                'updated_at'        => (string) $r->updated_at,
            ];
        });

        // 2) Brand -> salon eslemesi (ilk aktif salon). forTokens'a salon_id
        // gecmezsek app_bundle filtresi de calismaz, inbox kaydi da salonsuz olur.
        $salonId = \App\Salonlar::where('app_bundle', $appBundle)
            ->orderBy('id')
            ->value('id');

        // 3) Cihaz yoksa erken donerek net hata mesaji ver
        if ($kayitlar->isEmpty()) {
            return response()->json([
                'success' => false,
                'message' => 'Bu user_id + app_bundle kombinasyonu icin aktif cihaz kaydi yok. Once uygulamaya giris yapip cihaz kaydinin olusmasini saglayin.',
                'salon_id_cozumlenen' => $salonId,
                'cihazlar' => [],
                'push_sonuc' => null,
            ], 200);
        }

        // 4) Push at (forTokens raw mod -> findTokens icinde ayrica app_bundle
        // filtresi de var, cihazlar zaten dogru brand'de)
        $tokens = $kayitlar->pluck('bildirim_id')->filter()->values()->toArray();

        try {
            $sonuc = NotificationService::forTokens($tokens, $salonId ? (int) $salonId : null)
                ->type(NotificationTypes::SYSTEM_ANNOUNCEMENT)
                ->title($baslik)
                ->body($mesaj)
                ->send();
        } catch (\Throwable $e) {
            Log::warning('[BILDIRIM-TEST] send exception: ' . $e->getMessage());
            return response()->json([
                'success' => false,
                'message' => 'Push gonderim sirasinda hata: ' . $e->getMessage(),
                'salon_id_cozumlenen' => $salonId,
                'cihazlar' => $cihazlar,
                'push_sonuc' => null,
            ], 500);
