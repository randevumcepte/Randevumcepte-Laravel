 <script type="text/javascript">
     $(document).ready(function () {
        ...
        $('#musteri_arama').select2({
             placeholder: "Hasta arayın", // Placeholder ekleme
             language: {
 ...
 @if($pageindex == 2)
         <script type="text/javascript">
            ...
            if ($(window).width() < 768) { 
                // Mobil cihaz
                calendarHeight = 'auto'; // veya window.innerHeight
            } else {
                // Masaüstü
            ...
            viewRender: function(view, element) {
                ...
                switch(view.name) {
                    case 'agendaDay':
                        countText = "Günlük Randevu: " + <?php echo $randevular['randevu_sayisi']; ?>;
                        break;
                    case 'agendaWeek':
                        case 'basicWeek':
                        case 'week':
                        case 'listWeek':
                        case 'timelineWeek':
                            countText = "Haftalık Randevu: " + <?php echo $randevular['randevu_sayisi']; ?>;
                            break;
                    case 'month':
                    case 'basicMonth':
                    case 'listMonth':
                    case 'timelineMonth':
                        countText = "Aylık Randevu: " + <?php echo $randevular['randevu_sayisi']; ?>;
                        break;
                    default:
                        countText = "Günlük Randevu: " + <?php echo $randevular['randevu_sayisi']; ?>;
                }
        ...
        if (event.userid == 2012) {
            swal({
                ...
                if (result.value) {
                    kapalisaatsil(event.randevu_id);
                }
            });
            return false;
        }
            ...
            $(document).ready(function () {
                 ...
                 $.ajax({
                    ...
                    data: {
                        sube: $('input[name="sube"]').val(),
                        tur: '',
                        musteri_id: <?php echo $musteri_bilgi->id; ?>,
                        tariharaligi: $('#adisyon_tarihe_gore_filtre').val()
                    },
                    success: function (json) {
                        console.log(json);
                        $('#toplamOdenen').empty();
                        $('#toplamBorc').empty();
                        $('#toplamOdenen').text("Toplam Ödenen : " + json.odenen + ' ₺');
                        $('#toplamBorc').text("Toplam Borç   : " + json.kalan + ' ₺');
                    },
                    error: function(xhr, error, code) {
                       ...
                       "deferRender": true, // DOM yükünü azaltır!
                        "ajax": {
                            ...
                            dataSrc: function (json) {
                                console.log('İşlem Sayısı '+json.islemsayisi);
                                if(json.islemsayisi && json.urunsayisi && json.paketsayisi)
                                {
                                    $('#satis_takibi_islem_sayisi').empty();
                                    $('#satis_takibi_urun_sayisi').empty();
                                     $('#satis_takibi_paket_sayisi').empty();
                                      $('#satis_takibi_islem_sayisi').append('('+json.islemsayisi+')');
                                    $('#satis_takibi_urun_sayisi').append('('+json.urunsayisi+')');
                                     $('#satis_takibi_paket_sayisi').append('('+json.paketsayisi+')');
                                }
                                return json.data;
                            },
                            "error": function(xhr, error, code) {
               ...
               $('#cihaz_tablo').DataTable({
                 ordering: false,
                        ...
                        responsive: true,
                                ...
                                data: <?php echo $cihazlar["cihazlar"];?>,
                                "language" : {
                                    "url" : "//cdn.datatables.net/plug-ins/1.10.20/i18n/Turkish.json",
                                    searchPlaceholder: "Ara",
                                    paginate: {
