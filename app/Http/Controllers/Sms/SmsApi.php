<?php
	...
	require_once('GetSmsGatewayResponse.php');
	...
	class SmsApi {
	    ...
	    function getSenders(){
  		...
  		function getGateWays(){
	    	$response = new GetSmsGatewayResponse();
			...
			try {
				...
				$ch = curl_init();
				curl_setopt($ch, CURLOPT_URL, $this->host.'sms/list-gateway');
				curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "POST");
				curl_setopt($ch, CURLOPT_POSTFIELDS, "");
				curl_setopt($ch, CURLOPT_TIMEOUT, 120); 
				curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, 0);
				...
				if($hasError == ''){
					if(is_null($result['err'])){
						...
						foreach($data as $item){
							$temp = new SmsGatewayItem();
							$temp->uuid = $item['uuid'];
							$temp->smsType = $item['smsType'];
							$temp->operator = $item['operator'];
							$temp->subProvider = $item['subProvider'];
							$temp->isOTP = $item['isOTP'];
							$temp->simChangeControl = $item['simChangeControl'];
							
							$gateways[] = $temp;
						}

						$response->list = $gateways;
