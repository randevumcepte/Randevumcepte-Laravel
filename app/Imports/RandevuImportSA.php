...
class RandevuImportSA implements ToCollection, WithHeadingRow
{
    ...
    public function collection(Collection $rows)
    {
        foreach ($rows as $row) {
            ...
            //$adisyonId = self::yeni_adisyon_olustur($musteri->id,$this->sube,'',$randevuTarihi,$randevu->olusturan_personel_id);
            ...
            foreach($hizmetler as $hizmet)
            {
                ...
                {
                    $sHizmet = new SalonHizmetler();
                    $sHizmet->sure_dk = 30;
                    $sHizmet->hizmet_id = $hizmetVarmi->id;
                    $sHizmet->hizmet_kategori_id = $hizmetVarmi->hizmet_kategori_id;
                    $sHizmet->save();
                }
                elseif(!$hizmetVarmi)
                {
                    ...
                    $sHizmet = new SalonHizmetler();
                    $sHizmet->sure_dk = 30;
                    $sHizmet->hizmet_id = $yHizmet->id;
                    $sHizmet->hizmet_kategori_id = $yHizmet->hizmet_kategori_id;
                    $sHizmet->save();
                ...
                echo $baslangicSaat ." - ".$bitisSaat." <br>";
                /*$fiyat = 0;
                if ($hizmet === end($hizmetler)) {
                     $fiyat = str_replace(" TL","",$row['toplam_hizmet_fiyati']);
                }
                 
                
                $adisyonHizmet = self::adisyon_hizmet_ekle($adisyonId,$rHimzet->hizmet_id,$randevu->tarih,$randevu->saat,$rHimzet->sure_dk,$fiyat,$randevu->randevuya_geldi,$rHimzet->personel_id,$rHimzet->cihaz_id,'','',$randevu->id);*/

            }
            /*$odenenTutar = str_replace(' TL','',$row['kapora']);
            ...
            if($odenenTutar > 0)
            {
                ...
                $tahsilat->odeme_tarihi = $randevu->tarih;
                ...
                foreach($adisyon->hizmetler as $hizmet)
                {
                     ...
                     $tahsilatHizmet->tahsilat_id = $tahsilat->id;
                     $tahsilatHizmet->adisyon_hizmet_id =$hizmet->id;
                     $tahsilatHizmet->tutar = $hizmet->fiyat != 0 ? $odenenTutar : 0;
                     $tahsilatHizmet->salon_id = $this->sube;
                     $tahsilatHizmet->user_id = $musteri->id;
    ...
    public function yeni_adisyon_olustur($musteriid,$salonid,$adisyonnotu,$tarih,$olusturan)
    {
        ...
        $adisyon->olusturan_id =Personeller::where('id',$olusturan)->value('yetkili_id');
        $adisyon->tarih = $tarih;
        $adisyon->save();
        return $adisyon->id;
    }
