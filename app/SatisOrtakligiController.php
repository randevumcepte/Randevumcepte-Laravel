   ...
   public function yeni_musteri_ekle(Request $request)
    {
        ...
        $existingCustomer = Salonlar::where('yetkili_telefon', $request->yetkili_telefon)->count();
        
        if ($existingCustomer>0) {
            // Return a warning response
            return response()->json([
                'status' => 'warning',
                'message' => 'Bu müşteri zaten sistemde kayıtlı!'
            ], 200);
            exit;
        }
        ...
        return [
            ...
            'islemler' => '<div class="dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-right dropdown-menu-arrow">
                          ...
                          <a class="dropdown-item" name="satisyap" data-value="'.$musteri->id.'" href="#" data-toggle="modal" data-target="#satis-formu">Satış Yap</a>
                           <a class="dropdown-item" name="demouzat" data-value="'.$musteri->id.'" href="#" data-toggle="modal" >Demo Süresi Uzat</a>
                        </div>
                      </div>
                          '
        ];
    ...
    public function musteri_formunu_kaydet(Request $request){
        ...
        if($request->satis_odeme_turu == 1)
        {
            $mesaj = [
                [
                    ...
                    "message" => "Sayın ".$musteri->yetkili_adi.", ".$uyelik->uyelik_adi." Paketi ".$periyotyazi." hizmetinize ait ".$fiyat." TRY ödeme bilgileri aşağıdaki linkte yer almaktadır. Güvenli bir şekilde ödemenizi tamamlamak için lütfen linke tıklayın: https://webapp.randevumcepte.com.tr/odeme?satis_ortagi=".Auth::user()->id."&uyelikturu=".$str[0]."&periyot=".$str[1]."&sube=".$musteri->id
        ...
        if($request->satis_odeme_turu == 2)
        {
            $mesaj = [
                [
                    "to" => $musteri->yetkili_telefon,
                    "message" => "Sayın ".$musteri->yetkili_adi.", ".$musteri->salon->id." nolu üye referans numaralı ".$uyelik->uyelik_adi." Paket ".$periyotyazi." hizmetinize ait ".$fiyat." TRY ödeme bilgileri aşağıda belirtilmiştir. Ödemenizi banka havalesi/eft yoluyla aşağıdaki hesap bilgilerine yapabilirsiniz:\n\n
Banka Adı: Garanti BBVA\n
Hesap Adı: 1100-6297259 BAYRAKLI\n
