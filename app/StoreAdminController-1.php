...
class StoreAdminController extends Controller
{
    ...
    public function mevcutsube($request)
    {
        ...
        { 
            ...
            if($_SERVER['SERVER_NAME'] != 'webapp.randevumcepte.com.tr'){
    ...
    public function yetki_kontrol(Request $request)
    {
        ...
        if(DB::table('model_has_roles')->where('role_id',5)->where('model_id',Auth::user()->id)->where('salon_id',self::mevcutsube($request))->count() > 0)
        {
    ...
    public function randevular(Request $request){ 
         $isletmeler =  
             Auth::user()->yetkili_olunan_isletmeler->pluck('salon_id')->toArray();
        ...
        $randevular = '';
        if(DB::table('model_has_roles')->where('role_id',5)->where('model_id',Auth::user()->id)->where('salon_id',self::mevcutsube($request))->count() > 0|| Auth::user()->hasRole('Sosyal Medya Uzmanı'))
    ...
    public function randevuyukle(Request $request,$takvim_turu,$tarih1,$tarih2){
        ...
        if($takvim_turu == 1 || DB::table('model_has_roles')->where('role_id',5)->where('model_id',Auth::user()->id)->where('salon_id',self::mevcutsube($request))->count() > 0){
            if(DB::table('model_has_roles')->where('role_id',5)->where('model_id',Auth::user()->id)->where('salon_id',self::mevcutsube($request))->count() > 0)
        ...
        if($takvim_turu == 0 && DB::table('model_has_roles')->where('role_id',5)->where('model_id',Auth::user()->id)->where('salon_id',self::mevcutsube($request))->count() == 0){ 
            $randevu_hizmetler = DB::table('randevu_hizmetler')->
            join('randevular','randevu_hizmetler.randevu_id','=','randevular.id')
        ...
        if($takvim_turu == 2 && DB::table('model_has_roles')->where('role_id',5)->where('model_id',Auth::user()->id)->where('salon_id',self::mevcutsube($request))->count() == 0){
            $randevu_hizmetler = DB::table('randevu_hizmetler')->
            join('randevular','randevu_hizmetler.randevu_id','=','randevular.id')->
        ...
        if($takvim_turu == 3 && DB::table('model_has_roles')->where('role_id',5)->where('model_id',Auth::user()->id)->where('salon_id',self::mevcutsube($request))->count() == 0){
            $randevu_hizmetler = DB::table('randevu_hizmetler')->
            join('randevular','randevu_hizmetler.randevu_id','=','randevular.id')->
            ...
            if($cakisma_varmi == '' || $request->cakisanrandevuekle==1)
            {
                foreach($randevu_tarihleri as $tarihler){
                    ...
                    $yenirandevu = [
                            ...
                            'saat'=>$request->saat,
                            'salon'=>true,
                            'olusturan_personel_id'=>Auth::user()->id,
...
<input type="hidden" id="smstaslak'.$taslak->id.'" value="'.$taslak->taslak_icerik.'">
   ...
   leftjoin('adisyon_hizmetler','adisyon_hizmetler.adisyon_id','=','adisyonlar.id')
    ...
    DB::raw('CONCAT("<button class=\"btn btn-success btn-block\"  style=\"line-height:5px\">",FORMAT(COALESCE((SELECT COALESCE(SUM(tahsilat_hizmetler.tutar), 0) from tahsilat_hizmetler where tahsilat_hizmetler.adisyon_hizmet_id = adisyon_hizmetler.id) + 
            ...
            ->orHaving(DB::raw('(SELECT  DATE_ADD(adisyonlar.created_at , INTERVAL 3 MONTH) FROM adisyonlar where adisyonlar.user_id = users.id  order by adisyonlar.id desc limit 1)'),'>=',date('Y-m-d H:i:s', strtotime('+90 days', strtotime(date('Y-m-d H:i:s')))))->groupBy('users.id')->orderBy('users.id','desc');
   ...
   leftjoin('adisyon_hizmetler','adisyon_hizmetler.adisyon_id','=','adisyonlar.id')
    ...
    DB::raw('CONCAT("<button class=\"btn btn-success btn-block\"  style=\"line-height:5px\">",FORMAT(COALESCE((SELECT COALESCE(SUM(tahsilat_hizmetler.tutar), 0) from tahsilat_hizmetler where tahsilat_hizmetler.adisyon_hizmet_id = adisyon_hizmetler.id) + 
            ...
            ->having(DB::raw('(SELECT  DATE_ADD(adisyonlar.created_at , INTERVAL 3 MONTH) FROM adisyonlar where adisyonlar.user_id = users.id order by adisyonlar.id desc limit 1)'),'<',date('Y-m-d H:i:s', strtotime('+90 days', strtotime(date('Y-m-d H:i:s')))))->groupBy('users.id')->orderBy('users.id','desc');
 ...
 public function urun_liste_getir(Request $request,$returntext){
    ...
    public function tahsilatekle(Request $request){
        ...
        $tahsilat->olusturan_id = Personeller::where('salon_id',$request->sube)->where('yetkili_id',Auth::user()->id)->value('id');
    ...
    public function adisyonlar(Request $request){
        ...
        if(!isset($request->urun) && !isset($request->paket))
            $tur = '';
        if(DB::table('model_has_roles')->where('role_id',5)->where('model_id',Auth::user()->id)->where('salon_id',self::mevcutsube($request))->count() > 0)
            $personel_id = Personeller::where('salon_id',$isletme->id)->where('yetkili_id',Auth::user()->id)->value('id');
...
<input type="hidden" name="adisyon_urun_senet_id[]" value="'.$urun->senet_id.'">
    ...
    public function ongorusmeekleduzenle(Request $request)
    {
        ...
        else
        {
            ...
            $user->save();
            $portfoy = '';
            if(MusteriPortfoy::where('user_id',$user->id)->where('salon_id',$ongorusme->salon_id)->count()>0)
                $portfoy = MusteriPortfoy::where('user_id',$user->id)->where('salon_id',$ongorusme->salon_id)->first();
            else
                $portfoy = new MusteriPortfoy();
                ...
                if(SalonSMSAyarlari::where('ayar_id',12)->where('salon_id',$ongorusme->salon_id)->value('personel')==1)
                {
                    foreach($randevu->hizmetler as $hizmet)
                    {
                        ...
                        if($ongorusme->paket_id !== null)
                            $mesaj = $ongorusme->musteri->name." isimli müşterinin ". date('d.m.Y',strtotime($request->ongorusme_tarihi)) ." - ". date('H:i',strtotime($request->ongorusme_saati)) ." ".$ongorusme->paket->paket_adi." için ön görüşme randevusu randevusu ".Auth::user()->name." tarafından düzenlenmiştir.";
                        if($ongorusme->urun_id !== null)
                            $mesaj = $ongorusme->musteri->name." isimli müşterinin ". date('d.m.Y',strtotime($request->ongorusme_tarihi)) ." - ". date('H:i',strtotime($request->ongorusme_saati)) ." ".$ongorusme->urun->urun_adi." için ön görüşme randevusu randevusu ".Auth::user()->name." tarafından düzenlenmiştir.";
                        ...
                        self::bildirimekle($request,$ongorusme->salon_id,$mesaj,"#",$request->gorusmeyi_yapan,null, Auth::user()->profil_resim,$randevu->id);
    ...
    public function ongorusmegetir(Request $request,$gunluk){
        ...
        ->where(function ($q) use ($request){
            if(DB::table('model_has_roles')->where('role_id',5)->where('model_id',Auth::user()->id)->where('salon_id',self::mevcutsube($request))->count() > 0) 
                $q->where('salon_personelleri.id',Personeller::where('salon_id',self::mevcutsube($request))->where('yetkili_id',Auth::user()->id)->value('id'));
        })
            ...
            foreach($paket->hizmetler as $key2 => $hizmet2)
            {
                ...
                for($i=1;$i<=$hizmet2->seans;$i++)
                {
                    ...
                    if($key2==0||count($hizmete_ait_randevu)<$i)
                    {
                        ...
                        $seans_randevu->olusturan_personel_id = Auth::user()->id;  
    ...
    public function adisyon_paket_satis_getir($adisyon_id,$visibility,$paket_id)
    {
        ...
        foreach($satislar as $paket){
            $tahsilat_paket_eklenecek .= ' <div class="row" style="background:#cce5ff;margin:5px 0 5px 0; padding:5px;font-size:14px" data-value="0">
                        ...
                        ';
                        if(DB::table('model_has_roles')->where('role_id',5)->where('model_id',Auth::user()->id)->where('salon_id',$salon_id)->count() == 0)
                     ...
                     ';
                     if($visibility && $paket_id == $paket->id && DB::table('model_has_roles')->where('role_id',5)->where('model_id',Auth::user()->id)->where('salon_id',self::mevcutsube($request))->count() == 0)
            ...
            DB::raw('CASE WHEN ajanda.ajanda_durum=1 THEN CONCAT("<div class=\"dropdown\">
               ...
               WHEN  ajanda.ajanda_durum=0 THEN CONCAT("<div class=\"dropdown\">
                ...
                <div class=\"dropdown-menu dropdown-menu-right dropdown-menu-icon-list\">
                   ...
                   END AS islemler')
        )->where('ajanda.aktif',true)->where('ajanda.salon_id',self::mevcutsube($request))->where('ajanda.ajanda_olusturan',Personeller::where('salon_id',self::mevcutsube($request))->where('yetkili_id',Auth::user()->id)->value('id'))->orderBy('ajanda.id','desc')->get();
       return array(
                'status' => $returntext,
                'ajanda' => $ajanda,
 ...
 exit;
    ...
    public function formmusteribilgigetir(Request $request){
     ...
     public function arsivformekleme(Request $request){
        ...
        $form->form_olusturan=Personeller::where('salon_id',$request->sube)->where('yetkili_id',Auth::user()->id)->value('id');
    ...
    public function tahsilatbilgigetir(Request $request)
    {
        ...
        if(Adisyonlar::where('user_id',$request->musteriid)->where('salon_id',$isletme->id)->count()>3 
                              && 
                           date('Y-m-d H:i:s', strtotime('+90 days',strtotime(\App\Adisyonlar::where('user_id',$request->musteriid)->where('salon_id',$isletme->id)->orderBy('id','desc')->value('created_at')))) < date('Y-m-d H:i:s', strtotime('+90 days', strtotime(date('Y-m-d H:i:s')))))
        ...
        $senet_gelen_vadeler = SenetVadeleri::join('senetler','senet_vadeleri.senet_id','=','senetler.id')->select('senet_vadeleri.id as senet_vade_id','senet_vadeleri.vade_tarih as tarih','senet_vadeleri.tutar as tutar')->where('senetler.user_id',$request->musteriid)->where('senet_vadeleri.vade_tarih','<=',date('Y-m-d'))->where('odendi',false)->get();
        $taksit_gelen_vadeler = TaksitVadeleri::join('taksitli_tahsilatlar','taksit_vadeleri.taksitli_tahsilat_id','=','taksitli_tahsilatlar.id')->select('taksit_vadeleri.id as taksit_vade_id','taksit_vadeleri.vade_tarih as tarih','taksit_vadeleri.tutar as tutar')->where('taksitli_tahsilatlar.user_id',$request->musteriid)->where('taksit_vadeleri.vade_tarih','<=',date('Y-m-d'))->where('odendi',false)->get();
        $taksitler = '';
        $senetler = '';
    ...
    public function personeldropliste(Request $request,$personelid)
    {
        ...
        if(DB::table('model_has_roles')->where('role_id',5)->where('model_id',Auth::user()->id)->where('salon_id',self::mevcutsube($request))->count() > 0)
        {
            $personeldropliste .= '<option selected value="'.Personeller::where('salon_id',self::mevcutsube($request))->where('yetkili_id',Auth::user()->id)->value('id').'">'.Auth::user()->name.'</option>';
        }
        else{
