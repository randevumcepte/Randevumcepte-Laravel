...
class WhatsAppMesajFormat
{
    ...
    /**
     * Randevu BILGILENDIRME (WA) — sesli asistan "randevum ne zaman" sorgusunda musteriye
     * yaklasan randevusunun bilgileri gonderilir. Diger randevu bildirimleriyle AYNI gorunum
     * (emoji + markdown + konum + tel), farkli baslik.
     */
    public static function randevuBilgilendirme($salon, $musteriAdi, $tarihStr, $saatStr, $hizmetlerStr = '')
    {
        $hizmetIkon = self::hizmetIkon($salon);

        $mesaj  = "🌸 Merhaba, *" . $musteriAdi . "*,\n\n";
        $mesaj .= ($salon->salon_adi ?? '') . " yaklaşan randevunuzun bilgileri aşağıdadır. 📋\n\n";
        $mesaj .= "📅 *Tarih:* " . $tarihStr . "\n";
        $mesaj .= "🕒 *Saat:* " . $saatStr . "\n";
        if (!empty($hizmetlerStr)) {
            $mesaj .= $hizmetIkon . " *Hizmetler:* " . $hizmetlerStr . "\n";
