...
class StoreAdminController extends Controller
{
    ...
    public function mevcutsube($request)
    {
        ...
        else
        {
            ...
            if($_SERVER['SERVER_NAME'] != 'app.randevumcepte.com.tr'){
    ...
    public function randevuyukle(Request $request,$takvim_turu,$tarih1,$tarih2){
        ...
        if($takvim_turu == 0 && DB::table('model_has_roles')->where('role_id',5)->where(function($q){$q->where('model_id',Auth::guard('satisortakligi')->user()->id);$q->orWhere('model_id',Auth::guard('isletmeyonetim')->user()->id);})->where('salon_id',self::mevcutsube($request))->count() == 0){
            $randevu_hizmetler = DB::table('randevu_hizmetler')->
            join('randevular','randevu_hizmetler.randevu_id','=','randevular.id')
            ->join('users','randevular.user_id','=','users.id')->
        ...
        if($takvim_turu == 2 && DB::table('model_has_roles')->where('role_id',5)->where(function($q){$q->where('model_id',Auth::guard('satisortakligi')->user()->id);$q->orWhere('model_id',Auth::guard('isletmeyonetim')->user()->id);})->where('salon_id',self::mevcutsube($request))->count() == 0){
            $randevu_hizmetler = DB::table('randevu_hizmetler')->
            join('randevular','randevu_hizmetler.randevu_id','=','randevular.id')->
            join('users','randevular.user_id','=','users.id')->
            join('hizmetler','randevu_hizmetler.hizmet_id','=','hizmetler.id')->
        ...
        if($takvim_turu == 3 && DB::table('model_has_roles')->where('role_id',5)->where(function($q){$q->where('model_id',Auth::guard('satisortakligi')->user()->id);$q->orWhere('model_id',Auth::guard('isletmeyonetim')->user()->id);})->where('salon_id',self::mevcutsube($request))->count() == 0){
            $randevu_hizmetler = DB::table('randevu_hizmetler')->
            join('randevular','randevu_hizmetler.randevu_id','=','randevular.id')->
            join('users','randevular.user_id','=','users.id')->
            join('hizmetler','randevu_hizmetler.hizmet_id','=','hizmetler.id')->
            ...
            )->where('randevular.salon_id',self::mevcutsube($request))->where('randevular.durum','<',2)->where('randevular.tarih','>=',$tarih1)->where('randevular.tarih','<=',$tarih2)->groupBy('hizmetgrubu')->where(function($q){ if(self::personelmi($request)) $q->where('randevu_hizmetler.personel_id',Auth::guard('isletmeyonetim')->user()->personel_id); })->get();
            ...
            DB::raw('CASE WHEN ajanda.ajanda_durum=1 THEN CONCAT("<div class=\"dropdown\">
               ...
               WHEN  ajanda.ajanda_durum=0 THEN CONCAT("<div class=\"dropdown\">
                ...
                <div class=\"dropdown-menu dropdown-menu-right dropdown-menu-icon-list\">
                   ...
                   END AS islemler')
        )->where('ajanda.aktif',true)->where('ajanda.salon_id',self::mevcutsube($request))->where('ajanda.ajanda_olusturan',Personeller::where('salon_id',self::mevcutsube($request))->where('yetkili_id',Auth::guard('isletmeyonetim')->check() ? Auth::guard('isletmeyonetim')->user()->id : Auth::guard('satisortakligi')->user()->id)->value('id'))->orderBy('ajanda.id','desc')->get();
       return array(
                'status' => $returntext,
                'ajanda' => $ajanda,
        );
