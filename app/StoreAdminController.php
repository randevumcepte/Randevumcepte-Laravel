...
class StoreAdminController extends Controller
{
    ...
    public function mevcutsube($request)
    {
        ...
        else
        {
            ...
            if($_SERVER['SERVER_NAME'] != 'app.randevumcepte.com.tr' && $_SERVER['SERVER_NAME'] != 'demoapp.randevumcepte.com.tr' && $_SERVER['SERVER_NAME'] != 'randevu.randevumcepte.com.tr' ){
                foreach($subeler as $sube_list)
                    if(Salonlar::where('domain',$_SERVER['SERVER_NAME'])->value('id') == $sube_list)
                        $sube = $sube_list;
            }
            else
                $sube = $subeler[0];
    ...
    {   
        ...
        $randevusayisi = 0;
        $musterisayisi = 0;
        $asistan_randevu_yarinki=self::randevu_liste_getir(
            $request,
        ...
        //$asistan_randevu_bugunluk=self::randevu_liste_getir($request,date('Y-m-d'),date('Y-m-d'),true,true,true,null,self::mevcutsube($request),'');
        ...
        $asistan_alacak_hatirlatmalari=json_encode(array());
        $asistan_alacak_hatirlatmalari_yarin=json_encode(array());
        $asistan_on_gorusmeler=json_encode(array());  
        $asistan_on_gorusmeler_yarin=json_encode(array());    
        
        $etkinlikler = json_encode(array());
        $on_gorusmeler = json_encode(array());
        $paketler = json_encode(array());
        $santral_raporlari = array();
        $musteridanisantext = self::musteridanisantext(self::mevcutsube($request));
        $easistanquery = '';
        ...
        if($isletme->uyelik_turu > 1)
        {
            ...
            $asistan_alacak_hatirlatmalari_yarin =
            DB::table('alacaklar')
            ...
            ->where('planlanan_odeme_tarihi', '=', date('Y-m-d', strtotime('+1 day')))
            ->get();*/
            if($isletme->uyelik_turu > 2){
                $easistanquery = self::easistandata($request,0);
    ...
    public function musteri_arama_bolumu_verileri(Request $request)
    {
            ...
            $query = DB::table('musteri_portfoy')->join('users','users.id','=','musteri_portfoy.user_id')->select('users.id as id',DB::raw('CONCAT(users.name," (",users.cep_telefon,")") as ad_soyad'),'users.cep_telefon as cep_telefon', DB::raw('CONCAT("/isletmeyonetim/musteridetay/", users.id,"?sube=",musteri_portfoy.salon_id) as detayli_bilgi'))->where(function($q) use($search){
                    $q->where('users.name', 'LIKE', "%{$search}%");
                    $q->orWhere('users.cep_telefon','LIKE',"%{$search}%");

            })->where('musteri_portfoy.salon_id', $salonid)->limit(50)->get();
            return response()->json($query);
    }
    public function personel_secimi(Request $request)
    {
        $search = $request->input('query');
        $salonid = $request->input('sube');
        $query = DB::table('salon_personelleri')->select('id as id','personel_adi as ad_soyad')->where('personel_adi', 'LIKE', "%{$search}%")->where('salon_id', $salonid)->where('aktif',true)->limit(50)->get();
        return response()->json($query);
    }
    public function hizmet_secimi(Request $request)
    {
        $search = $request->input('query');
        $salonid = $request->input('sube');
        $query = DB::table('salon_sunulan_hizmetler')->join('hizmetler','salon_sunulan_hizmetler.hizmet_id','=','hizmetler.id')->select('hizmetler.id as id','hizmetler.hizmet_adi as hizmet_adi')->where('hizmetler.hizmet_adi', 'LIKE', "%{$search}%")->where('salon_sunulan_hizmetler.salon_id', $salonid)->where('salon_sunulan_hizmetler.aktif',true)->limit(50)->get();
        return response()->json($query);
    }
    public function cihaz_secimi(Request $request)
    {
        $search = $request->input('query');
        $salonid = $request->input('sube');
        $query = DB::table('cihazlar')->select('id as id','cihaz_adi as cihaz_adi')->where('cihaz_adi', 'LIKE', "%{$search}%")->where('salon_id', $salonid)->limit(50)->get();
        return response()->json($query);
    }
    public function oda_secimi(Request $request)
    {
        $search = $request->input('query');
        $salonid = $request->input('sube');
        $query = DB::table('odalar')->select('id as id','oda_adi as oda_adi')->where('oda_adi', 'LIKE', "%{$search}%")->where('salon_id', $salonid)->limit(50)->get();
        return response()->json($query);
    }
    ...
    public function avantajraporlar(Request $request){
         $isletmeler = '';
        $isletme='';
        if(Auth::guard('satisortakligi')->check())
        {
        ...
        return view ('isletmeadmin.kampanyaraporlar',['title' => 'Avantaj Raporları |randevumcepte.com.tr','pageindex' => 1052,'isletme' => $isletme,'avantajlar' => $avantajlar,'yetkiliolunanisletmeler'=>$isletmeler]);
    }
    public function toplusmsgonder(Request $request){
        ...
        return view('isletmeadmin.toplusmsgonder',['paketler'=>$paketler,'bildirimler'=>self::bildirimgetir($request),'title' => 'Toplu SMS Gönder','pageindex' => 106,'isletme'=>$isletme,'taslaklar'=>$taslaklar,'grup'=>$grup,'sms_ayarlari'=>$sms_ayarlari,'raporlar'=>$raporlar,'karaliste'=>$karaliste, 'sayfa_baslik'=>'SMS Yönetimi' , 'kalan_uyelik_suresi' => self::lisans_sure_kontrol($request),'urun_drop'=>self::urundropliste($request),'hizmet_drop'=>self::hizmetdropliste($request),'yetkiliolunanisletmeler'=>$isletmeler]);
    }
     public function e_asistan(Request $request){
        ...
        if(SalonEAsistanAyarlari::where('salon_id',self::mevcutsube($request))->count()==0)
        {
            for($i=1;$i<=8;$i++){
                ...
                $asistanAyar->salon_id=self::mevcutsube($request);
                $asistanAyar->ayar_id=$i;
                $asistanAyar->acik_kapali=1;
                $asistanAyar->save();
    ...
    public function smslistesi(){
        ...
        $listeler = SMSListeleri::where('user_id',Auth::guard('isletmeyonetim')->user()->id)->get();
         $isletme = Salonlar::where('id',Auth::guard('isletmeyonetim')->user()->salon_id)->first();
        return view('isletmeadmin.smslistesi',['title' => 'Toplu SMS Listelerim | randevumcepte.com.tr','pageindex' => 107,'isletme'=>$isletme,'listeler'=> $listeler, 'kalan_uyelik_suresi' => self::lisans_sure_kontrol($request),'portfoy_drop'=>self::musteriportfoydropliste($request),'urun_drop'=>self::urundropliste($request),'hizmet_drop'=>self::hizmetdropliste($request),
            'personel_drop'=>self::personeldropliste($request,array()),'yetkiliolunanisletmeler'=>$isletmeler]);
    }
    public function toplumailgonder(){
        ...
        $isletme = Salonlar::where('id',Auth::guard('isletmeyonetim')->user()->salon_id)->first();
        return view('isletmeadmin.toplumailgonder',['title' => 'Toplu Mail Gönder | randevumcepte.com.tr','pageindex' => 110,'isletme'=>$isletme, 'kalan_uyelik_suresi' => self::lisans_sure_kontrol($request),'portfoy_drop'=>self::musteriportfoydropliste($request),'urun_drop'=>self::urundropliste($request),'hizmet_drop'=>self::hizmetdropliste($request),
    ...
    public function ayarlar(Request $request){
       $isletmeler = '';
        ...
        if(!in_array(self::mevcutsube($request),$isletmeler ) ||  $_SERVER['SERVER_NAME']=='randevu.randevumcepte.com.tr')
        {
            return view('isletmeadmin.yetkisizerisim');
            exit(0);
        }
    ...
    public function randevuyukle(Request $request,$takvim_turu,$tarih1,$tarih2){
        $randevu_hizmetler = "";
        $resources = "";
        $personel_idler = Auth::guard('isletmeyonetim')->check() ? Auth::guard('isletmeyonetim')->user()->yetkili_olunan_isletmeler->where('salon_id',self::mevcutsube($request))->pluck('id')->toArray() : array();
        if($takvim_turu == 1 || self::personelmi($request)){
           ...
           DB::raw('CASE when randevular.user_id = 2012 THEN "Kapalı Saat" ELSE CONCAT(users.name, CASE WHEN (SELECT COUNT(*) FROM adisyon_paket_seanslar where adisyon_paket_seanslar.randevu_id = randevular.id) > 0 THEN " Paket Randevusu Detayları " ELSE "" END , CASE WHEN randevular.on_gorusme_id > 0 then " Ön Görüşme Randevusu Detayları" else "" end, CASE WHEN (SELECT COUNT(*) FROM adisyon_paket_seanslar where adisyon_paket_seanslar.randevu_id = randevular.id) = 0 AND randevular.on_gorusme_id is null THEN " Randevu Detayları" ELSE "" END ) END as modal_title'),
              ...
              DB::raw('CASE WHEN randevular.durum=0 then "#ffc107" WHEN randevular.user_id=2012 then "#000000" WHEN randevular.randevuya_geldi=0 then "#ff0000" WHEN randevular.randevuya_geldi=1 THEN "#008000" ELSE renk_duzenleri.renk END as color')
                ,'randevu_hizmetler.personel_id as resourceId')->where('randevular.salon_id',self::mevcutsube($request))->where('randevular.durum','<',2)->where(function($q) use($personel_idler,$request){ if(self::personelmi($request)) $q->whereIn('randevu_hizmetler.personel_id',$personel_idler); })->get();
            else
                  $randevu_hizmetler = DB::table('randevu_hizmetler')
            ->join('randevular','randevu_hizmetler.randevu_id','=','randevular.id')
            ->leftjoin('salon_personelleri','randevu_hizmetler.personel_id','=','salon_personelleri.id')
            ...
            ->select('randevular.id as randevu_id','randevular.user_id as userid','randevu_hizmetler.id as id',
                ...
                CONCAT(
                    ...
                    CASE WHEN (SELECT COUNT(*) FROM adisyon_paket_seanslar where adisyon_paket_seanslar.randevu_id = randevular.id) > 0 THEN " (PAKET) " ELSE "" END,
                    CASE WHEN randevular.on_gorusme_id > 0 then " (ÖN GÖRÜŞME) " else "" end,
                    CASE WHEN randevular.on_gorusme_id is not null THEN CONCAT("\nÖn Görüşme Nedeni:",COALESCE(paketler.paket_adi,""),COALESCE(urunler.urun_adi,"")) ELSE CONCAT("\n",COALESCE(hizmetler.hizmet_adi,""))END,
                    "\nOluşturan: ", CASE WHEN randevular.easistan=1 THEN "Asistan" WHEN randevular.web=1 THEN "Web" WHEN randevular.uygulama=1 THEN "Uygulama" WHEN randevular.salon=1 THEN isletmeyetkilileri.name END
                ) END as title'),
             DB::raw('CASE when randevular.user_id = 2012 THEN "Kapalı Saat" ELSE CONCAT(users.name, CASE WHEN (SELECT COUNT(*) FROM adisyon_paket_seanslar where adisyon_paket_seanslar.randevu_id = randevular.id) > 0 THEN " Paket Randevusu Detayları " ELSE "" END , CASE WHEN randevular.on_gorusme_id > 0 then " Ön Görüşme Randevusu Detayları" else "" end, CASE WHEN (SELECT COUNT(*) FROM adisyon_paket_seanslar where adisyon_paket_seanslar.randevu_id = randevular.id) = 0 AND randevular.on_gorusme_id is null THEN " Randevu Detayları" ELSE "" END ) END as modal_title'),
                    DB::raw('CASE WHEN randevular.on_gorusme_id is not null
                    then
             ...
             )->where('randevular.tarih','>=',$tarih1)->where('randevular.tarih','<=',$tarih2)->where('randevular.salon_id',self::mevcutsube($request))->where('randevular.durum','<',2)->where(function($q) use($personel_idler,$request){ if(self::personelmi($request)) $q->whereIn('randevu_hizmetler.personel_id',$personel_idler); })->get();
        ...
        if($takvim_turu == 0 && DB::table('model_has_roles')->where('role_id',5)->where(function($q){if(Auth::guard('satisortakligi')->check()) if(Auth::guard('satisortakligi')->check()) $q->where('model_id',Auth::guard('satisortakligi')->user()->id); if(Auth::guard('isletmeyonetim')->check()) $q->orWhere('model_id',Auth::guard('isletmeyonetim')->user()->id);})->where('salon_id',self::mevcutsube($request))->count() == 0){
            $randevu_hizmetler = DB::table('randevu_hizmetler')->
            join('randevular','randevu_hizmetler.randevu_id','=','randevular.id')
            ->join('users','randevular.user_id','=','users.id')->
            leftjoin('hizmetler','randevu_hizmetler.hizmet_id','=','hizmetler.id')->
        ...
        if($takvim_turu == 2 && DB::table('model_has_roles')->where('role_id',5)->where(function($q){if(Auth::guard('satisortakligi')->check()) $q->where('model_id',Auth::guard('satisortakligi')->user()->id);if(Auth::guard('isletmeyonetim')->check()) $q->orWhere('model_id',Auth::guard('isletmeyonetim')->user()->id);})->where('salon_id',self::mevcutsube($request))->count() == 0){
            $randevu_hizmetler = DB::table('randevu_hizmetler')->
            join('randevular','randevu_hizmetler.randevu_id','=','randevular.id')->
            join('users','randevular.user_id','=','users.id')->
            leftjoin('hizmetler','randevu_hizmetler.hizmet_id','=','hizmetler.id')->
        ...
        if($takvim_turu == 3 && DB::table('model_has_roles')->where('role_id',5)->where(function($q){if(Auth::guard('satisortakligi')->check()) $q->where('model_id',Auth::guard('satisortakligi')->user()->id);if(Auth::guard('isletmeyonetim')->check()) $q->orWhere('model_id',Auth::guard('isletmeyonetim')->user()->id);})->where('salon_id',self::mevcutsube($request))->count() == 0){
            $randevu_hizmetler = DB::table('randevu_hizmetler')->
            join('randevular','randevu_hizmetler.randevu_id','=','randevular.id')->
            join('users','randevular.user_id','=','users.id')->
            leftjoin('hizmetler','randevu_hizmetler.hizmet_id','=','hizmetler.id')->
            ...
            )->where('randevular.salon_id',self::mevcutsube($request))->where('randevular.durum','<',2)->where('randevular.tarih','>=',$tarih1)->where('randevular.tarih','<=',$tarih2)->groupBy('hizmetgrubu')->where(function($q) use($request){ if(self::personelmi($request)) $q->where('randevu_hizmetler.personel_id',Auth::guard('isletmeyonetim')->user()->personel_id); })->get();
        ...
        foreach(SalonCalismaSaatleri::where('salon_id',self::mevcutsube($request))->get() as $calisma_saati)
        {
    ...
    public function kasadefteri(Request $request){
       $isletmeler = '';
        ...
        return view('isletmeadmin.kasadefteri',['sayfa_baslik'=>'Kasa Raporu', 'title'=> 'Kasa Defteri | '.$isletme->salon_adi.' İşletme Yönetim Paneli',   'pageindex' => 103, 'bildirimler'=>self::bildirimgetir($request),'isletme'=>$isletme,'kasa'=>$kasa, 'kalan_uyelik_suresi' => self::lisans_sure_kontrol($request),'urun_drop'=>self::urundropliste($request),
            'yetkiliolunanisletmeler'=>$isletmeler]);
    }
    ...
    public function randevuguncelle(Request $request){
        ...
        if($cakisma_varmi != '' && !isset($request->cakisanrandevuekle))
        {
            ...
            if($cakisma_varmi == '' || $request->cakisanrandevuekle==1)
            {
                ...
                $randevu->hatirlatma_aramasi_yapildi = null;
                $randevu->hatirlatma_gorevi_iptal = null;
                $randevu->hatirlatma_ulasilamadi = null;

                $randevu->tekrar_arama_tarih_saat = "0000-00-00 00:00:00";
                $randevu->hatirlatma_gorevi_iptal = null;
                $randevu->arama_saat = null;
                $randevu->tekrar_arandi = null;
                $randevu->tekrar_aranacak = null;
    ...
    public function smslistedetay($listeid){
        ...
        $isletme = Salonlar::where('id',Auth::guard('isletmeyonetim')->user()->salon_id)->first();
        return view('isletmeadmin.smslistedetay',['title' => $listeadi.' SMS Liste Detayı | randevumcepte.com.tr','pageindex' => 107,'isletme'=>$isletme,'smslistedetaylari'=> $smslistedetaylari, 'kalan_uyelik_suresi' => self::lisans_sure_kontrol($request),'portfoy_drop'=>self::musteriportfoydropliste($request),'urun_drop'=>self::urundropliste($request),'hizmet_drop'=>self::hizmetdropliste($request),
            'personel_drop'=>self::personeldropliste($request,array()),'yetkiliolunanisletmeler'=>$isletmeler]);
    }
    ...
    public function smsraporlar(Request $request){
     ...
     public function smstaslakolarakkaydet(Request $request){
             ...
             $taslak->baslik=$request->sablon_adi;
...
<input type="hidden" id="smstaslak'.$taslak->id.'" value="'.$taslak->taslak_icerik.'">
    ...
    public function musteriliste(Request $request){
        ...
        $paketler = self::paket_liste_getir('',true,$request);
        return view('isletmeadmin.musteriler',[ 'bildirimler'=>self::bildirimgetir($request),'paketler'=>$paketler,'sayfa_baslik'=>'Müşteriler','pageindex' => 4,  'isletme'=> $isletme,'kalan_uyelik_suresi' => self::lisans_sure_kontrol($request),'yetkiliolunanisletmeler'=>$isletmeler]);
    }
   ...
   public function yenimusterilistesiekle(Request $request)
   {
    ...
    $resulthtml['sonuc'] = [];
    $resulthtml['liste'] = [];
    $resulthtml['toplammusteri'] = [];
    $portfoytablohtml = "";
    if ($request->hasFile('listedosyasi_yeni_musteri')) {
        $file = $request->file('listedosyasi_yeni_musteri');
        // Dosyayı belirli bir yere kaydetme
        $hedef = $file->storeAs('public/listeler', str_replace(' ', '_', $file->getClientOriginalName()));
        // Excel verilerini işle
        $import = new MusteriImport($_POST['sube']);
    ...
    public function musteridetay(Request $request,$id){
        ...
        return view('isletmeadmin.musteridetay',['bildirimler'=>self::bildirimgetir($request),'paketler'=>$paketler,'pageindex'=>41, 'sayfa_baslik'=> $portfoy->users->name,'isletme'=>$isletme,'portfoy'=>$portfoy,'arsiv'=>$form, 'musteri_bilgi'=>$portfoy->users,'randevular'=>$randevular,'adisyonlar'=>$adisyonlar, 'kalan_uyelik_suresi' => self::lisans_sure_kontrol($request),'arsiv_onayli'=>$form_onayli, 'arsiv_iptal'=>$form_iptal, 'arsiv_beklenen'=>$form_beklenen, 'arsiv_harici'=>$form_harici,'randevular_liste'=>$randevular_liste,'urun_drop'=>self::urundropliste($request),
            'islemler'=>$islemler,'yetkiliolunanisletmeler'=>$isletmeler]);
    }
 ...
 public function urun_liste_getir(Request $request,$returntext){
    ...
    $hizmet_liste = "";
      $hizmet_liste= DB::table('salon_sunulan_hizmetler')->
        ...
        ->select('hizmetler.hizmet_adi as hizmet_adi',DB::raw('GROUP_CONCAT(salon_personelleri.personel_adi ) as personel'),DB::raw('CONCAT("<div class=\"dropdown\">
                        <a class=\"btn btn-link font-24 p-0 line-height-1 no-arrow dropdown-toggle\"
                                  href=\"#\"
                                  role=\"button\"
                                  data-toggle=\"dropdown\"
                        ...
                        <div class=\"dropdown-menu dropdown-menu-right dropdown-menu-icon-list\">
                                ...
                                </div> </div>") AS islemler'))->where('salon_sunulan_hizmetler.salon_id',self::mevcutsube($request))->where('salon_sunulan_hizmetler.aktif',true)->groupBy('hizmetler.id')->get();
    ...
    public function hizmetsurefiyatgetir(Request $request){
        if($request->hizmet_id != "" && $request->hizmet_id != "0" || $request->hizmet_id != 0)
        {
            ...
            return json_encode(array('fiyat'=>$hizmet->baslangic_fiyat,'sure'=>$hizmet->sure_dk));
             ...
             return json_encode(array('fiyat'=>"",'sure'=>""));
    }
    ...
    public function adisyonhizmetekle(Request $request)
    {
            ...
            foreach($request->adisyonhizmetleriyeni as $key=>$adisyon_hizmet){
                ...
                for($i=1;$i<=$request->hizmetseanssayisi[$key];$i++){
                    ...
                    $seanstarih = "";
                    if($i == 1)
                        $seanstarih = date('Y-m-d',strtotime($request->islemtarihiyeni[$key]));
                    else
                        $seanstarih = date('Y-m-d',strtotime('+'.$request->hizmetseansperiyodu[$key].' days', strtotime($seanstarih)));
                    $request->tarih = $seanstarih;

                    $request->saat = date('H:i:s',strtotime($request->islemsaatiyeni[$key]));
                    $request->salon= 1;
                    $request->olusturan_user_id = Auth::guard('isletmeyonetim')->user()->id;
                    $request->durum = 1;
                    $request->salon_id = $request->sube;
                    $request->hizmetler = $request->adisyonhizmetleriyeni;
                    $request->hizmetSuresi = $request->adisyonhizmetsuresi;
                    $request->hizmetFiyati = $request->adisyonhizmetfiyati[$key];
                    $request->randevuPersonelleri = $request->adisyonhizmetpersonelleriyeni;

                    $randevuId = self::randevuEkle($request);

                    $adisyon_hizmet_seans = new AdisyonPaketSeanslar();
                    $adisyon_hizmet_seans->adisyon_hizmet_id = $adisyon_hizmet->id;
                    $adisyon_hizmet_seans->seans_no = $i;
                    $adisyon_hizmet_seans->randevu_id = $randevuId;
                    $adisyon_hizmet_seans->hizmet_id =$request->adisyonhizmetleriyeni[$key];
                    $adisyon_hizmet_seans->seans_tarih = $seanstarih;
                    $adisyon_hizmet_seans->seans_saat = date('H:i:s',strtotime($request->islemsaatiyeni[$key]));
    ...
    public function adisyonlar(Request $request){
        ...
        'taksitler_kapali'=>self::taksitleri_getir($request,1,''),
            ...
            ,'adisyonlar_paket'=>$adisyonlar_paket ,'urun_drop'=>self::urundropliste($request),
            'yetkiliolunanisletmeler'=>$isletmeler]);
    }
        public function testcases(Request $request)
    {
        ...
        /*$randevular = Randevular::where('salon_id',180)->where('user_id',18389)->get();
        $seans_lar = array();
        foreach($randevular as $randevu)
        {
            ...
            foreach($randevu->hizmetler as $key=> $hizmet)
            {
                ...
                $seanslar = DB::table('adisyon_paket_seanslar')->join('hizmetler','hizmetler.id','=','adisyon_paket_seanslar.hizmet_id')->join('adisyon_hizmetler','adisyon_hizmetler.id','=','adisyon_paket_seanslar.adisyon_hizmet_id')->join('adisyonlar','adisyon_hizmetler.adisyon_id','=','adisyonlar.id')->join('users','adisyonlar.user_id','=','users.id')->select(
                    'users.id as user_id',
                    'users.name as ad_soyad',
                    'hizmetler.hizmet_adi as hizmet_adi',
                    'hizmetler.id as hizmet_id',
                    'adisyon_paket_seanslar.oda_id as oda_id',
                    'adisyon_paket_seanslar.cihaz_id as cihaz_id',
                    'adisyon_paket_seanslar.id as seansId',
                    'adisyonlar.tarih as satis_tarihi',


                )->where('adisyonlar.salon_id',180)->where('adisyonlar.user_id',$randevu->user_id)->where('adisyon_paket_seanslar.hizmet_id',$hizmet->hizmet_id)->orderBy('adisyon_paket_seanslar.hizmet_id','asc')->orderBy('users.id','asc')->orderBy('adisyon_paket_seanslar.id','asc')->get();
                ...
                else
                {
                    ...
                    array_push($seans_lar,$seanslar);
                    /*echo $randevu->users->name ." ".$hizmet->hizmetler->hizmet_adi." seanslarına bakılacak.<br>";
                    echo json_encode($seanslar[$key]).'<br>';
                    if(isset($seanslar[$key])&& $randevu->tarih >= $seanslar[$key]->satis_tarihi)
                    {
                        
                        $seansD = AdisyonPaketSeanslar::where('id',$seanslar[$key]->seansId)->first();

                        $seansD->seans_no = $key+1;
                        $seansD->seans_tarih = $randevu->tarih;
                        $seansD->seans_saat = $randevu->saat;
                        $seansD->personel_id = $hizmet->personel_id;
                        $seansD->oda_id = $hizmet->oda_id;
                        $seansD->cihaz_id = $hizmet->cihaz_id;
                        $seansD->randevu_id = $randevu->id;
                        $seansD->geldi = $randevu->randevuya_geldi;
                        $seansD->aktarim_islendi = true;
                        $seansD->save();
                    }
        ...
        $seansHizmetleri = DB::table('adisyon_paket_seanslar')->join('hizmetler','hizmetler.id','=','adisyon_paket_seanslar.hizmet_id')->join('adisyon_hizmetler','adisyon_hizmetler.id','=','adisyon_paket_seanslar.adisyon_hizmet_id')->join('adisyonlar','adisyon_hizmetler.adisyon_id','=','adisyonlar.id')->join('users','adisyonlar.user_id','=','users.id')->select(
                    'users.id as user_id',
                    'users.name as ad_soyad',
                    'hizmetler.hizmet_adi as hizmet_adi',
                    'hizmetler.id as hizmet_id',
                    ...
                    'adisyon_paket_seanslar.id as seansId',
                    'adisyonlar.tarih as satis_tarihi',
        )->where('adisyonlar.salon_id',180)->where('adisyonlar.user_id',18389)->orderBy('adisyon_paket_seanslar.hizmet_id','asc')->orderBy('users.id','asc')->orderBy('adisyon_paket_seanslar.id','asc')->groupBy('hizmet_id')->pluck('hizmet_id')->toArray();
        ...
        $seanslar = DB::table('adisyon_paket_seanslar')->join('hizmetler','hizmetler.id','=','adisyon_paket_seanslar.hizmet_id')->join('adisyon_hizmetler','adisyon_hizmetler.id','=','adisyon_paket_seanslar.adisyon_hizmet_id')->join('adisyonlar','adisyon_hizmetler.adisyon_id','=','adisyonlar.id')->join('users','adisyonlar.user_id','=','users.id')->select(
                    ...
                    'adisyon_paket_seanslar.id as seansId',
                    'adisyonlar.tarih as satis_tarihi',
        )->where('adisyonlar.salon_id',180)->where('adisyonlar.user_id',18389)->orderBy('adisyon_paket_seanslar.hizmet_id','asc')->orderBy('users.id','asc')->orderBy('adisyon_paket_seanslar.id','asc')->groupBy('hizmet_id')->get();*/
         
        /*$randevular = Randevular::where('salon_id',180)->whereNotIn('user_id',[7645,18389])->where('durum',1)->get();
        ...
        foreach($randevular as $key => $randevu)
        {
            ...
            foreach($randevu->hizmetler as $hizmet)
            {
                if(!$hizmet->hizmet_seansa_isledi)
                {
                    echo $hizmet->hizmet_id." ".$hizmet->hizmetler->hizmet_adi ." işlenmemiş. Seanslara Bakılacak <br>";

                    $adisyonlar =Adisyonlar::where('user_id',$randevu->user_id)->where('salon_id',180)->where('tarih','<=',date('Y-m-d',strtotime($randevu->tarih)))->whereHas('hizmetler',function($q) use($hizmet){$q->where('hizmet_id',$hizmet->hizmet_id);})->get();

                    foreach($adisyonlar as $adisyon )
                    {


                        echo $hizmet->hizmetler->hizmet_adi." satışı bulundu Satış id:".$adisyon->id."<br>";

                        foreach($adisyon->hizmetler as $aHizmet)
                        {
                            if($aHizmet->hizmet_id == $hizmet->hizmet_id)
                            {
                                foreach($aHizmet->seanslar as $aSeans)
                                {
                                     
                                    if(!$aSeans->aktarim_islendi || $aSeans->aktarim_islendi === null ||$aSeans->aktarim_islendi==='')
                                    {
                                        $aSeans->id." işlenecek. Randevu tarihi : ".$randevu->tarih." Geldi mi : ".$randevu->randevuya_geldi." <br>";
                                        $aSeans->geldi = $randevu->randevuya_geldi;
                                        $aSeans->aktarim_islendi = true;
                                        $aSeans->cihaz_id = $hizmet->cihaz_id;
                                        $aSeans->personel_id = $hizmet->personel_id;
                                        $aSeans->oda_id = $hizmet->oda_id;
                                        $aSeans->seans_tarih = $randevu->tarih;
                                        $aSeans->seans_saat = $randevu->saat;
                                        $aSeans->randevu_id = $randevu->id;
                                        $aSeans->save();
                                        $hizmet->hizmet_seansa_isledi = true;
                                        $hizmet->save();
                                        break 3;
                                        echo $aHizmet->hizmet->hizmet_adi." seansı id ".$aSeans->id.". Aktarım İşlendi : ".$aSeans->aktarim_islendi."<br>";
                                    }
                                }
                            }
                    ...
                    /* $seans = AdisyonPaketSeanslar::join('adisyon_hizmetler','adisyon_hizmetler.id','=','adisyon_paket_seanslar.adisyon_hizmet_id')->join('adisyonlar','adisyonlar.id','=','adisyon_hizmetler.adisyon_id')
                    ->where('adisyonlar.user_id', $randevu->user_id)->where('adisyon_paket_seanslar.hizmet_id',$hizmet->hizmet_id)->whereNull('adisyon_paket_seanslar.aktarim_islendi')->where('adisyonlar.tarih','<=',date('Y-m-d',strtotime($randevu->tarih)))->orderBy('adisyon_paket_seanslar.id','asc')
                    ->first();
                                        if ($seans) {
                        echo "düzenlenecek seans id :".$seans->id."<br>";
                        
                        $seans->aktarim_islendi = true;
                        $seans->save();
                        
                        // Güncellenen seansı tekrar veritabanından çekerek kontrol edelim
                        $kontrolSeans = AdisyonPaketSeanslar::where('id', $seans->id)->first();
                        
                        if ($kontrolSeans->aktarim_islendi) {
                            echo "Seans güncellendi: " . $kontrolSeans->id . "<br>";
                        } else {
                            echo "Güncelleme başarısız: " . $kontrolSeans->id . "<br>";
                        }
        ...
        /*foreach($seanslar as $key=> $seans)
        {
            echo $seans->ad_soyad ." için ". $seans->hizmet_adi ."  ".$seans->hizmet_id .".seansına bakılıyor<br>";
            $randevular = Randevular::where('user_id',$seans->user_id)->where('salon_id',180)->whereHas('hizmetler.hizmetler',function($q) use($seans){
                $q->where('hizmet_adi',$seans->hizmet_adi);
            })->get();
            
            echo  $seans->hizmet_adi ." ".$seans->hizmet_id." için randevu sayısı : ".$randevular->count();
                ...
                foreach($randevular as $key=> $randevu)
                {
                    foreach($randevu->hizmetler as $hizmet)
                    {
                        echo $seans->ad_soyad ." için ".$seans->hizmet_adi." için harcanabilecek randevu tarihi : ".$randevu->tarih."<br>";
                        if($hizmet->hizmet_id == $seans->hizmet_id){
                            $oda_id = $hizmet->oda_id;
                            $cihaz_id = $hizmet->cihaz_id;
                            $personel_id = $hizmet->personel_id;
                            $randevu_id = $hizmet->randevu_id;
                            $geldi = $randevu->randevuya_geldi;
                            echo $seans->ad_soyad ." ".$randevu->tarih." tarihli randevusu seansa işlenecek<br>";
                            break 2;
                        }
                    }
                    $seansD = AdisyonPaketSeanslar::where('id',$seans->seansId)->whereNull('aktarim_islendi')->first();
                    $seansD->seans_tarih = $randevular[$key]->tarih;
                    $seansD->seans_saat = $randevular[$key]->saat;
                    $seansD->personel_id = $personel_id;
                    $seansD->oda_id = $oda_id;
                    $seansD->cihaz_id = $cihaz_id;
                    $seansD->randevu_id = $randevu_id;
                    $seansD->geldi = $geldi;
                    $seansD->aktarim_islendi = true;
                    $seansD->save();
                    echo $seans->ad_soyad ." ".$randevular[$key]->tarih." tarihli randevusu seansa işlendi<br><br>";
                }
                
               
            
        }
        echo "seanslar işlendi";*/

        /* $sonuclar = Adisyonlar::with(['musteri', 'hizmetler.seanslar'])
    ->where('salon_id',180)->get()
    ->groupBy('user_id')
    ->map(function ($adisyonlar) {
        $ilkAdisyon = $adisyonlar->first();
        $kullanici = $ilkAdisyon && $ilkAdisyon->musteri ? $ilkAdisyon->musteri : null;

        if (!$kullanici) {
            return null; // Eğer müşteri yoksa bu grubu atla
        }

        $hizmetler = $adisyonlar->flatMap(fn ($adisyon) => $adisyon->hizmetler)
            ->groupBy('hizmet_id')
            ->map(function ($hizmetler) use($kullanici,$adisyon) {
                 
                $randevular = Randevlar::where('user_id',$kullanici->id)->where('salon_id',180)->whereHas('hizmetler',function($q) use($hizmetler){
                    $q->where('hizmet_id',$hizmetler->first()->hizmet_id);
                })where('tarih','>=',$adisyon->tarih)->get();
                foreach($hizmetler->flatMap(fn ($hizmet) => $hizmet->seanslar)->pluck('id')->values() as $key=>$seans){
                    if(isset($randevular[$key])){
                        $seansD = AdisyonPaketSeanslar::where('id',$seans)->first();
                        if($seansD->geldi === 0 || $seansD->geldi ===null)
                        {
                            $seansD->seans_tarih = $randevu
                        }
                    }
                }

                return [
                    'hizmet_adi' => $hizmetler->first()->hizmet->hizmet_adi,
                    'toplam_seans' => $hizmetler->flatMap(fn ($hizmet) => $hizmet->seanslar)->pluck('id')->values()
                ];
        ...
        return [
            'kullanici' => [
                'id' => $kullanici->id,
                'name' => $kullanici->name,
                'cep_telefon' => $kullanici->cep_telefon
            ],
            'hizmetler' => $hizmetler->values()
        ];
    })
...
//return response()->json($sonuclar);
            ...
            /*$adisyonlar = Adisyonlar::where('salon_id',180)->get();
            foreach($adisyonlar as $adisyon)
            {
                foreach($adisyon->hizmetler as $hizmet)
                {
                    $seanslar = AdisyonPaketSeanslar::join('adisyon_hizmetler','adisyon_hizmetler.')where('hizmet_id',$hizmet->hizmet_id)->where('')
                    
                    foreach($hizmet->seanslar as $key=> $seans)
                    {
                        
                       
                            $seansD = AdisyonPaketSeanslar::where('id',$seans->id)->first();
                            $seansD->seans_tarih = null;
                            $seansD->seans_saat = null;
                            $seansD->personel_id = null;
                            $seansD->oda_id = null;
                            $seansD->cihaz_id = null;
                            $seansD->geldi = null;
                            $seansD->save();
                            //echo $randevuVar[$key]->tarih." ".$randevuVar[$key]->saat ."Seans geldi kaydedildi<br>";
    ...
    function birlestirAyniBaslik($data) {
        $sonuc = [];

        foreach ($data as $item) {
            $baslik = $item['baslik'];
            $seans = (int) $item['seans'];
            $fiyat = (float) $item['fiyat'];

            if (isset($sonuc[$baslik])) {
                // Aynı başlık varsa, seans sayısını ekleyelim
                $sonuc[$baslik]['seans'] += $seans;
                $sonuc[$baslik]['fiyat'] += $fiyat;
            } else {
                // Yeni başlık için kaydı ekleyelim
                $sonuc[$baslik] = [
                    'baslik' => $baslik,
                    'seans' => $seans,
        ...
        // Sonucu sıfırdan dizinin içine aktarıyoruz
        return array_values($sonuc);
    }

     public function seansAyristirma($text)
    {
    ...
    public function seanstakip(Request $request){
       ...
       return view('isletmeadmin.seanstakip',['bildirimler'=>self::bildirimgetir($request),'adisyonlar'=>$adisyonlar,'pageindex'=>14, 'sayfa_baslik'=>'Seans Takibi','isletme'=>$isletme,'seanstakip'=>$adisyonlar, 'kalan_uyelik_suresi' => self::lisans_sure_kontrol($request),'urun_drop'=>self::urundropliste($request),'yetkiliolunanisletmeler'=>$isletmeler]);
    }
        ...
        return DB::table('adisyon_paketler')
            ...
            ->select(
                ...
                'adisyonlar.created_at as olusturma_tarih',
                       ...
                       'users.name as musteri',
                        DB::raw('CONCAT(paketler.paket_adi, " (P)") as paket_adi'),
                        DB::raw('CONCAT("
                    ...
                    ->groupBy('adisyon_paket_seanslar.adisyon_paket_id')->orderBy('adisyonlar.id','desc')->unionAll(
                            DB::table('adisyon_hizmetler')
            ->leftjoin('adisyonlar','adisyon_hizmetler.adisyon_id','=','adisyonlar.id')
            ->leftjoin('adisyon_paket_seanslar','adisyon_hizmetler.hizmet_id','=','adisyon_paket_seanslar.hizmet_id')
            ->leftjoin('hizmetler','adisyon_hizmetler.hizmet_id','=','hizmetler.id')
            ->join('users','adisyonlar.user_id','=','users.id')
            ->select(
                'adisyonlar.id as id',
                'adisyonlar.created_at as olusturma_tarih',
                        DB::raw('DATE_FORMAT(adisyon_hizmetler.islem_tarihi, "%d.%m.%Y") as baslangic_tarihi'),
                       'users.name as musteri',
                        DB::raw('CONCAT(hizmetler.hizmet_adi, " (H)") as paket_adi'),
                        

                          DB::raw('CONCAT("
                            <button name=\"paketteki_seanslarlar\" type=\"button\" style=\"width:50px;font-size:10px\" class=\"btn btn-primary\">",
                                 (SELECT COUNT(*) FROM adisyon_paket_seanslar where adisyon_hizmetler.id = adisyon_paket_seanslar.adisyon_hizmet_id)
                            ,
                            "&nbsp;<i class=\"fa fa-plus\"></i></button>
                            <button name=\"paketteki_seanslari_beklemede_isaretle\" style=\"width:50px;font-size:10px\" type=\"button\" class=\"btn btn-warning\">",
                                 (SELECT COUNT(*) FROM adisyon_paket_seanslar
                                WHERE adisyon_paket_seanslar.geldi is null and adisyon_hizmetler.id = adisyon_paket_seanslar.adisyon_hizmet_id)
                            ,
                            "&nbsp;<i class=\"fa fa-calendar\"></i></button>
                                       <button type=\"button\" name=\"paketteki_seanslari_geldi_isaretle\" style=\"width:50px;font-size:10px\" class=\"btn btn-success\" >",
                                       (SELECT COUNT(*) FROM adisyon_paket_seanslar
                                WHERE adisyon_paket_seanslar.geldi = true and adisyon_hizmetler.id = adisyon_paket_seanslar.adisyon_hizmet_id)
                                       ,"&nbsp; <i class=\"fa fa-check\"></i></button>
                                       <button type=\"button\" name=\"paketteki_seanslari_gelmedi_isaretle\" style=\"width:50px;font-size:10px\" class=\"btn btn-danger\">",
                               (SELECT COUNT(*) FROM adisyon_paket_seanslar
                                WHERE adisyon_paket_seanslar.geldi = false and adisyon_hizmetler.id = adisyon_paket_seanslar.adisyon_hizmet_id)
                                      ,"&nbsp;<i class=\"fa fa-times\"></i></button>") as durum'),
                        DB::raw('CONCAT("<button type=\"button\" name=\"hizmet_seans_detay_getir_modal\" data-value=\"",adisyon_hizmetler.id,"\" class=\"btn btn-primary\"><i class=\"fa fa-eye\"></i></button>") as islemler'),
                    )->where('adisyonlar.salon_id',self::mevcutsube($request))->where('adisyonlar.tarih','>=',$tarih1)->where('adisyonlar.tarih','<=',$tarih2)
                    ->where(function($q) use($musteriid){if($musteriid!='') $q->where('adisyonlar.user_id',$musteriid);})
                    ->groupBy('adisyon_hizmetler.id', 'adisyonlar.id', 'users.id', 'hizmetler.id')
->orderBy('adisyonlar.id','desc')
                   ...
                   ((SELECT COALESCE(SUM(tahsilat_hizmetler.tutar),0) from tahsilat_hizmetler where tahsilat_hizmetler.adisyon_hizmet_id = adisyon_hizmetler.id)*(COALESCE(p1.hizmet_prim_yuzde,0)/100)) +
                    ((SELECT COALESCE(SUM(tahsilat_urunler.tutar),0) from tahsilat_urunler where tahsilat_urunler.adisyon_urun_id = adisyon_urunler.id)*(COALESCE(p2.urun_prim_yuzde,0)/100)) +
                     ((SELECT COALESCE(SUM(tahsilat_paketler.tutar),0) FROM tahsilat_paketler where tahsilat_paketler.adisyon_paket_id = adisyon_paketler.id)*(COALESCE(p3.paket_prim_yuzde,0)/100)) as hakedis_numeric'),
                       ...
                       'users.name as musteri',
                        ...
                        DB::raw('
                                CONCAT(
                                    ...
                                    FORMAT((
                                        SELECT COALESCE(SUM(tahsilat_hizmetler.tutar), 0) 
                                        FROM tahsilat_hizmetler 
                                        JOIN adisyon_hizmetler ON tahsilat_hizmetler.adisyon_hizmet_id = adisyon_hizmetler.id
                                        WHERE adisyon_hizmetler.adisyon_id = adisyonlar.id
                                    ) +
                                    (
                                        SELECT COALESCE(SUM(tahsilat_urunler.tutar), 0) 
                                        FROM tahsilat_urunler 
                                        JOIN adisyon_urunler ON tahsilat_urunler.adisyon_urun_id = adisyon_urunler.id
                                        WHERE adisyon_urunler.adisyon_id = adisyonlar.id
                                    ) +
                                    (
                                        SELECT COALESCE(SUM(tahsilat_paketler.tutar), 0) 
                                        FROM tahsilat_paketler 
                                        JOIN adisyon_paketler ON tahsilat_paketler.adisyon_paket_id = adisyon_paketler.id
                                        WHERE adisyon_paketler.adisyon_id = adisyonlar.id
                                    ), 2, "tr_TR"
                                    ),
                                    "</button>"
                                ) AS odenen
                       ...
                       DB::raw('
    CONCAT(
        "<button class=\"btn btn-danger btn-block\" style=\"line-height:5px\">",
        FORMAT(
            (
                (SELECT COALESCE(SUM(adisyon_hizmetler.fiyat), 0) 
                 FROM adisyon_hizmetler WHERE adisyon_hizmetler.adisyon_id = adisyonlar.id)
                +
                (SELECT COALESCE(SUM(adisyon_urunler.fiyat), 0) 
                 FROM adisyon_urunler WHERE adisyon_urunler.adisyon_id = adisyonlar.id)
                +
                (SELECT COALESCE(SUM(adisyon_paketler.fiyat), 0) 
                 FROM adisyon_paketler WHERE adisyon_paketler.adisyon_id = adisyonlar.id)
            )
            -
            (
                (SELECT COALESCE(SUM(tahsilat_hizmetler.tutar), 0) 
            ...
            -
            (
                (SELECT COALESCE(SUM(adisyon_hizmetler.indirim_tutari), 0) 
                 FROM adisyon_hizmetler WHERE adisyon_hizmetler.adisyon_id = adisyonlar.id)
                +
                (SELECT COALESCE(SUM(adisyon_urunler.indirim_tutari), 0) 
                 FROM adisyon_urunler WHERE adisyon_urunler.adisyon_id = adisyonlar.id)
                +
                (SELECT COALESCE(SUM(adisyon_paketler.indirim_tutari), 0) 
                 FROM adisyon_paketler WHERE adisyon_paketler.adisyon_id = adisyonlar.id)
            ), 
            2, "tr_TR"
        ),
...
<input type="hidden" name="adisyon_urun_senet_id[]" value="'.$urun->senet_id.'">
    ...
    public function adisyon_paket_satis_getir($adisyon_id,$visibility,$paket_id)
    {
        ...
        foreach($satislar as $paket){
            $tahsilat_paket_eklenecek .= ' <div class="row" style="background:#cce5ff;margin:5px 0 5px 0; padding:5px;font-size:14px" data-value="0">
                     ...
                     ';
                     if($visibility && $paket_id == $paket->id && DB::table('model_has_roles')->where('role_id',5)->where(function($q){$q->where('model_id',Auth::guard('satisortakligi')->user()->id);$q->orWhere('model_id',Auth::guard('isletmeyonetim')->user()->id);})->where('salon_id',self::mevcutsube($request))->count() == 0)
    ...
    public function randevu_liste_getir(Request $request,$tarih1,$tarih2,$salon,$web,$uygulama,$durum,$salon_id,$userid)
    {
        ...
        select(
            ...
            where(function($q) use ($durum){if($durum!='') $q->where('randevular.durum',$durum);})->where(function($q) use($salon_id,$request){ if(self::personelmi($request)) $q->where('randevu_hizmetler.personel_id',Personeller::where('salon_id',$salon_id)->where('yetkili_id',Auth::guard('isletmeyonetim')->user()->id)->value('id')); })->
    ...
    public function senetler(Request $request){
        ...
        $senetler_odenmemis = self::senetleri_getir($request,2,'');
         return view('isletmeadmin.senetler',['pageindex'=>17,'sayfa_baslik'=>'Senet Takibi','bildirimler'=>self::bildirimgetir($request),'paketler'=> self::paket_liste_getir('',true,$request),'isletme'=>$isletme,'senetler'=>$senetler,'senetler_acik'=>$senetler_acik,'senetler_kapali'=>$senetler_kapali,'senetler_odenmemis'=>$senetler_odenmemis, 'kalan_uyelik_suresi' => self::lisans_sure_kontrol($request),'urun_drop'=>self::urundropliste($request),'yetkiliolunanisletmeler'=>$isletmeler]);
    }
        ...
        foreach($vadeler as $key => $vade)
        {
            ...
            if($vade->odendi==true){
                ...
                if(date('Y-m-d') > $vade->vade_tarih)
                    $html .= '<button type="button"   data-value="'.$vade->id.'"  data-toggle="modal" data-target="#senet_onay_modal"   name="senet_vadesi"  class="btn btn-danger" style="float:right">Ödenmedi</button>';
                else
    ...
    public function senetvadegetir_tahsilat(Request $request)
    {
        ...
        foreach($vadeler as $key => $vade)
        {
                ...
                if($vade->odendi==true)
                    $html .= '<button type="button"   data-value="'.$vade->id.'"  data-toggle="modal" data-target="#senet_onay_modal"   name="senet_vadesi"  class="btn btn-success" style="float:right">Ödendi ('.$vade->odeme_turu->odeme_yontemi.')</button>';
                else{
                    if(date('Y-m-d') > $vade->vade_tarih)
                        $html .= '<button type="button"   data-value="'.$vade->id.'" data-toggle="modal" data-target="#senet_onay_modal"  name="senet_vadesi"  class="btn btn-danger" style="float:right">Ödenmedi</button>';
                    else
    ...
    public function yeniisletmeekle(Request $request)
    {
        ...
        $isletme->adres = $request->firma_adresi;
        $isletme->aktif = false;
        $isletme->save();
        if(isset($request->yonetici_ekle))
        {
            ...
            $personel->yetkili_id = $yetkili->id;
            $personel->save();
            self::sms_gonder_bildirimli($request,array(array("to"=>$yetkili->gsm1,"message"=>"RandevumCepte hesabınız oluşturulmuş olup ".$olusturulansifre." şifrenizle giriş yapabilirsiniz.")),false,1,false);
        }
        $supervisorler = Personeller::join('model_has_roles','salon_personelleri.yetkili_id','=','model_has_roles.model_id')->where(function($q) use($request){$q->where('model_has_roles.role_id',2);$q->orWhere('salon_personelleri.role_id',2);})->where('salon_personelleri.salon_id',$mevcutisletme->id)->get();
        $renkindex = 3;
        foreach($supervisorler as $supervisor)
        {
            DB::insert('insert into model_has_roles (role_id, model_type,model_id,salon_id) values (3, "App\\\IsletmeYetkilileri",'.$supervisor->id.','.$isletme->id.')');
            $personel_s = new Personeller();
            $personel_s->personel_adi = $supervisor->personel_adi;
            $personel_s->cep_telefon = $supervisor->cep_telefon;
            $personel_s->role_id = 2;
            $personel_s->renk = $renkindex;
            $personel_s->takvimde_gorunsun = true;
            $personel_s->takvim_sirasi = $renkindex;
            $personel_s->aktif = true;
            $personel_s->salon_id = $isletme->id;
            $personel_s->yetkili_id = $supervisor->yetkili_id;
            $personel_s->save();
            $renkindex++;
        }
        ...
        $_personel = new Personeller();
        $_personel->personel_adi = Auth::guard('isletmeyonetim')->user()->name;
        $_personel->cep_telefon = Auth::guard('isletmeyonetim')->user()->gsm1;
        $_personel->salon_id = $isletme->id;
        $_personel->renk = 1;
        $_personel->takvimde_gorunsun = true;
        $_personel->takvim_sirasi = 1;
        $_personel->aktif = true;
        $_personel->role_id = 1;
        $_personel->yetkili_id = Auth::guard('isletmeyonetim')->user()->id;
        $_personel->save();
        return array(
   ...
   public function etkinlikekleduzenle(Request $request)
    {
        ...
        if (isset($request->etkinlik_katilimci_musteriler)) {
            foreach($request->etkinlik_katilimci_musteriler as $key=>$katilimci)
            {
                ...
                if(SalonSMSAyarlari::where('ayar_id',10)->where('salon_id',$etkinlik->salon_id)->value('musteri')==1 )
                    $katilim_link = ' Katılım için : https://'.$_SERVER['SERVER_NAME'].'/etkinlikkatilim/'.$etkinlik->id.'/'.$toplumusteri->id;
        ...
        $gonder=self::sms_gonder_bildirimli($request,$mesajlar,true,6,false);
        return array(
   ...
   public function etkinlikbeklenensms(Request $request){
    ...
    public function etkinlikler(Request $request){
       ...
       $isletme = Salonlar::where('id',self::mevcutsube($request))->first();
       $katilimci= self::etkinlikyukle($request);
        return view('isletmeadmin.etkinlikler',['paketler'=>$paketler,'pageindex'=>20,'sayfa_baslik'=>'Etkinlikler','isletme'=>$isletme,'bildirimler'=>self::bildirimgetir($request),'katilimci'=>$katilimci, 'kalan_uyelik_suresi' => self::lisans_sure_kontrol($request),'portfoy_drop'=>self::musteriportfoydropliste($request),'urun_drop'=>self::urundropliste($request),
            'yetkiliolunanisletmeler'=>$isletmeler]);
    }
    ...
    public function randevu_dogrulama_kodu_gonder(Request $request)
    {
        ...
        var_dump( self::sms_gonder_bildirimli($request,$mesaj,false,1,true));
        return '';
    }
    ...
    public function randevugeldiisaretle(Request $request)
    {
        ...
        foreach($randevu->hizmetler as $hizmet)
        {
            ...
            $mesaj = $randevu->users->name." isimli müşteriniz ". date('H:i',strtotime($hizmet->saat)) ." ".$hizmet->hizmetler->hizmet_adi." randevusu için salonunuza gelmiştir.";
            $yetkiliid=Personeller::where('id',$hizmet->personel_id)->value('yetkili_id');
            array_push($mesajlar, array("to"=>IsletmeYetkilileri::where('id',$yetkiliid)->value('gsm1'),"message"=>$mesaj));
        
            self::bildirimekle($request,$randevu->salon_id,$mesaj,"#",$hizmet->personel_id,null, Auth::guard('isletmeyonetim')->user()->profil_resim,$randevu->id);
            $bildirimkimlikleri = BildirimKimlikleri::where('isletme_yetkili_id',$hizmet->personel_id)->pluck('bildirim_id')->toArray();
            self::bildirimgonder($bildirimkimlikleri,"Yeni Randevu",$mesaj,$randevu->salon_id);
        }
        self::sms_gonder_bildirimli($request,$mesajlar,false,1,false);
...
$odeme->tutar = round((str_replace(['.',','],['','.'],$request->urun_fiyat_senet[$key])/($on_odeme_tutari+str_replace(['.',','],['','.'],$request->senet_tutar)))*$on_odeme_tutari ,2 );
        ...
        for($i=1;$i<=$request->vade;$i++){
            ...
            $alacak->senet_vade_id = $yeni_vadeler->id;
            $alacak->save();
        }
    ...
    public function kampanya_yonetimi_liste(Request $request){
        ...
        $kampanya_katilimci_musteriler = MusteriPortfoy::where('salon_id', self::mevcutsube($request))
            ->where('aktif', true)
            ->paginate(50); // Her sayfada 50 kayıt göster
    ...
    public function kampanyaekleduzenle(Request $request)
    {
        ...
        if($request->gonderim_turu = 1 || $request->gonderim_turu==3)
            $kampanya_yonetimi->arama_ile_gonderim= true;
        if($request->gonderim_turu =2 )
        ...
        if(isset($request->kampanya_katilimci_musteriler)){
            foreach($request->kampanya_katilimci_musteriler as $katilimci)
            {
                ...
                if(SalonSMSAyarlari::where('ayar_id',10)->where('salon_id',$kampanya_yonetimi->salon_id)->value('musteri')==1)
                    $katilim_link = ' Katılım için : https://'.$_SERVER['SERVER_NAME'].'/kampanyakatilim/'.$kampanya_yonetimi->id.'/'.$toplumusteri->id;
    ...
    public function randevudetayigetir(Request $request)
    {
        ...
        return array(
            ...
            'musteri' => "<option value='".$randevu->user_id."'>".$randevu->users->name."</option>",
            'sms_hatirlatma' =>$randevu->sms_hatirlatma,
            'randevu_tarih' => $randevu->tarih,
            'randevu_saat' => date('H:i',strtotime($randevu->saat)),
    ...
    public function ongorusmesatisyapildi(Request $request)
    {
        ...
        if($ongorusme->user_id == null)
        {
            if(User::where('cep_telefon',$ongorusme->cep_telefon)->count() == 0){
                ...
                $user->save();
                self::sms_gonder_bildirimli($request,array(array("to"=>$user->cep_telefon,"message"=>"RandevumCepte hesabınız oluşturulmuş olup ".$olusturulansifre." şifrenizle giriş yapabilirsiniz.")),false,1,false);
            }
            else
        ...
        if($ongorusme->paket_id != null)
        {
            ...
            $bilgi = Paketler::where('id',$ongorusme->paket->id)->first();
            ...
            $yenisaatbaslangic = '';
            $toplamtutar=0;
            foreach ($bilgi->hizmetler as $hizmet)
            {
                $toplamtutar+=$hizmet->fiyat;
            }
            
            foreach($bilgi->hizmetler as $key2 => $hizmet2)
            {
                $randevu_id  ='';
                for($i=1;$i<=$hizmet2->seans;$i++)
                {
                    ...
                    if($key2==0||count($hizmete_ait_randevu)<$i)
                    {
                        $yenisaatbaslangic = $request->seans_saati;
                        $seans_randevu = new Randevular();
                        $seans_randevu->user_id = $user->id;
                        $seans_randevu->tarih = $seanstarih;
                        $seans_randevu->salon_id = $request->sube;
                        $seans_randevu->durum = 1;
                        $seans_randevu->saat = $request->seans_saati;
                        $seans_randevu->olusturan_personel_id = Auth::guard('isletmeyonetim')->user()->id;
                        $seans_randevu->salon = 1;
                        $seans_randevu->save();
                        $randevu_id = $seans_randevu->id;
                        array_push($hizmete_ait_randevu,$randevu_id);
                        if($i==$hizmet2->seans)
                            $yenisaatbaslangic = date("H:i", strtotime('+'.$hizmet_sure.' minutes', strtotime($request->seans_saati)));
                    }
                    else
                        $randevu_id = $hizmete_ait_randevu[$i-1];
                    $seans = new AdisyonPaketSeanslar();
                    ...
                    $seans_randevu_hizmet->sure_dk = $hizmet_sure;
                    if($key2==0||count($hizmete_ait_randevu)<$i)
                        $seans_randevu_hizmet->saat = $request->seans_saati;
                    else
                        $seans_randevu_hizmet->saat = $yenisaatbaslangic;
                    $seans_randevu_hizmet->saat_bitis = date("H:i", strtotime('+'.$hizmet_sure.' minutes', strtotime($yenisaatbaslangic)));
                    $seans_randevu_hizmet->save();
    ...
    public function e_asistan_ayar_kaydet(Request $request)
    {
       
        if(isset($request->e_asistan_alacak_acik_kapali))
            SalonEAsistanAyarlari::where('salon_id',$request->sube)->where('ayar_id',1)->update(['acik_kapali'=>true]);
        else
            SalonEAsistanAyarlari::where('salon_id',$request->sube)->where('ayar_id',1)->update(['acik_kapali'=>false]);
        if(isset($request->e_asistan_randevu_acik_kapali))
            SalonEAsistanAyarlari::where('salon_id',$request->sube)->where('ayar_id',4)->update(['acik_kapali'=>true]);
        else
            SalonEAsistanAyarlari::where('salon_id',$request->sube)->where('ayar_id',4)->update(['acik_kapali'=>false]);
        if(isset($request->e_asistan_ongorusme_acik_kapali))
            SalonEAsistanAyarlari::where('salon_id',$request->sube)->where('ayar_id',2)->update(['acik_kapali'=>true]);
        else
            SalonEAsistanAyarlari::where('salon_id',$request->sube)->where('ayar_id',2)->update(['acik_kapali'=>false]);
        if(isset($request->e_asistan_stok_acik_kapali))
            SalonEAsistanAyarlari::where('salon_id',$request->sube)->where('ayar_id',5)->update(['acik_kapali'=>true]);
        else
            SalonEAsistanAyarlari::where('salon_id',$request->sube)->where('ayar_id',5)->update(['acik_kapali'=>false]);
        if(isset($request->e_asistan_tekrar_acik_kapali))
            SalonEAsistanAyarlari::where('salon_id',$request->sube)->where('ayar_id',3)->update(['acik_kapali'=>true]);
        else
            SalonEAsistanAyarlari::where('salon_id',$request->sube)->where('ayar_id',3)->update(['acik_kapali'=>false]);
        if(isset($request->e_asistan_karaliste_acik_kapali))
            SalonEAsistanAyarlari::where('salon_id',$request->sube)->where('ayar_id',6)->update(['acik_kapali'=>true]);
        else
            SalonEAsistanAyarlari::where('salon_id',$request->sube)->where('ayar_id',6)->update(['acik_kapali'=>false]);
    ...
    public function grupsmsgonderme(Request $request){
        ...
        if(count($mesajlar) > 0){
            return self::sms_gonder_bildirimli($request,$mesajlar,true,2,false);
            exit;
        }
        else
        {
    ...
    public function cinsiyetehizmetegore(Request $request){
     ...
     public function filtrelismsgonder(Request $request){
        ...
        if(count($mesajlar) > 0){
            return self::sms_gonder_bildirimli($request,$mesajlar,true,3,false);
            exit;
        }
        else
        {
    ...
    public function kampanyabeklenensms(Request $request){
          ...
          if($request->hemen_gonderilecek=='on')
          {
              $mesajlar=array();
              ...
              foreach ($kampanyabeklenen->kampanya_katilimcilari as  $katilimci) {
                ...
                if((isset($request->beklenen) && $katilimci->durum === null) || (isset($request->katilmayan) && $katilimci->durum === 0))
                {
                    $sayac++;
                    $katilim_link = '';
                    if(SalonSMSAyarlari::where('ayar_id',10)->where('salon_id',$kampanyabeklenen->salon_id)->value('musteri')==1)
                        $katilim_link = ' Katılım için : https://'.$_SERVER['SERVER_NAME'].'/kampanyakatilim/'.$kampanyabeklenen->id.'/'.$katilimci->user_id;
              ...
              $gonder=self::sms_gonder_bildirimli($request,$mesajlar,true,5,false);
              return array(
                  "mesaj" => "SMS başarıyla gönderildi",
                  "gonder"=>$gonder,
              );
          ...
          else
          {
             ...
             $kampanyabeklenen->sms_tarih = $request->gonderim_tarih;
             $kampanyabeklenen->sms_saat =$request->gonderim_saat;
             $kampanyabeklenen->sms_ile_gonderim = true;
    ...
    public function seansdetaylari(Request $request){
        $paket_seanslar = "";
        $paket = "";
        if(isset($request->adisyonpaketid)){
            $paket_seanslar = AdisyonPaketSeanslar::where('adisyon_paket_id',$request->adisyonpaketid)->get();
            
        }
        else{
            $paket_seanslar = AdisyonPaketSeanslar::where('adisyon_hizmet_id',$request->adisyonhizmetid)->get();
    ...
    public function senetvadeguncelle(Request $request)
    {
        ...
        $alacak = Alacaklar::where('senet_vade_id',$vade->id)->first();
         
        $eskivadetarihi = $vade->vade_tarih;
        $vade->vade_tarih = $request->planlanan_odeme_tarihi;
        $vade->notlar = $request->notlar;
        $vade->save();
        $alacak->planlanan_odeme_tarihi = $vade->vade_tarih;
        $alacak->hatirlatma_aramasi_yapildi = null;
        if(date('Y-m-d',strtotime($vade->vade_tarih))==date('Y-m-d',strtotime('+2 days',strtotime(date('Y-m-d')))))
            $alacak->arama_saat = date('H:i',strtotime('+2 hours',strtotime('H:i')));
        else
            $alacak->arama_saat = "10:00";
        $alacak->hatirlatma_ulasilamadi = null;
        $alacak->hatirlatma_gorevi_iptal = null;
        $alacak->tekrar_arandi = null;
        $alacak->save();
     ...
     public function taksitvadeguncelle(Request $request)
    {
        ...
        $eskivadetarihi = $vade->vade_tarih;
        $alacak = Alacaklar::where('taksit_vade_id',$vade->id)->first();
        $alacak->planlanan_odeme_tarihi = $vade->vade_tarih;
          $alacak->hatirlatma_aramasi_yapildi = null;
        $alacak->hatirlatma_ulasilamadi = null;
        $alacak->hatirlatma_gorevi_iptal = null;
    ...
    public function senet_odeme_dogrulama_kodu_gonder(Request $request)
    {
        ...
        self::sms_gonder_bildirimli($request,$mesaj,false,1,true);
        return '';
    }
    ...
    public function smsraportest(Request $request)
    {
          ...
          $kampanyabeklenen= KampanyaYonetimi::where('id',119)->first();
    ...
    public function randevu_excel(Request $request)
    {
        ...
        try {
            $import = new RandevuImport(20);
            $result = Excel::import($import, $fullPath);
  ...
  public function adisyon_sil(Request $request)
   {
        ...
        foreach($bilgi as $key => $paket)
        {
            ...
            foreach($paket_mevcut->hizmetler as $key2 => $hizmet2)
            {
                ...
                for($i=1;$i<=$hizmet2->seans;$i++)
                {
                    ...
                    $seans_randevu_hizmet->randevu_id = $randevu_id;
                    $seans_randevu_hizmet->hizmet_id =$hizmet2->hizmet_id;
                    $seans_randevu_hizmet->personel_id = $request->paket_satis_personel_id[$key];
                    $seans_randevu_hizmet->sure_dk = $hizmet_sure;
                    if($key2==0||count($hizmete_ait_randevu)<$i)
    ...
    public function ajanda_liste_getir(Request $request, $returntext){
        $ajanda = "";
        $ajanda=DB::table('ajanda')->when(Auth::guard('isletmeyonetim')->check(), function ($query) {
        return $query->join('salon_personelleri', 'ajanda.ajanda_olusturan', '=', 'salon_personelleri.id');
    })->
        select(
            'ajanda.id as id',
            ...
            DB::raw(Auth::guard('isletmeyonetim')->check() ? 'salon_personelleri.personel_adi AS ajanda_olusturan' : 'NULL AS ajanda_olusturan'),
            ...
            DB::raw(Auth::guard('isletmeyonetim')->check() ? 'CASE WHEN ajanda.ajanda_durum=1 THEN CONCAT("<div class=\"dropdown\">
                <a class=\"btn btn-link font-24 p-0 line-height-1 no-arrow dropdown-toggle\" href=\"#\" role=\"button\" data-toggle=\"dropdown\"><i class=\"dw dw-more\"></i>
                </a>
                <div class=\"dropdown-menu dropdown-menu-right dropdown-menu-icon-list\">
               ...
               WHEN  ajanda.ajanda_durum=0 THEN CONCAT("<div class=\"dropdown\">
                ...
                <div class=\"dropdown-menu dropdown-menu-right dropdown-menu-icon-list\">
                   ...
                   END AS islemler' : 'CASE WHEN ajanda.ajanda_durum=1 THEN CONCAT("<div class=\"dropdown\">
                <a class=\"btn btn-link font-24 p-0 line-height-1 no-arrow dropdown-toggle\" href=\"#\" role=\"button\" data-toggle=\"dropdown\"><i class=\"dw dw-more\"></i>
                </a>
                <div class=\"dropdown-menu dropdown-menu-right dropdown-menu-icon-list\">
                    <a class=\"dropdown-item\" disabled data-toggle=\"modal\" data-value=\"",ajanda.id,"\" data-target=\"#ajanda_duzenle_modal\"  href=\"#\"> <i class=\"fa fa-edit\"></i> Düzenle</a>
                    <a class=\"dropdown-item \" disabled data-value=\"",ajanda.id,"\"   href=\"#\"> <i class=\"fa fa-times\"></i>Sil</a>
                   </div></div>")
               WHEN  ajanda.ajanda_durum=0 THEN CONCAT("<div class=\"dropdown\">
                <a class=\"btn btn-link font-24 p-0 line-height-1 no-arrow dropdown-toggle\" href=\"#\" role=\"button\" data-toggle=\"dropdown\"><i class=\"dw dw-more\"></i>
                </a>
                <div class=\"dropdown-menu dropdown-menu-right dropdown-menu-icon-list\">
                    <a class=\"dropdown-item\" disabled data-toggle=\"modal\" data-value=\"",ajanda.id,"\" data-target=\"#ajanda_duzenle_modal\" href=\"#\"> <i class=\"fa fa-edit\"></i> Düzenle</a>
                    <a class=\"dropdown-item \" disabled  data-value=\"",ajanda.id,"\"  href=\"#\"> <i class=\"fa fa-check\"></i>Okundu</a>
                    <a  class=\"dropdown-item \" disabled data-value=\"",ajanda.id,"\"    href=\"#\"> <i class=\"fa fa-times\"></i>Sil</a>
                   </div></div>")
                   END AS islemler')
        )->where('ajanda.aktif',true)->where('ajanda.salon_id',self::mevcutsube($request))->where('ajanda.ajanda_olusturan',Auth::guard('satisortakligi')->check() ? null : Personeller::where('salon_id',self::mevcutsube($request))->where('yetkili_id',Auth::guard('isletmeyonetim')->user()->id)->value('id'))->orderBy('ajanda.id','desc')->get();
       return array(
                'status' => $returntext,
                'ajanda' => $ajanda,
        );
    ...
    public function santral_raporlari($salon_id,$tarih1,$tarih2,$durum,$request)
    {
        ...
        if($result['data']['fetchAllCdrs']['totalCount']>0)
        {
            foreach($result['data']['fetchAllCdrs']['cdrs'] as $cdr)
            {
                ...
                {
                    $dahililer = Personeller::where('salon_id',self::mevcutsube($request))->pluck('dahili_no')->toArray();
                    if($cdr['src']==SabitNumaralar::where('salon_id',self::mevcutsube($request))->value('numara') || $cdr['did']==SabitNumaralar::where('salon_id',self::mevcutsube($request))->value('numara')||in_array($cdr["dst"],$dahililer))
                    {
                        ...
                        array_push($rapor,array(
                            'tarih' => "<span style='display:none'>".date('YmdHis',strtotime($cdr['calldate']))."</span> ".date('d.m.Y',strtotime($cdr['calldate'])),
 ...
 exit;
    ...
    public function formmusteribilgigetir(Request $request){
     ...
     public function arsivformekleme(Request $request){
        ...
        $gonder=self::sms_gonder_bildirimli($request,$mesajlar,true,6,true);
         if ($request->formpersonelsec) {
            $katilim_link2 = ' İmza atmak için : https://'.$_SERVER['SERVER_NAME'].'/personelformdoldurma/'.$form->id.'/'.$request->formpersonelsec;
                    array_push($mesajlar2, array("to"=>$request->formmpersonelceptelefon,"message"=>$katilim_link2));
        }
        $gonder2=self::sms_gonder_bildirimli($request,$mesajlar2,true,6,false);
        return array(
        "gonder"=>$gonder,
        "gonder"=>$gonder2,
        "arsiv"=>self::arsiv_liste_getir($request,'',''),
   ...
   public function islemdetayigetir(Request $request){
    ...
    public function personelmi(Request $request)
    {
        ...
        ->where(function($q) use ($userSatis, $userIsletme, $request) {
            ...
            if ($userSatis) {
                $q->where('model_id', $userSatis->id);
            }

            // IsletmeYonetim guard'ından gelen kullanıcı varsa
            if ($userIsletme) {
    ...
    {
        if(Salonlar::where('id',$salon_id)->value('salon_turu_id') == 15 ||Salonlar::where('id',$salon_id)->value('salon_turu_id') == 28|| Salonlar::where('id',$salon_id)->value('salon_turu_id') == 29)
        {
            return array(
                'kelime'=>'danışan',
                'ek'=>'ın'
    ...
    public function easistandata(Request $request,$bugunYarin)
    {
         return DB::table('randevular')
                ...
                ->select(
                    ...
                    DB::raw("CASE 
                                ...
                                WHEN randevular.hatirlatma_ulasilamadi = 1 AND randevular.tekrar_arandi != 1 THEN CONCAT ('Ulaşılamadı, ',DATE_FORMAT(randevular.tekrar_arama_tarih_saat, '%H:%i'), ' de tekrar arayacağım')
                                WHEN randevular.asistan_guncelledi = 1 THEN CONCAT('Asistan tarafından ', 
                                                                                  DATE_FORMAT(randevular.tarih, '%d.%m.%Y'), 
                                                                                  ' ', DATE_FORMAT(randevular.saat, '%H:%i'), 
                                                                                  ' olarak güncellendi') 
                                ELSE CONCAT(DATE_FORMAT(randevular.tekrar_arama_tarih_saat,'%H:%i'),' saatinde tekrar aradım, ulaşılamadı') 
                             END as sonuc"),
                       DB::raw("CASE WHEN randevular.hatirlatma_ulasilamadi = 0 THEN '<button class=\'btn btn-success btn-block\' style=\'line-height:5px;width:auto\'>Ulaşıldı</button>' WHEN randevular.hatirlatma_ulasilamadi=1 THEN '<button class=\'btn btn-danger btn-block\' style=\'line-height:5px;width:auto\'>Ulaşılamadı</button>' ELSE '' END as durum"),
                       ...
                       DB::raw("CASE 
                            ...
                            THEN CONCAT('<button name=\'gorev_iptal_et_randevu\' class=\'btn btn-primary btn-block\' data-value=\'', randevular.id, '\' style=\'line-height:5px;width:auto\'>Görevi İptal Et</button>') 
                            ELSE '' 
                        END as islemler"),
                ...
                ->where('randevular.salon_id', self::mevcutsube($request))
                ->where(function ($q) use($bugunYarin) {
                    $q->where('randevular.tarih', date('Y-m-d', strtotime('+'.($bugunYarin+1) .' days', strtotime(date('Y-m-d')))));
                    $q->orWhere('randevular.onceki_tarih', date('Y-m-d', strtotime('+'.($bugunYarin+1).' days', strtotime(date('Y-m-d')))));
                })
                ->unionAll(
                    DB::table('alacaklar') ->join('users', 'alacaklar.user_id', '=', 'users.id')
                ->join('salonlar', 'salonlar.id', '=', 'alacaklar.salon_id')
                        ->select(
                            ...
                            DB::raw("CASE 
                                        ...
                                        WHEN alacaklar.asistan_guncelledi = 1 THEN CONCAT('Asistan tarafından ', 
                                                                                          DATE_FORMAT(alacaklar.planlanan_odeme_tarihi, '%d.%m.%Y'), 
                                                                                          ' olarak güncellendi.') 
                                        WHEN alacaklar.tekrar_arandi != 1 THEN CONCAT('Ulaşılamadı.', DATE_FORMAT(alacaklar.tekrar_arama_tarih_saat, '%H:%i'),' saatinde tekrar arayacağım')
                                        ELSE CONCAT(DATE_FORMAT(alacaklar.tekrar_arama_tarih_saat,'%H:%i'),' saatinde tekrar aradım, ulaşılamadı') 
                                     END as sonuc"),
                             DB::raw("CASE WHEN alacaklar.hatirlatma_ulasilamadi = 0 THEN '<button class=\'btn btn-success btn-block\' style=\'line-height:5px;width:auto\'>Ulaşıldı</button>' WHEN alacaklar.hatirlatma_ulasilamadi=1 THEN '<button class=\'btn btn-danger btn-block\' style=\'line-height:5px;width:auto\'>Ulaşılamadı</button>' ELSE '' END as durum"),
                            ...
                            THEN CONCAT('<button name=\'gorev_iptal_et_alacak\' class=\'btn btn-primary btn-block\' data-value=\'', alacaklar.id, '\' style=\'line-height:5px;width:auto\'>Görevi İptal Et</button>') 
                            ELSE '' 
                        END as islemler"),
                        ...
                        ->where('alacaklar.salon_id', self::mevcutsube($request))
                        ->where(function ($q) use($bugunYarin) {
                            $q->where('alacaklar.planlanan_odeme_tarihi', date('Y-m-d', strtotime('+'.($bugunYarin+2).' days', strtotime(date('Y-m-d')))));
                            $q->orWhere('alacaklar.onceki_planlanan_odeme_tarihi', date('Y-m-d', strtotime('+'.($bugunYarin+2).' days', strtotime(date('Y-m-d')))));
                        })
                )
                ->unionAll(
                    DB::table('kampanya_yonetimi')
                ->join('salonlar', 'salonlar.id', '=', 'kampanya_yonetimi.salon_id')
                        ->select(
                            ...
                            DB::raw("CONCAT('<a style=\'color:#fff;width:auto;line-height:5px\' class=\'btn btn-primary\' data-value=\'',kampanya_yonetimi.id,'\' data-toggle=\'modal\' name=\'kampanya_detay\' data-target=\'#kampanya_detay_modal\' >Rapor Detayı >></a>') as durum"),
                            ...
                            DB::raw('"Kampanya Tanıtımı" as baslik'),
                             ...
                             DB::raw("CASE WHEN (kampanya_yonetimi.arama_yapildi = 0 OR kampanya_yonetimi.arama_yapildi is null) AND (kampanya_yonetimi.tanitim_gorev_iptal = 0 or kampanya_yonetimi.tanitim_gorev_iptal is NULL)  THEN CONCAT('<button name=\'gorev_iptal_et_kampanya\' class=\'btn btn-primary\' data-value=\'',kampanya_yonetimi.id,'\' style=\'line-height:5px;width:auto\'>Görevi İptal Et</button>') ELSE '' END as islemler"),
                             DB::raw('kampanya_yonetimi.id as kampanya_id')
                        )
                        ->where('kampanya_yonetimi.salon_id', self::mevcutsube($request))
                        ->where(function ($q) use($bugunYarin) {
                            $q->where('kampanya_yonetimi.asistan_tarih_saat', 'LIKE','%'.date('Y-m-d', strtotime('+'.($bugunYarin).' days', strtotime(date('Y-m-d')))).'%');
                            $q->orWhere('kampanya_yonetimi.asistan_tarih_saat','LIKE', '%'.date('Y-m-d', strtotime('+'.($bugunYarin).' days', strtotime(date('Y-m-d')))).'%');
                        })
                )
                ->get();
    }
    ...
    {
        $mesaj = self::gorevIptalEt($request);
        return array(
            "mesaj"=>$mesaj,
            "easistandata"=> self::easistandata($request,0),
            "easistanYarinYapilacak" =>self::easistandata($request,1),
    ...
    {
        $userId = auth()->guard('satisortakligi')->id(); // Get the authenticated user ID
        $qrCodeData = route('whatsapp.auth', ['user_id' => $userId]); // URL for QR code scan
        
        $qrCode = new QrCode($qrCodeData); // Generate QR code with the login URL
        $qrCode->setSize(300);
        
        // Return the QR code as a data URI
        return response()->json([
            'qr_code' => $qrCode->writeDataUri() // This will give you a base64 encoded image
        ]);
    }
    
    // Authenticate user after scanning QR Code
    public function authenticateUser(Request $request, $userId)
    {
        if ($request->user_id == $userId) {
            // Successful authentication, set the session
            Session::put('whatsapp_authenticated', true);
            return response()->json(['status' => 'success', 'message' => 'User authenticated successfully']);
        }
    ...
    {
        if (Session::has('whatsapp_authenticated')) {
    ...
    {
        self::drKlinikMusteriEkle($request->salonid,$request->dosya);
    }
    public function drKlinikRandevuAktarma (Request $request)
    {
        self::drKlinikRandevuAktar($request->salonid,$request->dosya);
    }
    public function drKlinikPersonelAktarma(Request $request)
    {
        self::drKlinikPersonelAktar($request->salonid,$request->dosya);
    }
    public function drKlinikSatisAktarma(Request $request)
    {
        self::drKlinikSatisAktar($request->salonid,$request->dosya);
    }
}