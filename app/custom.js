...
$(document).on('click','#bir_hizmet_daha_ekle',function (e) {
    ...
    $(document).find(".custom-select2, .opsiyonelSelect").each(function(index){
    ...
    $("select.custom-select2").each(function(i){
        ...
        $(this).attr('id', currentId);  // ID'yi geri ata
    });
...
$(document).on('submit','#paket_formu,#paket_formu_duzenleme',function(e){
    ...
    $('select[name="hizmetler[]"]').each(function(){
        if($(this).val()=="")
            hizmet_secili = false;
    });
    ...
    swal({
        ...
        if(result.value)
        {
            $.ajax({
                ...
                dataType: "json",
                data : {tahsilatid:id,adisyon_id:adisyonid,tahsilatekrani:$('#tahsilat_ekrani').length,sube:$('input[name="sube"]').val()},
                headers: {
                    'X-CSRF-TOKEN': $('input[name="_token"]').val()
                },
                beforeSend: function() {
    ...
    if(durum && tur==1)
    {
        ...
        title: "Onayla",
          html: "<p>Paket satışına devam etmek için lütfen aşağıdan başlangıç tarihi seçip seans gün aralığını belirleyin!</p>"+
                "<div class='row'><div class='col-md-4'><label>Seans Başlangıç Tarihi</label><input type='text' class='form-control'"+
                " id='ongorusme_satis_seans_baslangic'></div><div class='col-md-4'><label>Seans Aralığı (Gün)</label><input type='tel' class='form-control' "+
                "id='ongorusme_satis_seans_araligi'></div><div class='col-md-4'><label>Saat</label><input type='time' class='form-control' value='09:00' style=' height:30px;' id='ongorusme_satis_saat'></div></div>",
        type: "warning",
        showCancelButton: true,
        confirmButtonColor: '#00bc8c',
        confirmButtonText: 'Gönder',
        ...
        cancelButtonClass: 'btn btn-danger',
            ...
            if(result.value){
                if($('#ongorusme_satis_seans_baslangic').val()!='' && $('#ongorusme_satis_seans_araligi').val()!=''&& $('#ongorusme_satis_saat').val()!='')
                {
                    $.ajax({
                        type: "POST",
                        url: '/isletmeyonetim/ongorusmesatisyapildi',
                        dataType: "json",
                        data : {sube:$('input[name="sube"]').val(),ongorusmeid:ongorusme_id,_token:$('input[name="_token"]').val(),baslangic_tarihi:$('#ongorusme_satis_seans_baslangic').val(),seans_araligi:$('#ongorusme_satis_seans_araligi').val(),seans_saati:$('#ongorusme_satis_saat').val()},
                        headers: {
                            'X-CSRF-TOKEN': $('input[name="_token"]').val()
                        },
                        beforeSend: function() {
           ...
           nowIndicator:true,
             ...
             eventDragStop: function( event, jsEvent, view){
              ...
              eventRender: function(event, element) {
                ...
                // Stilin doğru uygulanmasını sağlamak için yeniden kontrol et
                if (event.title === 'Boş slot') {
                    $(element).find('.fc-title, .fc-time').css({
                        color: 'transparent',
                     ...
                     swal(
                      ...
                      $('#randevu_saat').val(tarihsaattext[1]);
                       ...
                       if($('#randevu_ayarina_gore').val()==1){
                            $('#randevupersonelleriyeni').append(seciliOption).trigger('change');
                            if(event.odaId != ""){
                       ...
                       if($('#randevu_ayarina_gore').val()==3){
                            $('select[name="randevuodalariyeni[]"]').append(seciliOption).trigger('change');
                             if(event.personelId != ""){
                                ...
                                $('#randevupersonelleriyeni').append(personelOption).trigger('change');
                            }
                            else
                                $('#randevupersonelleriyeni').val(null).trigger('change');
                       }
                      //randevusaatlerinigetir(tarihsaattext[0],$('input[name="sube"]').val(),tarihsaattext[1]);
                      jQuery("#modal-view-event-add").modal();
                   }
        ...
        if(result.value){
            if($('#dogrulama_kodu_ayari').val()==1)
            {
                $.ajax({
                                ...
                                dataType: "text",
                                data : {randevuid:randevu_id,_token: $('input[name="_token"]').val(),sube:$('input[name="sube"]').val()},
                                headers: {
                                    'X-CSRF-TOKEN': $('input[name="_token"]').val()
                                },
                                beforeSend: function() {
            ...
            else
            {
                $.ajax({
                                            ...
                                            dataType: "json",
                                            data : {randevuid:randevu_id,dogrulama_kodu:'',sube:$('input[name="sube"]').val()},
                                            headers: {
                                                'X-CSRF-TOKEN': $('input[name="_token"]').val()
                                            },
                                            beforeSend: function() {
                        ...
                        confirmButtonClass: 'btn btn-success',
                            ...
                            if(result.value){
                                $.ajax({
                                    ...
                                    url: '/isletmeyonetim/randevugeldiisaretle',
                                    dataType: "json",
                                    data : {randevuid:randevu_id,dogrulama_kodu:result.value,sube:$('input[name="sube"]').val()},
                                    headers: {
                                        'X-CSRF-TOKEN': $('input[name="_token"]').val()
                                    },
                                    beforeSend: function() {
    ...
    $('#adisyon_liste_personel').DataTable({
                       ...
                       "serverSide": true,
                        "ajax": {
                            ...
                            "data": function (d) {
                               var sube = $('input[name="sube"]').val();
                                d.tarih1=$('#personel_rapor_tarih_araligi_1').val(),
                                d.tarih2=$('#personel_rapor_tarih_araligi_2').val(),
                                d.ay=$('#personel_rapor_ay').val(),
                                d.yil=$('#personel_rapor_yil').val(),
                                d.sube = sube;
                                d.tur = $('#adisyon_icerigi_personel').val();
                                d.musteri_id=$('#adisyon_musteriye_gore_filtrele').val();
                                d.tariharaligi=$('#adisyon_tarihe_gore_filtre').val();
                             ...
                             "dataSrc": function (result) {
                                // API'den gelen ekstra verileri HTML elementlerine ekle
                                $('#hizmet_satisi').text(result.hizmetSatislariToplami);
                                $('#hizmet_primi').text(result.hizmetHakedisleriToplami);
                                $('#urun_satisi').text(result.urunSatislariToplami);
                                $('#urun_primi').text(result.urunHakedisleriToplami);
                                $('#paket_satisi').text(result.paketSatislariToplami);
                                $('#paket_primi').text(result.paketHakedisleriToplami);
                                $('#toplam_hakedis').text(result.primHakedisleriToplami);
                                
                                return result.data; // Sadece DataTable'a uygun veriyi döndür
                            },
                            "error": function(xhr, error, code) {
                                console.error('DataTables AJAX error:', error, code);
                                console.error('XHR:', xhr);
...
function kampanyaolustur(tur)
{
    ...
    $('input[name="kampanya_katilimci_musteriler[]"]').each(function() {
      if ($(this).is(":checked")) {
        secilensayi = secilensayi + 1;
      }
    });
    $('input[name="grup_katilimci_musteriler[]"]').each(function() {
      if ($(this).is(":checked")) {
        secilensayi = secilensayi + 1;
      }
    });
         ...
         swal(
                            {
                                type: "warning",
                                title: "Uyarı",
                                html:  'Devam etmek için lütfen kampanya katılımcılarını seçiniz.',
                                showCloseButton: false,
                                showCancelButton: false,
                                showConfirmButton:false,
                                timer:3000,
     ...
     swal({
        ...
        if(result.value){
            $.ajax({
                ...
                data: { portfoy_id : portfoyid,sube:$('input[name="sube"]').val()},
                dataType: "json",
                headers: {
                    'X-CSRF-TOKEN': $('input[name="_token"]').val()
                },
...
$('.tab-pane,.tab-content, button[data-toggle="tab"],a[data-toggle="tab"]').on('shown.bs.tab',function(event){
    ...
    if($(this).attr('href')=="#aktif-musteriler")
        url = '/isletmeyonetim/musterilistegetir/1';
    if($(this).attr('href')=='#tum_adisyonlar' ||$(this).attr('href')=='#hizmet_adisyonlar' ||$(this).attr('href')=='#ürün_adisyonlar'||$(this).attr('href')=='#paket_adisyonlar' )
    {
        adisyonlistefiltre();
    }
    if(url != '')
...
$('#adisyon_liste,#adisyon_liste_paket,#adisyon_liste_hizmet,#adisyon_liste_urun').on('click','button[name="adisyon_sil"]',function(){
...
$('#adisyon_liste_musteri').on('click','button[name="adisyon_sil"]',function(){
    var adisyonid = $(this).attr('data-value');
     swal({
                        title: "Emin misiniz?",
                        text: "Adisyon kaydını silmek istediğinize emin misiniz? Adisyona ait tüm içerik, tahsilat, ve alacak kayıtları silinecek olup bu işlem geri alınamayacaktır.",
...
+"</p></div><div class='col-md-3'><label>Başlangıç tarihi</label><input  style=' height:30px;' type='text' class='form-control date-picker'"+
                " name='paket_satis_seans_baslangic[]'></div><div class='col-md-3'><label>Randevu Saati</label><input type='time' class='form-control' value='11:00' style=' height:30px;' name='paket_satis_seans_saati[]'></div>"+
                "<div class='col-md-3'><label>Seans Aralığı (Gün)</label><input type='tel' class='form-control' style=' height:30px;' name='paket_satis_seans_araligi[]'></div>"+
                 "<div class='col-md-3'><label>Personel Seç</label><select class='form-control custom-select2 opsiyonelSelect personel_secimi' name='paket_satis_personel_id[]'>" +
            "<option></option>" +
            "</select></div></div>";
    });
...
$('#tum_tahsilatlar').on('click','.tahsilat_hizmet_hediye_ver', function(){
    $.ajax({
                    ...
                    dataType: "json",
                    data : {_token:$('input[name="_token"]').val(),adisyonhizmetid :$(this).attr('data-value'),adisyon_id :$('input[name="adisyon_id"]').val(),sube:$('input[name="sube"]').val()},
                    success: function(result)  {
                        $('#tum_tahsilatlar').empty();
                        $('#tum_tahsilatlar').append(result.kalemler);
                        tahsilatyenidenhesapla();
...
$('#tum_tahsilatlar').on('click','.tahsilat_urun_hediye_ver', function(){
     $.ajax({
                    ...
                    dataType: "json",
                    data : {_token:$('input[name="_token"]').val(),adisyonurunid :$(this).attr('data-value'),adisyon_id :$('input[name="adisyon_id"]').val(),sube:$('input[name="sube"]').val()},
                    success: function(result)  {
                        $('#tum_tahsilatlar').empty();
                        $('#tum_tahsilatlar').append(result.kalemler);
                        tahsilatyenidenhesapla();
...
$('#tum_tahsilatlar').on('click','.tahsilat_urun_hediye_kaldir', function(){
     $.ajax({
                    ...
                    dataType: "json",
                    data : {_token:$('input[name="_token"]').val(),adisyonurunid :$(this).attr('data-value'),adet: $('input[name="urun_adet_girilen[]"][data-value="'+$(this).attr('data-value')+'"]').val(),adisyon_id :$('input[name="adisyon_id"]').val(),sube:$('input[name="sube"]').val()},
                    success: function(result)  {
                        $('#tum_tahsilatlar').empty();
                        $('#tum_tahsilatlar').append(result.kalemler);
                        tahsilatyenidenhesapla();
...
$('#tum_tahsilatlar').on('click','.tahsilat_paket_hediye_ver', function(){
     $.ajax({
                    ...
                    dataType: "json",
                    data : {_token:$('input[name="_token"]').val(),adisyonpaketid :$(this).attr('data-value'),adisyon_id :$('input[name="adisyon_id"]').val(),sube:$('input[name="sube"]').val()},
                    success: function(result)  {
                        $('#tum_tahsilatlar').empty();
                        $('#tum_tahsilatlar').append(result.kalemler);
                        tahsilatyenidenhesapla();
    ...
    swal({
        ...
        if(result.value){
                $.ajax({
                    ...
                    url: '/isletmeyonetim/paketadisyondansil',
                    dataType: "json",
                    data : {adisyonpaketid:id,musteri_id:musteriid,adisyon_id :$('input[name="adisyon_id"]').val(),sube:$('input[name="sube"]').val()},
                    headers: {
                        'X-CSRF-TOKEN': $('input[name="_token"]').val()
                    },
                    beforeSend: function() {
...
$(document).on('click', '.clickable-slot', function() {
    let slotId = $(this).attr('data-event-id'); // Event ID'yi al
    alert('Boş slot tıklandı: ' + slotId);
});